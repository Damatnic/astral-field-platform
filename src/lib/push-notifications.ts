...
class PushNotificationService { private swRegistration: ServiceWorkerRegistration | null = null;
  ...
  private async initializeServiceWorker()  { if (!("serviceWorker" in navigator)) {
  ...
  async requestPermission(): Promise<"granted" | "denied" | "default"> { if (!("Notification" in window)) {
  ...
  async subscribeToPushNotifications(): Promise<PushSubscription | null> { if (!this.swRegistration) {
  ...
  async unsubscribeFromPushNotifications(): Promise<boolean> { if (!this.swRegistration) {
      return false;
     }
  ...
  async getSubscription(): Promise<PushSubscription | null> { if (!this.swRegistration) {
      return null;
     }
  ...
  showLocalNotification(payload: NotificationPayload) { if (!("Notification" in window)) {
  ...
  private getNotificationDuration(priority: string); number { switch (priority) {
      case 'urgent':
