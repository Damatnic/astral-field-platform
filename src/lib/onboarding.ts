...
export function updateOnboardingPreferences(preferences: Partial<OnboardingProgress['preferences']>): void {
  const progress = getOnboardingProgress()
...
export const FEATURE_HINTS: FeatureHint[] = [
  ...
  },
  {
    ...
    content: 'Let our AI suggest the best possible lineup based on projections and matchups.',
    showAfter: 3,
    showOnPages: ['/roster'],
    priority: 'high'
  },
  {
    ...
    content: 'Get instant analysis on trade fairness and impact before proposing.',
    showAfter: 2,
    showOnPages: ['/trades'],
    priority: 'high'
  },
  {
    ...
    content: 'Add Astral Field to your home screen for offline access and push notifications!',
    showAfter: 10,
    expireAfter: 30,
    priority: 'medium'
  }
...
export function shouldShowFeatureHint(hintId: string, pageViews: number = 0, currentPath: string = ''): boolean {
  const progress = getOnboardingProgress()
  ...
  if (hint.showOnPages && !hint.showOnPages.some(page => currentPath.includes(page))) {
    return false
  }
  ...
  // Check expiration
  if (hint.expireAfter) {
    const daysSinceFirstVisit = (new Date().getTime() - progress.lastActive.getTime()) / (1000 * 60 * 60 * 24)
