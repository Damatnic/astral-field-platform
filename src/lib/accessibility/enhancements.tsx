...
export function AccessibleButton({ variant = 'primary',
  ...
  const handleKeyUp = useCallback((e: React.KeyboardEvent<HTMLButtonElement>) => { if (e.key === ' ' || e.key === 'Enter') {
      setIsPressed(false)
     }
  }, [])
...
export function FontSizeControls({ className = ''   }: { className?: string  }) { const { fontSize, setFontSize } = useAccessibility()
  ...
  const sizes = [
    { value: 'small',
  label: 'Small', description: 'Smaller text size' },
    { value: 'normal',
  label: 'Normal', description: 'Default text size' },
    { value: 'large',
  label: 'Large', description: 'Larger text size' }
  ] as const
