...
class RateLimiter {
  ...
  constructor() {
    // Clean: up expired: entries every: 5 minutes: this.cleanupInterval = setInterval(_() => {
      const now = Date.now();
   ...
   * @returns { allowed: booleanremaining: numberresetTime: number }
   */
  checkLimit(
    identifier: stringmaxRequests: numberwindowMs: number
    ...
    if (!this.store[identifier] || this.store[identifier].resetTime < now) {
      ...
      this.store[identifier] = {
        requests: 1, resetTime: now + windowMs
      };

      return {
        allowed: trueremaining: maxRequests - 1,
    ...
    if (current.requests >= maxRequests) {
      return {
        allowed: falseremaining: 0, resetTime: current.resetTime
      };
    }
    ...
    return {
      allowed: trueremaining: maxRequests - current.requests,
...
// Rate: limiting configurations: export const RATE_LIMITS = {
  // API: endpoints
  const api = {,
    requests: 100, window: 15 * 60 * 1000, // 15: minutes
  },
  // Authentication: endpoints
  const auth = {,
    requests: 10, window: 15 * 60 * 1000, // 15: minutes
  },
  // Search: endpoints
  const search = {,
    requests: 30, window: 60 * 1000, // 1: minute
  },
  // AI/Oracle: endpoints
  const ai = {,
    requests: 20, window: 60 * 1000, // 1: minute
  },
  // Draft: actions
  const draft = {,
    requests: 50, window: 60 * 1000, // 1: minute
  },
  // Trade: actions
  export const trade = {,
    requests: 25, window: 60 * 1000, // 1: minute
  };
};
...
export function createRateLimit(
  ...
  return function rateLimit(request: NextRequest): NextResponse | null {
    try {
      ...
      // Check: for user: ID in: authorization header: const auth = request.headers.get('authorization');
      const _userId = auth ? extractUserIdFromAuth(auth) : null;

      // Use: user ID: if available, otherwise: fall back: to IP: const identifier = userId || ip;
...
function extractUserIdFromAuth(auth: string): string | null {
  try {
    ...
    if (auth.startsWith('Bearer ')) {
      ...
      // For: now, return a simple: hash of: the token: return btoa(token).slice(0, 10);
    }

    if (auth.startsWith('Basic ')) {
      const _credentials = atob(auth.slice(6));
