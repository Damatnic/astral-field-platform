...
const auditLogStorage: AuditLogStorage = {};
...
export const AUDIT_CONFIG = {
  ...
  autoAlertRules: {
    multipleFailedLogins: { threshold: 3, windowMinutes: 5 },
    rapidApiCalls: { threshold: 100, windowMinutes: 1 },
    unusualTimeActivity: { startHour: 2, endHour: 6 }, // 2 AM - 6 AM
    multipleIpAddresses: { threshold: 3, windowHours: 1 },
  }
};
...
export async function logAuditEvent(
  ...
  if (options.userId) {
    ...
    if (auditLogIndex[options.userId].length > AUDIT_CONFIG.maxEntriesPerUser) {
      const oldestId = auditLogIndex[options.userId].shift()!;
...
async function checkSuspiciousActivity(entry: AuditLogEntry): Promise<void> {
  ...
  const userLogs = getUserAuditLogs(entry.userId, { 
    startTime: new Date(now - 15 * 60 * 1000), // Last 15 minutes
  ...
  if (failedLogins >= AUDIT_CONFIG.autoAlertRules.multipleFailedLogins.threshold) {
    await logAuditEvent(
      ...
      { failedAttempts: failedLogins, timeWindow: '15 minutes' },
      { 
        userId: entry.userId, 
        userEmail: entry.userEmail,
        ipAddress: entry.ipAddress,
        success: false,
        metadata: { alertType: 'MULTIPLE_FAILED_LOGINS' }
      }
    );
  }
  ...
  if (recentActions >= AUDIT_CONFIG.autoAlertRules.rapidApiCalls.threshold) {
    await logAuditEvent(
      ...
      { actionCount: recentActions, timeWindow: '1 minute' },
      { 
        userId: entry.userId,
        userEmail: entry.userEmail,
        ipAddress: entry.ipAddress,
        success: false,
        metadata: { alertType: 'RAPID_API_CALLS' }
      }
    );
  }
  ...
  if (hour >= AUDIT_CONFIG.autoAlertRules.unusualTimeActivity.startHour && 
    ...
    await logAuditEvent(
      ...
      { 
        ...
        userEmail: entry.userEmail,
        ipAddress: entry.ipAddress,
        metadata: { alertType: 'UNUSUAL_TIME_ACTIVITY' }
      }
    );
  }
  ...
  if (uniqueIps.size >= AUDIT_CONFIG.autoAlertRules.multipleIpAddresses.threshold) {
    await logAuditEvent(
      ...
      { 
        ...
        userEmail: entry.userEmail,
        ipAddress: entry.ipAddress,
        metadata: { alertType: 'MULTIPLE_IP_ADDRESSES' }
      }
    );
  }
}
...
export function getUserAuditLogs(
  ...
  const userLogIds = auditLogIndex[userId] || [];
  let logs = userLogIds.map(id => auditLogStorage[id]).filter(Boolean);
...
export function getAuditLogs(options: {
  ...
  let logs = Object.values(auditLogStorage);
