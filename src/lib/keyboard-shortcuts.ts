...
export function useKeyboardShortcuts({
  ...
  }, [disabled, onShowHelp, onNewTrade, onOptimizeLineup, onCloseModal, onNavigateUp, onNavigateDown, onSelectItem])
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (event: KeyboardEvent) => {
      ...
      if (key === 'g' && !isInputFocused()) {
        event.preventDefault()
        handleSequenceKey('g')
      } else if (sequenceBuffer.length === 1 && sequenceBuffer[0] === 'g' && !isInputFocused()) {
        event.preventDefault()
