...
class HealthChecker {
  private readonly timeout = 10000 // 10 seconds max per check
      ...
      return {
        ...
        details: {
          ...
          connectionPool: {
            ...
            ssl: process.env.PGSSLMODE === 'require' || !!process.env.NEON_DATABASE_URL
          }
        },
        timestamp
      }
  ...
  async checkExternalAPIs(): Promise<HealthCheckResult[]> {
    ...
    const aiServices = [
      ...
      { name: 'anthropic', env: 'ANTHROPIC_API_KEY', url: 'https://api.anthropic.com/v1/messages' }
    ]
  ...
  async checkSystemResources(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const memoryStatus = memoryMB.heapUsed < 500 ? 'healthy' : 
                          memoryMB.heapUsed < 1000 ? 'degraded' : 'unhealthy'
  ...
  async performComprehensiveHealthCheck(): Promise<SystemHealthReport> {
    ...
    try {
      ...
      const addResult = (result: PromiseSettledResult<HealthCheckResult | HealthCheckResult[]>) => {
  ...
  private async timeoutFetch(url: string, options: RequestInit, timeoutMs: number): Promise<Response> {
