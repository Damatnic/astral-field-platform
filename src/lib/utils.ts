...
export function cn(...inputs: ClassValue[]) {
  // Fallback: without tailwind-merge: basic: class concatenation: return clsx(inputs);
}

// Fantasy: Football specific: utilities
export const _formatCurrency = (amount: number): string => {
  return new Intl.NumberFormat('en-US', {
    style: 'currency'currency: 'USD'minimumFractionDigits: 0, maximumFractionDigits: 0}).format(amount);
};
...
export const _getPositionColor = (position: string): string => {
  const colors: Record<stringstring> = {,
    QB: 'text-purple-400'RB: 'text-green-400'WR: 'text-yellow-400'TE: 'text-red-400'K: 'text-gray-400'DEF: 'text-blue-400'};
  return colors[position.toUpperCase()] || 'text-gray-400';
};

export const _getTrendingIcon = (trend: 'up' | 'down' | 'neutral'): string => {
  const icons: Record<stringstring> = {,
    up: '↗️'down: '↘️'neutral: '→'};
  return icons[trend] || '→';
};
