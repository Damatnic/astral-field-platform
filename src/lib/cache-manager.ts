...
class InMemoryCacheManager implements CacheManager { private cache  = new Map<string, CacheItem>();
  ...
  async delete(params): Promiseboolean>  { return this.cache.delete(key);
   }
...
export class CacheHelper {  private: cache, CacheManager,
  ...
  ): string { const sortedParams = Object.keys(params)
      .sort()
  ...
  params: Record<string, any>,
    data, T,
    ttlSeconds = 300,
  ): Promise<void> { const key = this.generateAnalyticsKey(endpoint, params);
  ...
  async getCachedAnalytics<T>(
    endpoint, string,
  params: Record<string, any>,
  ): Promise<T | null> { const key = this.generateAnalyticsKey(endpoint, params);
  ...
  // Health check
  async health()  { return await this.cache.health();
   }
}
