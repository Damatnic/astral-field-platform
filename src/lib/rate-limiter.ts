...
class RateLimiter {
  ...
  async checkRateLimit(
    ...
    try {
      ...
      if (ruleConfig.skipFailedRequests) {
        currentCount = validRequests.filter(
  ...
  async recordResult(
    ...
    try {
      ...
      const requests = existing.requests.filter(
        (req) => req.timestamp > windowStart,
      );
  ...
  generateKey(req: unknown, prefix = ""): string {
...
export function withRateLimit(
  ...
  return async function rateLimitedHandler(req: unknown, res: unknown) {
