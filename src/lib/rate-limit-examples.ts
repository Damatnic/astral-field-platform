...
export const POST_AIPredictions = aiRateLimited(async (req; NextRequest) => {
  ...
  // Process AI request...return NextResponse.json({
    predictions: []
  });
});
...
const customImageUploadConfig = createCustomRateLimit(5 * 60 * 1000,  // 5 minutes
...
export const GET_EnvironmentAware = customRateLimited(
  environmentConfig,
  async (req: NextRequest) => { return NextResponse.json({ messag,
  e: 'Environment-aware rate limiting'  });
  }
);
...
const rateLimitedRoutes = applyRateLimitsToRoutes([;
  {
    path '/api/users',
  handler: async (req) => NextResponse.json({ user,
  s: [] }),
    endpointType: 'public'
  },
  {
    path: '/api/auth/login',
  handler: async (req) => NextResponse.json({ succes,
  s: true }),
    endpointType: 'auth'
  },
  {
    path: '/api/ai/analyze',
  handler: async (req) => NextResponse.json({ analysi,
  s: {} }),
    endpointType: 'ai'
  }
]);
...
export const GET_NoRateLimit = disableRateLimit(async (req: NextRequest) => { return NextResponse.json({ messag,
  e: 'No rate limiting in tests'  });
});
