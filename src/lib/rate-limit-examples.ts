...
export const GET_CurrentLeague = standardRateLimited(async (req: NextRequest) => {
  ...
  return NextResponse.json({
    league: { id: '123', name: 'Demo League' }
  });
});
...
const customImageUploadConfig = createCustomRateLimit(
  5 * 60 * 1000,  // 5 minutes
  3,              // 3 uploads per 5 minutes
...
const environmentConfig = createEnvironmentBasedRateLimit(
...
export const GET_EnvironmentAware = customRateLimited(
  environmentConfig,
  async (req: NextRequest) => {
    return NextResponse.json({ message: 'Environment-aware rate limiting' });
  }
);
...
const timeBasedConfig = createTimeBasedRateLimit(
  RATE_LIMIT_PRESETS.STANDARD,
...
export const GET_TimeBased = customRateLimited(
  ...
  async (req: NextRequest) => {
    return NextResponse.json({ 
      message: 'Rate limiting adjusts based on time of day' 
    });
  }
);
...
export async function GET(request: NextRequest) {
  return standardRateLimited(async (req: NextRequest) => {
    // Your GET logic here
...
export async function POST(request: NextRequest) {
  return strictRateLimited(async (req: NextRequest) => {
...
const rateLimitedRoutes = applyRateLimitsToRoutes([
  {
    path: '/api/users',
    handler: async (req) => NextResponse.json({ users: [] }),
    endpointType: 'public'
  },
  {
    path: '/api/auth/login', 
    handler: async (req) => NextResponse.json({ success: true }),
    endpointType: 'auth'
  },
  {
    path: '/api/ai/analyze',
    handler: async (req) => NextResponse.json({ analysis: {} }),
    endpointType: 'ai'
  }
]);
...
export const GET_NoRateLimit = disableRateLimit(async (req: NextRequest) => {
  return NextResponse.json({ message: 'No rate limiting in tests' });
});
...
export async function POST_Migrated(request: NextRequest) {
  return rateLimitedWithErrorHandling('public', async (req: NextRequest) => {
    const body = await req.json();
