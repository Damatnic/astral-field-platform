...
export abstract class BaseAgent implements SpecializedAgent {
  ...
  abstract processTask(task: Task): Promise<{ success: boolean; result?: any; error?: string }>;
  ...
  async reportHealth(): Promise<any> {
  ...
  async reportMetrics(): Promise<any> {
  ...
  protected async processTaskWithMetrics(task: Task): Promise<{ success: boolean; result?: any; error?: string }> {
  ...
  private determineHealthStatus(): 'healthy' | 'warning' | 'critical' {
    const errorRate = this.taskCount > 0 ? (this.errorCount / this.taskCount) * 100 : 0;
    const memoryUsage = process.memoryUsage();
  ...
  protected validateTask(task: Task): { valid: boolean; reason?: string } {
    if (!task.id) {
      return { valid: false, reason: 'Task ID is required' };
    }

    if (!task.type) {
      return { valid: false, reason: 'Task type is required' };
    }

    // Check if this agent can handle this task type
    if (!this.canHandleTaskType(task.type)) {
      return { valid: false, reason: `Agent ${this.type} cannot handle task type: ${task.type}` };
    }
  ...
  // Success helper
  protected success(result?: any): { success: true; result?: any } {
    return {
      success: true,
      result
    };
