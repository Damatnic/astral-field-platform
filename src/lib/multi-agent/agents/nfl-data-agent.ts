...
export class NFLDataAgent extends BaseAgent { public, typ,
  ...
  private async handleGameDataSync(params): Promise { success, boolean, result?, any, error?: string }> { try {
      ...
      if (!gameId && !week) {
        return { success, false,
  error: 'Game ID or week is required'  }
      }
  ...
  private async handleDataValidation(params): Promise { success, boolean, result?, any, error?: string }> { try {
      const validationResults = {
        players: {,
  total: this.playerCache.size, valid, 0, invalid, 0,
  issues: [] as string[]
         },
        games: {,
  total: this.gameCache.size, valid, 0, invalid, 0,
  issues: [] as string[]
        }
      }
  ...
  private async handleHealthCheck(params): Promise { success, boolean, result?, any, error?: string }> { try {
      ...
      return this.success({
        overallHealth, sources, healthResults,
  timestamp: new Date()
      });
  ...
  private async validatePlayerData(params): Promise { valid, boolean, reason?: string }> { if (!player.id) return { valid, false,
  ...
  private async validateGameData(params): Promise { valid, boolean, reason?: string }> { if (!game.gameId) return { valid, false,
