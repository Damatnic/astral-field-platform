...
export class NFLDataAgent extends BaseAgent {
  public type: AgentType = 'nfl-data';
  ...
  protected async getSpecializedMetrics(): Promise<any> {
    return {
      dataFreshness: {
        playerDataAge: this.calculateDataAge('players'),
  ...
  // Task handlers
  private async handleDataSync(task: Task): Promise<{ success: boolean; result?: any; error?: string }> {
    try {
      const syncResults = [];
      ...
      // Sync game data
      if (task.metadata?.syncGames !== false) {
        const gameResult = await this.syncGameData();
        syncResults.push({ type: 'games', ...gameResult });
      }
      
      // Sync live game updates if specified
      if (task.metadata?.liveGames) {
        const liveResult = await this.syncLiveGameData(task.metadata.liveGames);
      ...
      return this.handleError(error, 'handleDataSync');
    }
  }

  private async handlePlayerStatsUpdate(task: Task): Promise<{ success: boolean; result?: any; error?: string }> {
    try {
      ...
      return this.handleError(error, 'handlePlayerStatsUpdate');
    }
  }

  private async handleGameDataSync(task: Task): Promise<{ success: boolean; result?: any; error?: string }> {
    try {
      ...
      if (gameId) {
        ...
        games = await this.fetchWeekGameData(week);
      }
      ...
      return this.handleError(error, 'handleGameDataSync');
    }
  }

  private async handleDataValidation(task: Task): Promise<{ success: boolean; result?: any; error?: string }> {
    try {
      const validationResults = {
        players: {
          total: this.playerCache.size,
          valid: 0,
          invalid: 0,
          issues: [] as string[]
        },
        games: {
          total: this.gameCache.size,
          valid: 0,
          invalid: 0,
          issues: [] as string[]
        }
      ...
      for (const [playerId, playerData] of this.playerCache) {
        const validation = await this.validatePlayerData(playerData);
        if (validation.valid) {
      ...
      // Validate game data
      for (const [gameId, gameData] of this.gameCache) {
      ...
      return this.handleError(error, 'handleDataValidation');
    }
  }

  private async handleHealthCheck(task: Task): Promise<{ success: boolean; result?: any; error?: string }> {
    try {
      const healthResults = [];
      ...
      return this.handleError(error, 'handleHealthCheck');
    }
  }
  ...
  private async syncPlayerData(): Promise<{ success: boolean; updated: number; errors: number }> {
    let updated = 0;
    let errors = 0;
  ...
  private async syncGameData(): Promise<{ success: boolean; updated: number; errors: number }> {
    let updated = 0;
    let errors = 0;
  ...
  private async syncLiveGameData(gameIds: string[]): Promise<{ success: boolean; updated: number; errors: number }> {
    let updated = 0;
    let errors = 0;
  ...
  // Helper methods
  private async fetchPlayerData(playerId: string): Promise<PlayerData | null> {
  ...
  private async fetchGameData(gameId: string): Promise<GameData | null> {
    for (const [sourceName, source] of this.dataSources) {
      if (!source.isActive) continue;
  ...
  private async fetchWeekGameData(week: number): Promise<GameData[]> {
    // Implementation would fetch all games for the specified week
    // This is a simplified version
    return [];
  }

  private async fetchLiveGameData(gameId: string): Promise<GameData | null> {
  ...
  private async validatePlayerData(player: PlayerData): Promise<{ valid: boolean; reason?: string }> {
    if (!player.id) return { valid: false, reason: 'Missing player ID' };
    if (!player.name) return { valid: false, reason: 'Missing player name' };
    if (!player.team) return { valid: false, reason: 'Missing team' };
    if (!player.position) return { valid: false, reason: 'Missing position' };
    
    // Additional validation logic...
    return { valid: true };
  }

  private async validateGameData(game: GameData): Promise<{ valid: boolean; reason?: string }> {
    if (!game.gameId) return { valid: false, reason: 'Missing game ID' };
    if (!game.homeTeam) return { valid: false, reason: 'Missing home team' };
    if (!game.awayTeam) return { valid: false, reason: 'Missing away team' };
  ...
  private async checkDataSourceHealth(name: string, source: NFLDataSource): Promise<{ healthy: boolean; responseTime?: number; error?: string }> {
    try {
  ...
  private async makeAPIRequest(source: NFLDataSource, endpoint: string, options?: any): Promise<any> {
    // Implementation would make actual HTTP request
    // This is a placeholder
    return { data: 'mock response' };
  }

  private async canMakeRequest(sourceName: string): Promise<boolean> {
    const source = this.dataSources.get(sourceName);
    if (!source) return false;
    
    const limiter = this.rateLimiters.get(sourceName);
    if (!limiter) {
      this.rateLimiters.set(sourceName, { requests: 1, resetTime: Date.now() + 1000 });
      return true;
    }
  ...
  private async respectRateLimit(operation: string): Promise<void> {
    // Simple rate limiting - wait 100ms between operations
    await new Promise(resolve => setTimeout(resolve, 100));
  }

  private getRateLimitStatus(sourceName: string): any {
    const limiter = this.rateLimiters.get(sourceName);
    const source = this.dataSources.get(sourceName);
  ...
  private getCurrentNFLWeek(): number {
    return 1; // Placeholder - would calculate current NFL week
  }
  ...
  private calculateDataAge(type: string): number {
    return 0; // Placeholder - age in minutes
  }
  ...
  private async getSuccessfulRequestCount(): Promise<number> {
    return this.taskCount - this.errorCount; // Simplified
  }
  ...
  private async calculateErrorRate(): Promise<number> {
    return this.taskCount > 0 ? (this.errorCount / this.taskCount) * 100 : 0;
  }
