...
export class WebSocketManager {
  ...
  private setupEventHandlers(): void {
    ...
    this.io.on('connection', (socket: Socket) => {
      ...
      socket.on('task:status', (data: { taskId: string; status: string; progress?: number; metadata?: any }) => {
      ...
      socket.on('agent:heartbeat', (data: { health: any; metrics?: any }) => {
        const connection = this.connections.get(socket.id);
        if (connection) {
      ...
      socket.on('knowledge:update', (data: {
        type: 'best_practice' | 'bug_fix' | 'optimization' | 'pattern';
        title: string;
        content: string;
        tags: string[];
        files: string[];
  ...
  private async sendDirectMessage(message: AgentMessage): Promise<boolean> {
    if (!this.io) {
  ...
  broadcast(eventType: string, data: any, channel?: string): void {
    if (!this.io) return;
  ...
  getConnectionStatus(): {
    ...
    return {
      ...
      agentsByType,
      activeChannels: this.io ? this.io.sockets.adapter.rooms.size : 0
    };
  }

  private async validateAgent(authToken: string, agentId: string): Promise<boolean> {
  ...
  private handleAgentMessage(socket: Socket, message: AgentMessage): void {
    const connection = this.connections.get(socket.id);
  ...
  private handleConflictAlert(agentId: string, message: AgentMessage): void {
  ...
  private handleCoordinationMessage(agentId: string, message: AgentMessage): void {
    console.log(`🤝 Coordination message from ${agentId}:`, message.content);
  }

  private handleErrorMessage(agentId: string, message: AgentMessage): void {
    console.error(`❌ Error from ${agentId}:`, message.content.error);
  }

  private handleConflictReport(agentId: string, data: any): void {
  ...
  private handleKnowledgeUpdate(agentId: string, data: any): void {
  ...
  private handleAgentDisconnect(socket: Socket, reason: string): void {
    const connection = this.connections.get(socket.id);
    if (connection) {
  ...
  sendTaskAssignment(agentId: string, task: any): void {
  ...
  broadcastSystemAlert(alertType: string, data: any, severity: 'low' | 'medium' | 'high' = 'medium'): void {
    const message: AgentMessage = {
      id: this.generateMessageId(),
      type: 'coordination',
      senderId: 'coordinator',
  ...
  sendQualityGateRequest(agentId: string, taskId: string, gateId: string): void {
    const message: AgentMessage = {
      id: this.generateMessageId(),
      type: 'coordination',
      senderId: 'coordinator',
