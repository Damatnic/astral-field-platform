...
interface AssignmentStrategy {
  name: string;
  execute(task: Task, availableAgents: Array<{ agentId: string; agentType: AgentType; load: number; performance: any }>): string | null;
}
...
export class TaskQueue {
  ...
  getNextTaskForAgent(agentId: string, agentType: AgentType, agentLoad: number, agentPerformance: any): Task | null {
    // Find the best task for this specific agent
    for (let i = 0; i < this.priorityQueue.length; i++) {
      const queueItem = this.priorityQueue[i];
  ...
  blockTask(taskId: string, reason: string, unblockAt?: Date): void {
    const queueItem = this.queue.get(taskId);
    if (!queueItem) return;
  ...
  unblockTask(taskId: string): void {
    const queueItem = this.blockedTasks.get(taskId);
    if (!queueItem) return;
  ...
  findBestAgent(task: Task, availableAgents: Array<{ agentId: string; agentType: AgentType; load: number; performance: any }>): string | null {
    const strategy = this.assignmentStrategies.get(this.config.taskAssignmentStrategy);
    if (!strategy) {
  ...
  private calculatePriority(task: Task): number {
    ...
    if (task.dependencies.length === 0) {
      score += 100; // Bonus for no dependencies
    } else {
      score -= task.dependencies.length * 50; // Penalty for dependencies
    }

    // Estimated duration factor (shorter tasks get slight boost)
    if (task.estimatedDuration <= 30) {
      score += 50; // Quick tasks
    } else if (task.estimatedDuration >= 240) {
      score -= 100; // Long tasks get lower priority
    }

    return Math.round(score);
  }

  private calculateSkillMatchScore(task: Task): number {
    // This would be enhanced with actual skill matching logic
  ...
  private calculateResourceRequirements(task: Task): number {
  ...
  private calculateAgentTaskScore(agent: any, task: Task): number {
    let score = 0;
    ...
    if (avgResponseTime < task.estimatedDuration * 0.8) {
      score += 20; // Bonus if agent is typically faster
    }

    return score;
  }

  private calculateAgentCapabilityScore(agent: any, task: Task): number {
  ...
  private isTaskSuitableForAgent(task: Task, agentType: AgentType, agentLoad: number): boolean {
    // Check load capacity
    if (agentLoad >= 90) return false;

    // Check agent type suitability
    if (this.isAgentTypeForTask(agentType, task.type)) {
      return true;
    }
  ...
  private isAgentTypeForTask(agentType: AgentType, taskType: string): boolean {
  ...
  private processBlockedTasks(): void {
    const now = new Date();
  ...
  private processFailedAssignments(): void {
    ...
    for (const queueItem of this.priorityQueue) {
      if (
  ...
  recordAssignmentAttempt(taskId: string): void {
    const queueItem = this.queue.get(taskId);
    if (queueItem) {
  ...
  getTasksWaitingForDependencies(): Task[] {
