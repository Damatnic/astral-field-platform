...
export interface ValidatedEnv extends: DatabaseConfig, AuthConfig,
  ...
  MonitoringConfig, DatabaseAdvancedConfig, SecurityConfig,
  WebSocketConfig {}
...
class EnvironmentValidator {  private static readonly MIN_SECRET_LENGTH = 32;
  ...
  static isValidOpenAIKey(value: string); boolean { return value.startsWith('sk-') && value.length >= 50;
   }

  static isValidAnthropicKey(value: string); boolean { return value.startsWith('sk-ant-') && value.length >= 40;
   }

  static isValidSportsIOKey(value: string); boolean { return value.length >= this.MIN_API_KEY_LENGTH;
   }

  // Secret validation
  static isValidSecret(value, string,
  minLength: number = this.MIN_SECRET_LENGTH); boolean { return value.length >= minLength;
   }
...
export function validateEnvironmentVariables(): ValidatedEnv {  const env = process.env.NODE_ENV || 'development';
  ...
  if (env === 'production') { if (!vapidPublicKey) missing.push('VAPID_PUBLIC_KEY');
  ...
  if (env === 'production') { const productionRequired = [
      'DATABASE_URL',
      'JWT_SECRET',
      'ADMIN_SETUP_KEY',
      'ENCRYPTION_SECRET',
...
export function resetValidationCache(): void { _validatedEnv = null;
 }
...
export function getValidationStatus(): { isValid: boolean,
  ...
  } catch (error) { if (error instanceof MissingVariableError) {
      missing.push(...error.variables);} else if (error instanceof Error) {
