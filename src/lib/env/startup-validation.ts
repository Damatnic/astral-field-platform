...
export function validateStartupEnvironment(): StartupValidationResult { try {
    ...
    if (env === 'production') { if (!process.env.REDIS_URL && !process.env.KV_URL && !process.env.UPSTASH_REDIS_URL) {
 ...
 */
export function validateOrExit(): void { if (!printStartupValidation()) {
