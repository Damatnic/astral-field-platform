...
function printColored(message: string, color: string = colors.white): void {
...
function printStatus(label: string, status: boolean, details?: string): void {
...
function printSetupInstructions(missingVars: string[]): void {
...
function generateEnvExample(missingVars: string[]): string {
...
export function checkEnvironment(options: {
  ...
  verbose?: boolean;
} = {}): { isValid: boolean; env?: ValidatedEnv } {
  const { 
    exitOnError = false, 
    showWarnings = true, 
    verbose = false 
  ...
  if (status.isValid) {
    ...
    if (verbose) {
      try {
        ...
        return { isValid: true, env: validatedEnv };
      } catch (error) {
        printError(`Unexpected validation error: ${error instanceof Error ? error.message : 'Unknown error'}`);
