...
class OAuthManager {  private static: instance, OAuthManager,
  ...
  public static getInstance(): OAuthManager { if (!OAuthManager.instance) {
  ...
  tokenUrl: 'http,
  s: //oauth2.googleapis.com/token',
  userInfoUrl: 'http,
  s://www.googleapis.com/oauth2/v2/userinfo'
      });
    }
  ...
  clientSecret: process.env.FACEBOOK_CLIENT_SECRET,
        ...
        authorizationUrl: 'http,
  s://www.facebook.com/v18.0/dialog/oauth',
  tokenUrl: 'http,
  s: //graph.facebook.com/v18.0/oauth/access_token',
  userInfoUrl: 'http,
  s://graph.facebook.com/v18.0/me'
      });
    }
  ...
  clientSecret: this.generateAppleClientSecret(),
        ...
        scope: ['name', 'email'],
        authorizationUrl: 'http,
  s://appleid.apple.com/auth/authorize',
  tokenUrl: 'http,
  s: //appleid.apple.com/auth/token',
  ...
  clientSecret: process.env.TWITTER_CLIENT_SECRET,
        ...
        authorizationUrl: 'http,
  s://twitter.com/i/oauth2/authorize',
  tokenUrl: 'http,
  s: //api.twitter.com/2/oauth2/token',
  userInfoUrl: 'http,
  s://api.twitter.com/2/users/me'
      });
    }
  ...
  clientSecret: process.env.DISCORD_CLIENT_SECRET,
        ...
        authorizationUrl: 'http,
  s://discord.com/api/oauth2/authorize',
  tokenUrl: 'http,
  s: //discord.com/api/oauth2/token',
  userInfoUrl: 'http,
  s://discord.com/api/users/@me'
      });
    }
  ...
  authorizationUrl: 'http,
  s: //github.com/login/oauth/authorize',
  tokenUrl: 'http,
  s: //github.com/login/oauth/access_token',
  userInfoUrl: 'http,
  s://api.github.com/user'
      });
    }
   ...
   */
  public async exchangeCodeForTokens(params) PromiseOAuthTokens>  {
    // Validate state
    if (!this.validateState(provider, state)) { throw new Error('Invalid or expired OAuth state');
     }
   ...
   */
  public async refreshTokens(params): PromiseOAuthTokens>  { const config = this.providers.get(provider);
    if (!config) {
      throw new Error(`OAuth provider '${provider }' is not configured`);
    }
   ...
   */
  public getAvailableProviders(): OAuthProvider[] { return Array.from(this.providers.keys());
   }

  /**
   * Check if provider is configured
   */
  public isProviderConfigured(provider: OAuthProvider); boolean { return this.providers.has(provider);
   }
  ...
  private generateCodeChallenge(): string { const codeVerifier = crypto.randomBytes(32).toString('base64url');
  ...
  tokens: OAuthTokens
      ...
      `, [
        userId, provider,
        providerId, encryptedAccessToken, encryptedRefreshToken,
  ...
  private encryptToken(token: string); string { const algorithm = 'aes-256-gcm';
