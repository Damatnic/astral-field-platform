...
class SessionManager {
  ...
  public async createSession(
    ...
    try {
      ...
      const expiresAt = options.rememberMe 
        ? new Date(now.getTime() + this.settings.refreshTokenTimeout)
        : new Date(now.getTime() + this.settings.sessionTimeout);
      ...
      let location;
      if (this.settings.enableLocationTracking && enhancedDeviceInfo.ipAddress) {
        location = await this.getLocationFromIP(enhancedDeviceInfo.ipAddress);
      }
      ...
      await auditLogger.logEvent({
        ...
        eventCategory: 'login',
        severity: riskAssessment.score > 70 ? 'high' : 'info',
      ...
      if (riskAssessment.score > 80) {
        await this.sendSecurityAlert(userId, sessionData, riskAssessment);
      }
  ...
  public async validateSession(token: string): Promise<{
    ...
    try {
      ...
      if (suspiciousActivity.isSuspicious) {
        await this.flagSession(sessionId, {
        ...
        if (suspiciousActivity.shouldTerminate) {
          await this.terminateSession(sessionId, 'suspicious_activity');
          return { valid: false, error: 'Session terminated due to suspicious activity' };
        }
      }
  ...
  public async refreshSession(refreshToken: string): Promise<{
    ...
    try {
      ...
      // Generate new tokens
      const tokenPayload = { userId: session.userId, sessionId: session.id, type: 'access' };
  ...
  public async terminateSession(
    sessionId: string, 
    reason: 'logout' | 'expired' | 'suspicious_activity' | 'admin_action' | 'concurrent_limit'
  ): Promise<boolean> {
    try {
      ...
      if (session) {
        ...
        await auditLogger.logEvent({
          ...
          severity: reason === 'suspicious_activity' ? 'high' : 'info',
   ...
   */
  public async terminateOtherUserSessions(userId: string, currentSessionId: string): Promise<number> {
    try {
      const result = await database.query(`
        UPDATE user_sessions 
        SET is_active = false 
   ...
   */
  public async getSessionAnalytics(userId?: string, timeRange: number = 24 * 60 * 60 * 1000): Promise<{
    ...
    try {
      ...
      return {
        ...
        averageSessionDuration: analytics.totalSessions > 0 ? analytics.totalDuration / analytics.totalSessions : 0,
  ...
  private getDefaultSettings(): SessionSecuritySettings {
    return {
  ...
  private async enhanceDeviceInfo(deviceInfo: Partial<DeviceInfo>): Promise<Partial<DeviceInfo>> {
  ...
  private async assessSessionRisk(
    ...
    if (options.geoLocation) {
      const locationRisk = await this.assessLocationRisk(userId, options.geoLocation);
  ...
  private async assessIPRisk(ipAddress: string): Promise<{ score: number; flags: SessionFlag[] }> {
    ...
    if (/^(10\.|192\.168\.|172\.(1[6-9]|2[0-9]|3[01])\.|127\.)/.test(ipAddress)) {
      score = 0; // Local network
    } else {
      score = 10; // External IP has base risk
    }
  ...
  private async detectSuspiciousActivity(session: SessionData): Promise<{
  ...
  private async flagSession(sessionId: string, flag: SessionFlag): Promise<void> {
    const session = this.activeSessions.get(sessionId);
    if (session) {
    ...
    try {
      await database.query(`
        INSERT INTO user_sessions (
          ...
          device_info, ip_address, user_agent, location, risk_score,
  ...
  private mapDatabaseRowToSession(row: any): SessionData {
    return {
      id: row.id,
      userId: row.user_id,
  ...
  private async sendSecurityAlert(userId: string, session: SessionData, riskAssessment: any): Promise<void> {
