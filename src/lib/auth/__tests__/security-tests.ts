...
class SecurityTestUtils {
  ...
  static generateRandomEmail(): string {
    return `test-${this.generateRandomString(8)}@security-test.com`;
  }
}
...
describe('Security Middleware Tests', () => {
  describe('Rate Limiting', () => {
    ...
    test('should apply different rate limits per endpoint', async () => {
      ...
      const registerRequest = SecurityTestUtils.createMockRequest({
        url: `http://localhost:3000${registerEndpoint}`,
        ip
      });
  ...
  describe('IP Blocking', () => {
    test('should block malicious IP addresses', async () => {
    ...
    test('should unblock IP addresses', async () => {
...
describe('Password Security Tests', () => {
  describe('Password Validation', () => {
    ...
    test('should detect password reuse', async () => {
      ...
      const result = await enhancedPasswordSecurity.validatePassword(
        password,
        userId
      );
    ...
    test('should calculate password entropy correctly', async () => {
      const testCases = [
        { password: 'abc', expectedLowEntropy: true },
        { password: 'MyStr0ng!P@ssw0rd2024', expectedLowEntropy: false },
        { password: '1234567890', expectedLowEntropy: true }
      ];

      for (const testCase of testCases) {
        const result = await enhancedPasswordSecurity.calculatePasswordStrength(testCase.password);
  ...
  describe('Password Hashing', () => {
    ...
    test('should verify passwords correctly', async () => {
      ...
      const isValid = await enhancedPasswordSecurity.verifyPassword(
        password,
        hashResult.hash,
      ...
      const isInvalid = await enhancedPasswordSecurity.verifyPassword(
        'WrongPassword',
        hashResult.hash,
        hashResult.algorithm
      );
    ...
    test('should handle different hashing algorithms', async () => {
      const password = 'TestPassword123!';
  ...
  describe('Breach Detection', () => {
    test('should detect breached passwords', async () => {
      const breachedPassword = 'password123'; // Known weak password
...
describe('Multi-Factor Authentication Tests', () => {
  describe('MFA Setup', () => {
    ...
    test('should include available methods based on user config', async () => {
      const userWithPhone = {
        id: 'test-user-2',
        email: 'test@security-test.com',
        phoneNumber: '+1234567890'
  ...
  describe('MFA Challenges', () => {
    test('should create and verify MFA challenges', async () => {
      const userId = 'test-user-1';
    ...
    test('should handle challenge expiration', async () => {
      const userId = 'test-user-1';
      const challengeId = await enhancedMFA.createMFAChallenge(userId, 'email');
...
describe('Role-Based Access Control Tests', () => {
  describe('Permission Checking', () => {
    test('should grant access for valid permissions', async () => {
      const context = {
        userId: 'admin-user',
        resource: 'users',
        action: 'read'
  ...
  describe('Role Management', () => {
    ...
    test('should handle permission overrides', async () => {
      const override = {
        userId: 'test-user-1',
        resource: 'analytics',
        actions: ['read', 'create'],
...
describe('OAuth Integration Tests', () => {
  describe('OAuth Flow', () => {
    test('should generate valid authorization URLs', () => {
...
describe('Audit Logging Tests', () => {
  describe('Event Logging', () => {
    test('should log authentication events', async () => {
      await auditLogger.logAuthentication('test-user-1', 'login_success', {
    ...
    test('should log authorization events', async () => {
      await auditLogger.logAuthorization('test-user-1', 'teams', 'update', true, {
        ipAddress: '192.168.1.100'
      });
    ...
    test('should log security incidents', async () => {
      await auditLogger.logSecurityIncident('brute_force', 'Multiple failed login attempts detected', {
        userId: 'test-user-1',
        ipAddress: '192.168.1.100',
        severity: 'high'
      });
  ...
  describe('Event Querying', () => {
    test('should query events by criteria', async () => {
      const query = {
        userId: 'test-user-1',
...
describe('Integration Tests', () => {
  ...
  describe('Security Under Load', () => {
    test('should handle concurrent requests without security bypass', async () => {
      const endpoint = '/api/auth/login';
      const ip = '192.168.1.300';
      
      // Create multiple concurrent requests
      const requests = Array(20).fill(null).map(() =>
        SecurityTestUtils.createMockRequest({ url: `http://localhost:3000${endpoint}`, ip })
      );

      const results = await Promise.allSettled(
        requests.map(req => securityMiddleware.validateRequest(req, endpoint))
      );
  ...
  describe('Data Consistency', () => {
    test('should maintain data integrity under concurrent operations', async () => {
      // Test concurrent role assignments, permission grants, etc.
