...
class RBACManager {
   ...
   * Assign role to user
   */
  public async assignRole(userId: string, role: UserRole, assignedBy: string, reason?: string): Promise<boolean> {
    try {
   ...
   */
  public async grantPermissionOverride(
   ...
   */
  public async revokePermissionOverride(
    userId: string,
    resource: string,
    revokedBy: string
  ): Promise<boolean> {
   ...
   */
  public async getEffectivePermissions(userId: string): Promise<Permission[]> {
   ...
   * Get role definition
   */
  public getRoleDefinition(role: UserRole): RoleDefinition | null {
  ...
  ): Promise<Array<{ resource: string; action: string; granted: boolean; reason: string }>> {
    const results = [];
  ...
  ): Promise<{ passes: boolean; reason: string; suggestions?: string[] }> {
    // No conditions means permission is granted
    if (Object.keys(conditions).length === 0) {
      return { passes: true, reason: 'No conditions to check' };
    }

    try {
      // Check ownership condition
      if (conditions.owner && context.ownerId !== context.userId) {
        return {
          passes: false,
      ...
      if (conditions.sameLeague) {
        const inSameLeague = await this.checkSameLeague(context.userId, context.leagueId);
        if (!inSameLeague) {
          return {
            passes: false,
      ...
      if (conditions.commissionerOf) {
        const isCommissioner = await this.checkCommissioner(context.userId, context.leagueId);
        if (!isCommissioner) {
          return {
            passes: false,
      ...
      if (conditions.public) {
        const isPublic = await this.checkResourcePublic(context.resource, context.resourceId);
        if (!isPublic) {
          return {
            passes: false,
      ...
      if (conditions.member) {
        const isMember = await this.checkLeagueMember(context.userId, context.leagueId);
        if (!isMember) {
          return {
            passes: false,
      ...
      if (conditions.participant) {
        const isParticipant = await this.checkParticipant(context.userId, context.resource, context.resourceId);
        if (!isParticipant) {
          return {
            passes: false,
      ...
      return { passes: true, reason: 'All conditions satisfied' };
    } catch (error) {
  ...
  private async checkSameLeague(userId: string, leagueId?: string): Promise<boolean> {
    if (!leagueId) return true; // No league specified
  ...
  private async checkCommissioner(userId: string, leagueId?: string): Promise<boolean> {
  ...
  private async checkLeagueMember(userId: string, leagueId?: string): Promise<boolean> {
    return this.checkSameLeague(userId, leagueId);
  }

  private async checkParticipant(userId: string, resource: string, resourceId?: string): Promise<boolean> {
  ...
  private getSuggestions(context: AccessContext, userRole: UserRole): string[] {
    const suggestions = [];
