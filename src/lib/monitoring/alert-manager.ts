...
class AlertManager {
  ...
  triggerAlert(metricName: string, value: number, context?: Record<string, any>): AlertInstance | null {
  ...
  private async evaluateRule(rule: AlertRule) {
    const metric = this.lastMetricValues.get(rule.metricName);
  ...
  private getAlertDuration(metricName: string, condition: string, threshold: number): number {
  ...
  private createAlert(rule: AlertRule, value: number, context?: Record<string, any>): AlertInstance {
  ...
  private async sendNotification(alert: AlertInstance, type: 'triggered' | 'resolved' | 'escalated') {
  ...
  private async sendToChannel(channel: NotificationChannel, message: string, alert: AlertInstance) {
    if (channel.testMode) {
  ...
  private async sendSlackNotification(channel: NotificationChannel, message: string, alert: AlertInstance) {
  ...
  private async sendEmailNotification(channel: NotificationChannel, message: string, alert: AlertInstance) {
    // Email notification implementation would go here
    console.log(`Email notification: ${message}`);
  }

  private async sendWebhookNotification(channel: NotificationChannel, message: string, alert: AlertInstance) {
  ...
  private async sendPagerDutyNotification(channel: NotificationChannel, message: string, alert: AlertInstance) {
    // PagerDuty notification implementation would go here
    console.log(`PagerDuty notification: ${message}`);
  }

  private async checkEscalation(alert: AlertInstance, rule: AlertRule) {
  ...
  private formatAlertMessage(alert: AlertInstance, type: 'triggered' | 'resolved' | 'escalated'): string {
    const action = type === 'resolved' ? 'RESOLVED' : type === 'escalated' ? 'ESCALATED' : 'TRIGGERED';
...
First Triggered: ${new Date(alert.firstTriggered).toISOString()}
${alert.resolvedAt ? `Resolved: ${new Date(alert.resolvedAt).toISOString()}` : ''}
${alert.context ? `Context: ${JSON.stringify(alert.context, null, 2)}` : ''}`;
  }

  private formatEscalationMessage(alert: AlertInstance, level: number): string {
    return `⬆️ ALERT ESCALATION - Level ${level}
