...
export class Logger {
  ...
  public debug(message: string, metadata?: Record<string, any>, requestId?: string, userId?: string): void {
    this.log(LogLevel.DEBUG, message, metadata, requestId, userId);
  }

  public info(message: string, metadata?: Record<string, any>, requestId?: string, userId?: string): void {
    this.log(LogLevel.INFO, message, metadata, requestId, userId);
  }

  public warn(message: string, metadata?: Record<string, any>, requestId?: string, userId?: string): void {
    this.log(LogLevel.WARN, message, metadata, requestId, userId);
  }

  public error(message: string, error?: Error, metadata?: Record<string, any>, requestId?: string, userId?: string): void {
  ...
  public fatal(message: string, error?: Error, metadata?: Record<string, any>, requestId?: string, userId?: string): void {
    const errorData = error ? {
      name: error.name,
      message: error.message,
      stack: error.stack,
  ...
  private consoleOutput(entry: LogEntry): void {
    ...
    switch (entry.level) {
      ...
      case LogLevel.ERROR:
      case LogLevel.FATAL:
        console.error(output, entry.error, entry.metadata);
        break;
    }
  }
...
export class PerformanceMonitor {
  ...
  private activeOperations: Map<string, { start: Date; metadata?: Record<string, any> }> = new Map();
  ...
  public startOperation(operationId: string, operationName: string, metadata?: Record<string, any>): void {
  ...
  public endOperation(operationId: string, success: boolean = true, metadata?: Record<string, any>): void {
  ...
  public getActiveOperations(): Array<{ id: string; operation: string; duration: number; metadata?: Record<string, any> }> {
    const now = new Date();
...
export class AlertManager {
  ...
  private alertRules: Array<{
    id: string;
    condition: (metrics: Metric[], logs: LogEntry[]) => boolean;
