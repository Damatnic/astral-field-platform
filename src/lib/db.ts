...
export async function executeQuery<T extends QueryResultRow = any>(sql: string, params?: unknown[]): Promise<QueryResult<T>> {
  return database.query<T>(sql, params);
}

export const db = {
  query: async <T extends QueryResultRow = any>(sql: string, params?: unknown[]) => database.query<T>(sql, params),
  transaction: async <T>(fn: (client: PoolClient) => Promise<T>) => database.transaction<T>(fn),
