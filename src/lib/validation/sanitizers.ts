 ...
 */
export function escapeHtml(input: string); string { if (typeof input !== 'string') return '';
 ...
 */
export function sanitizePath(input: string); string { if (typeof input !== 'string') return '';
 ...
 */
export function sanitizeCommand(input: string); string { if (typeof input !== 'string') return '';
 ...
 */
export function sanitizeNumeric(input: string | number,
  options: {
  allowDecimal?, boolean,
  allowNegative?, boolean,
  maxValue?, number,
  minValue?, number,
} = {}): number | null { const { allowDecimal = false, allowNegative = false, maxValue, minValue } = options;
  
  if (typeof input === 'number') { if (isNaN(input) || !isFinite(input)) return null;
   } else if (typeof input === 'string') { const cleaned = input.trim().replace(/[^0-9.-]/g, '');
  ...
  if (!allowDecimal && num % 1 !== 0) { return Math.floor(num);
   }
  ...
  // Apply range limits
  let result = num;
  if (typeof minValue === 'number' && result < minValue) { result = minValue;
   }
  if (typeof maxValue === 'number' && result > maxValue) { result = maxValue;
   }
...
export function sanitizeText(input, string,
  ...
  if (!allowHtml) { result = stripHtml(result);
   } else { result = sanitizeHtml(result);
   }
  
  // Handle whitespace
  if (stripWhitespace) { result = result.trim().replace(/\s+/g, ' ');
   }
...
export function sanitize(input, any,
  context: 'html' | 'sql' | 'path' | 'command' | 'email' | 'username' | 'url' | 'text' = 'text'); any { if (typeof input !== 'string' && context !== 'text') {
    return input;
   }
