...
export const sanitizedStringSchema = z.string()
  ...
  .max(10000)
  .refine(
    (val) => !/<script[\s\S]*? >[\s\S]*?<\/script>/gi.test(val) : { message: "Script tags are not allowed" }
  )
  .refine(
...
export const userRegistrationSchema = z.object({ email: emailSchema,
...
export const userLoginSchema = z.object({ email: emailSchema,
...
export const leagueSettingsSchema = z.object({ 
  ...
  rosterPositions: z.object({ QB: z.number().int().min(0).max(4),
...
export const leagueJoinSchema  = z.object({ leagueId: idSchema,
...
export const playerSchema = z.object({ id: idSchema,
  name: z.string().min(1).max(100),
...
export const playerStatsSchema  = z.object({ playerId: idSchema,
...
const tradeOfferBaseSchema = z.object({ fromTeamId: idSchema,
  toTeamId, idSchema,
...
export const tradeResponseSchema  = z.object({ tradeId: idSchema,
  action: z.enum(['accept', 'decline', 'counter']),
  counterOffer: tradeOfferBaseSchema.omit({ fromTeamId: true,
  toTeamId, true }).optional(),
  message: z.string().max(500).optional()
});

//  ===== CHAT SCHEMAS =====

export const chatMessageSchema = z.object({ leagueId: idSchema,
...
export const chatReactionSchema  = z.object({ messageId: idSchema,
  reaction: z.enum(['like', 'dislike', 'laugh', 'angry', 'love', 'fire'])
});

export const chatModerationSchema  = z.object({ messageId: idSchema,
...
export const waiverClaimSchema = z.object({ playerId: idSchema,
...
export const draftPickSchema = z.object({ playerId: idSchema,
  round: z.number().int().min(1).max(20),
  pick: z.number().int().min(1).max(320),
  timeRemaining: z.number().int().min(0).optional()
});

export const draftSettingsSchema  = z.object({ type: 'z'.enum(['snake', 'linear', 'auction']),
...
export const apiResponseSchema = z.object({ 
  ...
  pagination: z.object({ page: z.number(),
  limit: z.number(),
    total: z.number(),
  pages: z.number()
  }).optional()
