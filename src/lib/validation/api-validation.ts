...
export class ApiValidator {
  private config: validators.ValidationConfig;
  ...
  async validateRequest<TBody = any, TQuery = any, TParams = any>(
    ...
    const { bodySchema, querySchema, paramsSchema, params } = options;
    const results: any = {};
    const errors: validators.ValidationError[] = [];

    // Validate request body
    if (bodySchema && ['POST', 'PUT', 'PATCH'].includes(request.method)) {
      const bodyResult = await validators.validateRequestBody(request, bodySchema, this.config);
    ...
    if (querySchema) {
      const queryResult = validators.validateQueryParams(request, querySchema, this.config);
    ...
    if (paramsSchema && params) {
      const paramsResult = validators.validateRouteParams(params, paramsSchema);
   ...
   */
  createErrorResponse(errors: validators.ValidationError[], status = 400): NextResponse {
   ...
   */
  async validateAuth(request: NextRequest, type: 'login' | 'register' | 'password-change') {
    return validators.validateAuth(request, type);
  }

  /**
   * Validates league-related operations
   */
  async validateLeague(request: NextRequest, action: 'create' | 'update' | 'join') {
    return validators.validateLeague(request, action);
  }

  /**
   * Validates chat messages
   */
  async validateChatMessage(request: NextRequest) {
    return validators.validateChatMessage(request);
  }

  /**
   * Validates trade operations
   */
  async validateTrade(request: NextRequest, action: 'offer' | 'response') {
    return validators.validateTrade(request, action);
  }

  /**
   * Validates admin actions
   */
  async validateAdminAction(request: NextRequest) {
    return validators.validateAdminAction(request);
  }
  ...
  validateFileUpload(file: File) {
    return validators.validateFileUpload(file);
  }
}
...
export const quickValidate = {
   ...
   */
  email: (email: string) => schemas.emailSchema.safeParse(email),

  /**
   * Validates username format
   */
  username: (username: string) => schemas.usernameSchema.safeParse(username),

  /**
   * Validates password strength
   */
  password: (password: string) => schemas.passwordSchema.safeParse(password),

  /**
   * Validates UUID format
   */
  id: (id: string) => schemas.idSchema.safeParse(id),

  /**
   * Validates positive integer
   */
  positiveInt: (value: number) => schemas.positiveIntSchema.safeParse(value),

  /**
   * Sanitizes HTML content
   */
  html: (content: string) => sanitizers.sanitizeHtml(content),
...
export function ValidateParams<T>(schema: z.ZodSchema<T>) {
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (request: NextRequest, context: { params: any }) {
      const validator = new ApiValidator();
...
export const createValidator = {
   ...
   */
  auth: (type: 'login' | 'register' | 'password-change') => {
    return middleware.createValidationMiddleware({
      bodySchema: {
   ...
   */
  league: (action: 'create' | 'update' | 'join', querySchema?: z.ZodSchema<any>) => {
    return middleware.createValidationMiddleware({
      bodySchema: {
  ...
  chat: () => {
    return middleware.createValidationMiddleware({
      bodySchema: schemas.chatMessageSchema,
      sanitize: true,
  ...
  admin: () => {
    return middleware.createValidationMiddleware({
      bodySchema: schemas.adminActionSchema,
      sanitize: true,
      maxPayloadSize: 10 * 1024, // 10KB
      requireAuth: true,
      rateLimiting: {
  ...
  crud: (schema: z.ZodSchema<any>, options: {
    ...
    return middleware.createValidationMiddleware({
      bodySchema: schema,
      querySchema: schemas.queryParamsSchema,
      sanitize: true,
...
): Promise<validators.ValidationResult<any[]>> {
  const results: any[] = [];
  const errors: validators.ValidationError[] = [];
