...
describe('Schema Validation', () => {
  ...
  describe('Chat Message Schema', () => {
    ...
    it('should reject XSS attempts in content', async () => {
      const maliciousData = {
        leagueId: '123e4567-e89b-12d3-a456-426614174000',
        roomType: 'general',
...
describe('Security Validation', () => {
  ...
  describe('validateFileUpload', () => {
    ...
    it('should reject files that are too large', () => {
      const mockFile = new File(['fake data'], 'large.jpg', {
        type: 'image/jpeg'
      });
    ...
    it('should reject invalid file types', () => {
      const mockFile = new File(['fake data'], 'malware.exe', {
    ...
    it('should reject files with malicious names', () => {
      const mockFile = new File(['fake data'], '../../../malware.jpg', {
        type: 'image/jpeg'
      });
...
describe('Request Validation', () => {
  ...
  describe('validateQueryParams', () => {
    it('should validate correct query parameters', () => {
      const mockRequest = createMockRequest({
        method: 'GET',
        url: 'http://localhost/api/test?page=2&limit=10&search=hello'
      });

      const schema = {
        page: jest.fn().mockReturnValue({ safeParse: jest.fn().mockReturnValue({ success: true, data: { page: 2, limit: 10, search: 'hello' } }) })
      };
  ...
  describe('createValidationMiddleware', () => {
    ...
    it('should validate request security', async () => {
      ...
      const maliciousRequest = createMockRequest({
        url: 'http://localhost/api/test?param=<script>alert("xss")</script>'
      });
...
describe('Integration Tests', () => {
  ...
  it('should validate end-to-end API request flow', async () => {
    const mockRequest = createMockRequest({
      body: {
        leagueId: '123e4567-e89b-12d3-a456-426614174000',
        roomType: 'general',
...
describe('Error Handling', () => {
  ...
  it('should provide clear error messages', async () => {
    const result = await validateSchema(userLoginSchema, {
      email: 'invalid',
      password: ''
    });
