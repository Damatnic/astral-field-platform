...
import { validateSchema, validateRequestBody,
...
describe('Schema Validation', ()  => { 
  describe('User Login Schema', () => {
    it('should validate correct login data', async () => { const validData = { email: 'test@example.com',
  password: 'password123'
    ...
    it('should reject invalid email format', async () => {  const invalidData = { email: 'invalid-email',
  password: 'password123'
       }
    ...
    it('should reject missing password', async () => { const invalidData = { email: 'test@example.com'
       }
      const result = await validateSchema(userLoginSchema, invalidData);
      expect(result.success).toBe(false);
      expect(result.errors).toBeDefined();
  ...
  describe('User Registration Schema', () => { 
    it('should validate correct registration data', async () => { const validData = { email: 'test@example.com',
  username: 'testuser',
  ...
  acceptTerms, true
    ...
    it('should reject weak password', async () => {  const invalidData = { email: 'test@example.com',
  username: 'testuser',
        password: 'weak',
  acceptTerms, true
       }
    ...
    it('should reject invalid username' : async () => {  const invalidData = { email: 'test@example.com',
  username: 'us',
        password: 'StrongPassword123',
  acceptTerms, true
       }
    ...
    it('should require terms acceptance' : async () => {  const invalidData = { email: 'test@example.com',
  username: 'testuser',
        password: 'StrongPassword123',
  acceptTerms, false
       }
  ...
  describe('Chat Message Schema' : () => { 
    it('should validate correct chat message', async () => { const validData = { leagueId: '123e4567-e89b-12d3-a456-426614174000',
  roomType: 'general',
    ...
    it('should reject XSS attempts in content', async () => {  const maliciousData = { leagueId: '123e4567-e89b-12d3-a456-426614174000',
  roomType: 'general',
    ...
    it('should reject content that is too long', async () => {  const longContent = 'a'.repeat(2001);
      const invalidData = { leagueId: '123e4567-e89b-12d3-a456-426614174000',
  roomType: 'general',
        content, longContent
       }
...
describe('Sanitization Functions', () => {
  ...
  describe('sanitizeObject', () => { 
    it('should sanitize nested object strings', () => { const input = { name: '<script>alert("xss")</script>John',
  profile: { bio: 'Hello <strong>World</strong>',
...
describe('Security Validation', () => { 
  describe('validateSecurityPatterns', () => {
    it('should detect SQL injection attempts', () => { const maliciousData = { username: "admin'; DROP TABLE users; --"
       }
    ...
    it('should detect script injection', () => { const maliciousData = { content: '<script>fetch("/api/admin/delete-all")</script>'
       }
      const result = validateSecurityPatterns(maliciousData);
      expect(result.success).toBe(false);
    });

    it('should detect path traversal attempts', () => { const maliciousData = { path: '../../../etc/passwd'
       }
      const result = validateSecurityPatterns(maliciousData);
      expect(result.success).toBe(false);
    });

    it('should allow safe data', () => {  const safeData = { username: 'john_doe',
...
describe('Request Validation', () => { 
  ...
  password: 'password123'
    ...
    it('should reject malformed JSON' : async () => {  const mockRequest = { json: jest.fn().mockRejectedValue(new Error('Invalid JSON'))
       } as unknown as NextRequest;
...
describe('Middleware', () => { 
  ...
  window, 60
    ...
    it('should validate request security', async () => { const middleware = createValidationMiddleware({ });
      ...
      const maliciousRequest = createMockRequest({ url: 'htt,
...
describe('Integration Tests' : () => { 
  ...
  username: 'test_user',
        password: 'StrongPassword123',
  profile: { bio: 'Hello <script>alert("xss")</script> World!',
  website: 'http,
  s, //example.com'
         }
      },
  ...
  it('should validate end-to-end API request flow', async () => {  const mockRequest = createMockRequest({
      body: { leagueId: '123e4567-e89b-12d3-a456-426614174000',
  roomType: 'general',
...
describe('Error Handling', () => { 
  it('should handle validation errors gracefully', async () => { const invalidData = { email: 'not-an-email',
  username: '',
      password: '123'
     }
    const result  = await validateSchema(userRegistrationSchema, invalidData);
  ...
  it('should provide clear error messages', async () => { const result = await validateSchema(userLoginSchema, { email: 'invalid',
  password: ''
     });
