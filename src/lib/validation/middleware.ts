...
export function createValidationMiddleware(config: ValidationMiddlewareConfig = {}) { return function validationMiddleware<T extends ApiHandler | ApiHandlerWithParams>(handler: T); T {
  ...
  message: `Request payload too large.Maximum size; ${maxPayloadSize } bytes`,
        ...
        if (querySchema) { const queryResult = validateQueryParams(request, querySchema, { sanitize  });
          if (hasValidationErrors(queryResult)) {
            validationErrors.push(...queryResult.errors);} else if (queryResult.success) {
            validatedRequest.validatedQuery = queryResult.data;
          }
        }

        // ===== ROUTE PARAMETERS VALIDATION =====
        if (paramsSchema && args.length > 0 && typeof args[0] === 'object') { const paramsResult = validateRouteParams(args[0].params || { }, paramsSchema);
          if (hasValidationErrors(paramsResult)) {
            validationErrors.push(...paramsResult.errors);} else if (paramsResult.success) {
            validatedRequest.validatedParams = paramsResult.data;
          }
        }

        // ===== CUSTOM VALIDATORS =====
        for (const validator of customValidators) { const customResult = await validator(request);
...
async function checkRateLimit(
  ...
  s, number, window: number }
    ...
    for (const [entryKey, entry] of rateLimitStore.entries()) { if (entry.resetTime < now) {
