...
import { 
  ArrowRightLeft, Plus, Clock, CheckCircle, XCircle, 
  TrendingUp, TrendingDown, Star, AlertCircle, 
  Calendar, Filter, Search, MessageCircle, Users, 
  ...
  const [teams, setTeams] = useState<Team[]>([
    {
      ...
      record: "8-4",
      players: [
        { id: "p1", name: "Josh Allen", position: "QB", team: "BUF", points: 312.4, avgPoints: 26.0, projectedPoints: 24.8, injury_status: null, bye_week: 12, trend: 'up' },
        { id: "p2", name: "Christian McCaffrey", position: "RB", team: "SF", points: 285.6, avgPoints: 23.8, projectedPoints: 22.1, injury_status: null, bye_week: 9, trend: 'stable' },
        { id: "p3", name: "Tyreek Hill", position: "WR", team: "MIA", points: 245.2, avgPoints: 20.4, projectedPoints: 18.9, injury_status: null, bye_week: 6, trend: 'down' }
      ]
    },
    {
      id: "2", 
      ...
      record: "7-5",
      players: [
        { id: "p4", name: "Patrick Mahomes", position: "QB", team: "KC", points: 298.7, avgPoints: 24.9, projectedPoints: 26.2, injury_status: null, bye_week: 10, trend: 'up' },
        { id: "p5", name: "Derrick Henry", position: "RB", team: "TEN", points: 198.3, avgPoints: 16.5, projectedPoints: 15.8, injury_status: "Questionable", bye_week: 7, trend: 'down' },
        { id: "p6", name: "Davante Adams", position: "WR", team: "LV", points: 201.4, avgPoints: 16.8, projectedPoints: 17.3, injury_status: null, bye_week: 13, trend: 'stable' }
      ]
    }
  ]);
        ...
        <div className="animate-pulse">
          ...
          <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="space-y-4">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="h-20 bg-white dark:bg-gray-800 rounded-lg"></div>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
          ...
          <div className="mt-4 lg:mt-0 flex items-center space-x-4">
            <div className="bg-white dark:bg-gray-800 px-3 py-2 rounded-lg border border-red-200 dark:border-red-800">
        ...
        <div className="border-b border-gray-200 dark:border-gray-700 mb-8">
          <nav className="-mb-px flex space-x-8">
            {[
              { id: 'propose', label: 'Propose Trade', icon: Plus },
              { id: 'active', label: 'Active Trades', icon: Clock },
                    ...
                    {currentTeam?.players.map((player) => (
                      <div
                        ...
                        className={`p-4 rounded-lg border cursor-pointer transition-colors ${
                          offeredPlayers.some(p => p.id === player.id)
                            ? 'border-green-500 bg-green-50 dark:bg-green-900/20'
                            : 'border-gray-200 dark:border-gray-700 hover:border-gray-300'
                        }`}
                        onClick={() => {
                          if (offeredPlayers.some(p => p.id === player.id)) {
                  ...
                  <div className="space-y-4">
                    {filteredPlayers.map((player) => (
                      <div
                        key={player.id}
                        className={`p-4 rounded-lg border cursor-pointer transition-colors ${
                          requestedPlayers.some(p => p.id === player.id)
                            ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20'
                            : 'border-gray-200 dark:border-gray-700 hover:border-gray-300'
                        }`}
                        onClick={() => {
                          if (requestedPlayers.some(p => p.id === player.id)) {
                ...
                {(() => {
                  ...
                  return (
                    <div className={`p-4 rounded-lg border-l-4 mb-6 ${
                      analysis.color === 'green' ? 'bg-green-50 border-green-500 dark:bg-green-900/20' :
                      analysis.color === 'blue' ? 'bg-blue-50 border-blue-500 dark:bg-blue-900/20' :
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      <span className={`px-3 py-1 text-xs rounded-full font-medium ${
                        trade.status === 'pending' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300' :
                        trade.status === 'countered' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300' :
                        ...
                        <tr key={trade.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 py-1 text-xs rounded-full ${
                              trade.status === 'accepted' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300' :
                              trade.status === 'rejected' ? 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300' :
                              'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300'
                            }`}>
                              {trade.status}
                            </span>
