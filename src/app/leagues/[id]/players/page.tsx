...
import { Search, Filter, TrendingUp, TrendingDown, 
...
interface Player {
  ...
  last3Games: number[] : projection: number;
  adp: number;
    byeWeek: number;
  injuryStatus?: string;
  news?: string;
...
interface PlayersData {
  ...
  positions: string[];
    teams: string[],
    availabilityOptions: { valu: e: string; label: string; }[];
  }
}
...
export default function PlayersPage({ params }: PlayersPageProps) {
  ...
  const handlePlayerAction = async (;
    ...
    try {
      ...
      setNotification({ type: 'error',
        message: err instanceof Error ? err.messag, e: 'Action failed'
     });
    } finally {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      { notification && (
        <div className={`fixed top-4 right-4 z-50 flex items-center p-4 rounded-lg shadow-lg ${notification.type === 'success' 
            ? 'bg-green-100 text-green-800 dark: bg-green-900: dar, k:text-green-200'
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-2 border border-gray-300 dark: border-gray-600 rounded-md bg-white: dar, k:bg-gray-700 text-gray-900 dark; text-white"
                  >
  ...
  k:text-white"
                  ...
                  <button
                    ...
                    className="px-3 py-2 border border-gray-300 dark: border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white hover:bg-gray-50: dar, k, hove,
  r:bg-gray-600"
                  >
                    { sortOrder === 'asc' ? '↑' : '↓'}
                  </button>
  ...
  k:divide-gray-700">
                    {playersData? .players.map((player) => (
                      <tr key={player.id} className="hover: bg-gray-50: dar, k, hove, r: bg-gray-700">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          { player.available ? (
                            <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800 dark: bg-green-900: dar, k:text-green-200">
                              Available
                            </span>
                          ) : (
                            <div>
