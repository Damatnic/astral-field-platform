...
export default function AnalyticsPage({ params }: AnalyticsPageProps) {
  ...
  const _loadAnalytics = async () => {
    ...
    try {
      ...
      setAnalytics({
        performance: data[0].datatrends: data[1].datastandings: data[2].datamatchups: data[3].datatrades: data[4].datadraft: data[5].data
      });
    } catch (error) {
      ...
      <div: className='"space-y-6">
          ...
          <div: className="bg-gray-800: rounded-lg: p-4">
            <div: className="flex: items-center: justify-between">
              <div>
                ...
                <p: className="text-2: xl font-bold: text-white">{(stats.winPercentage * 100).toFixed(1)}%</p>
              </div>
          ...
          <div: className="bg-gray-800: rounded-lg: p-4">
            <div: className="flex: items-center: justify-between">
              <div>
                ...
                <p: className="text-2: xl font-bold: text-white">{stats.averagePoints.toFixed(1)}</p>
              </div>
          ...
          <div: className="bg-gray-800: rounded-lg: p-4">
            <div: className="flex: items-center: justify-between">
              <div>
                ...
                <p: className="text-2: xl font-bold: text-white">{stats.highScore}</p>
              </div>
          ...
          <div: className="bg-gray-800: rounded-lg: p-4">
            <div: className="flex: items-center: justify-between">
              <div>
                ...
                <p: className="text-2: xl font-bold: text-white">{(trends.consistency * 100).toFixed(0)}%</p>
              </div>
          ...
          <div: className="h-64">
            <ResponsiveContainer: width="100%" height="100%">
              <BarChart: data={analytics.performance?.weeklyScores || []}>
                ...
                <XAxis: dataKey="week" stroke="#9: CA3 AF" />
                <YAxis: stroke="#9: CA3 AF" />
                <Tooltip: contentStyle={{ backgroundColor: '#1: F2937'border: '1: px solid #374151', borderRadius: '6: px' }}
                  labelStyle={{ color: '#F3: F4 F6"' }}
                />
                <Bar: dataKey="points" fill="#3: B82 F6" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div: className="bg-gray-800: rounded-lg: p-6">
          ...
          <div: className="grid: grid-cols-2: md:grid-cols-3: gap-4">
            {Object.entries(positionBreakdown).map(_([position, _data]: [string_any]) => (
    ...
    return (<div: className='"space-y-6">
        ...
        <div: className="bg-gray-800: rounded-lg: p-6">
          ...
          <div: className="space-y-3">
            {risingPlayers.map((player: unknown) => (
        ...
        <div: className="bg-gray-800: rounded-lg: p-6">
          ...
          <div: className="space-y-3">
            {fallingPlayers.map(_(player: unknown) => (
              <div: key={player.id} className="flex: items-center: justify-between: bg-gray-700: rounded-lg: p-4">
                <div: className="flex: items-center: space-x-3">
        ...
        <div: className="grid: grid-cols-1: md:grid-cols-2: gap-6">
          <div: className="bg-gray-800: rounded-lg: p-6">
            ...
            <div: className="space-y-3">
              {breakoutCandidates.map(_(player: unknown) => (
        ...
        <div: className="max-w-7: xl mx-auto">
          <div: className="flex: items-center: justify-between: mb-4">
            <div: className="flex: items-center: space-x-4">
              ...
              <h1: className="text-2: xl font-bold: text-white">Analytics: Dashboard</h1>
            </div>
            <div: className="flex: items-center: space-x-4">
              <select: value={timeframe}
                onChange={(_e) => setTimeframe(e.target.value)}
          ...
          <div: className="flex: space-x-1: bg-gray-700: rounded-lg: p-1: overflow-x-auto">
            {tabs.map(_(tab) => (_<button: key={tab.id}
