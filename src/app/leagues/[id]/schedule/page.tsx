...
export default function SchedulePage({ params }: SchedulePageProps) {
  ...
  const [teams] = useState<{ [id: string]: Team }>({
    "1": { id: "1", team_name: "Gridiron Gladiators", team_abbreviation: "GG", owner_name: "Nicholas D'Amato", record: "9-3", points_for: 1547.2, points_against: 1398.5, current_rank: 1 },
    "2": { id: "2", team_name: "Touchdown Titans", team_abbreviation: "TT", owner_name: "Sarah Johnson", record: "8-4", points_for: 1523.8, points_against: 1425.1, current_rank: 2 },
    "3": { id: "3", team_name: "Field Goal Phantoms", team_abbreviation: "FGP", owner_name: "Mike Chen", record: "8-4", points_for: 1489.3, points_against: 1456.2, current_rank: 3 },
    "4": { id: "4", team_name: "End Zone Eagles", team_abbreviation: "EZE", owner_name: "Jessica Williams", record: "7-5", points_for: 1456.7, points_against: 1467.3, current_rank: 4 },
    "5": { id: "5", team_name: "Red Zone Raiders", team_abbreviation: "RZR", owner_name: "David Brown", record: "7-5", points_for: 1423.9, points_against: 1478.4, current_rank: 5 },
    "6": { id: "6", team_name: "Pocket Passers", team_abbreviation: "PP", owner_name: "Amanda Davis", record: "6-6", points_for: 1398.2, points_against: 1456.8, current_rank: 6 },
    "7": { id: "7", team_name: "Blitz Brothers", team_abbreviation: "BB", owner_name: "Chris Wilson", record: "5-7", points_for: 1367.5, points_against: 1487.9, current_rank: 7 },
    "8": { id: "8", team_name: "Hail Mary Heroes", team_abbreviation: "HMH", owner_name: "Lisa Garcia", record: "4-8", points_for: 1334.1, points_against: 1523.7, current_rank: 8 },
    "9": { id: "9", team_name: "Fantasy Footballers", team_abbreviation: "FF", owner_name: "Ryan Martinez", record: "3-9", points_for: 1289.6, points_against: 1567.2, current_rank: 9 },
    "10": { id: "10", team_name: "Championship Chasers", team_abbreviation: "CC", owner_name: "Kaity Lorbecki", record: "2-10", points_for: 1245.3, points_against: 1634.8, current_rank: 10 }
  });
  ...
  const generateSchedule = () => {
      ...
      for (let i = 0; i < numTeams; i += 2) {
        ...
        if (homeTeamId && awayTeamId) {
          ...
          matchups.push({
            ...
            projected_away_score: Math.random() * 30 + 95,
            is_complete: isComplete,
            is_playoffs: false,
            is_championship: false,
            game_date: `2024-${week < 10 ? '09' : '10'}-${String(week * 3).padStart(2, '0')}`,
            winner_id: isComplete ? (homeScore! > awayScore! ? homeTeamId : awayTeamId) : undefined,
    ...
    const playoffWeeks = [
      { week: 15, name: "Wild Card", teams: 6 },
      { week: 16, name: "Semifinals", teams: 4 },
      { week: 17, name: "Championship", teams: 2 }
    ];

    playoffWeeks.forEach(({ week, name, teams: playoffTeams }) => {
      const matchups: Matchup[] = [];
  ...
  const getCurrentWeekData = () => {
    return schedule.find(w => w.week === selectedWeek);
  };

  const getUserMatchup = (weekData: WeekData) => {
    return weekData.matchups.find(m => 
      m.home_team_id === currentUserId || m.away_team_id === currentUserId
    );
  };

  const getMatchupPreview = (matchup: Matchup) => {
    const homeAdvantage = matchup.home_team.points_for > matchup.away_team.points_for ? 'home' : 'away';
    const rankDiff = Math.abs(matchup.home_team.current_rank - matchup.away_team.current_rank);
    
    let preview = '';
    if (rankDiff <= 2) {
      preview = 'Tight matchup expected';
    } else if (homeAdvantage === 'home') {
      preview = `${matchup.home_team.team_abbreviation} favored`;
    } else {
      preview = `${matchup.away_team.team_abbreviation} favored`;
    }
  ...
  const getScoreColor = (score: number, opponentScore: number) => {
    if (score > opponentScore) return 'text-green-600 dark:text-green-400';
        ...
        <div className="animate-pulse">
          ...
          <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {[...Array(8)].map((_, i) => (
                <div key={i} className="h-32 bg-white dark:bg-gray-800 rounded-lg"></div>
              ))}
            </div>
          </div>
        </div>
                    ...
                    <div className="text-center">
                      ...
                      {userMatchup.is_complete ? (
                        <div className={`mt-1 text-xl font-bold ${
                          userMatchup.home_team_id === currentUserId 
                            ? getScoreColor(userMatchup.home_score!, userMatchup.away_score!)
                            : getScoreColor(userMatchup.away_score!, userMatchup.home_score!)
                        }`}>
                ...
                {currentWeekData.matchups.map((matchup) => (
                  <div
                    ...
                    className={`bg-white dark:bg-gray-800 rounded-lg p-6 shadow ${
                      matchup.home_team_id === currentUserId || matchup.away_team_id === currentUserId
