...
export default function DraftPage({ params }: DraftPageProps) {
  ...
  const createDraftRoom = async () => {
    ...
    setCreating(true);
    try {
      // Create demo participants for testing
      const demoParticipants = [
        {
          id: `participant_${user.id}`,
          userId: user.id,
          teamId: `team_${user.id}`,
          teamName: user.username || 'Your Team',
          draftPosition: 1,
          isActive: true,
          isOnline: false,
      ...
      const response = await fetch('/api/draft-socket', {
        ...
        body: JSON.stringify({
          ...
          totalRounds: 16,
          rosterSize: 16,
          draftOrder: demoParticipants.map(p => p.teamId),
          settings: {
            allowTrades: true,
        ...
        <div className="max-w-4xl mx-auto p-8">
          <div className="text-center">
            <div className="bg-gray-800 rounded-lg p-8 mb-8">
              ...
              <h2 className="text-2xl font-bold text-white mb-4">Ready to Draft?</h2>
  ...
  // Draft room exists, show the main draft interface
  return (
