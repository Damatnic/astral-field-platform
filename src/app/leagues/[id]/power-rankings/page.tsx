...
export default function PowerRankingsPage({ params }: LeaguePageProps) {
  ...
  const handleLikeComment = (teamId: string, commentId: string) => {
    setRankings(rankings.map(team => {
      if (team.id === teamId) {
        return {
          ...team,
                ...
                {viewMode === 'detailed' && team.analysis && (
                  <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 mb-4">
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                      <span className="font-semibold text-gray-900 dark:text-white">Analysis:</span> {team.analysis}
                    </p>
                  </div>
                )}
                          ...
                          <div key={comment.id} className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3">
                            <div className="flex items-start justify-between">
                              ...
                              <button
                                ...
                                className={`inline-flex items-center gap-1 text-sm ml-4 ${
                                  comment.hasLiked
