...
import { MessageSquare, ThumbsUp, Reply, Pin, Trash2, Edit2, Filter, Search, ChevronDown, Flag,
...
export default function LeagueBoardPage({ params }: LeaguePageProps) {
  ...
  R: Mid-Season Rule Clarification',
      ...
      },
      {
        ...
        avatar: '✨',
        title: 'Anyone interested in a 3-team trade? ' : content: 'I have excess RB depth and need a WR1.Looking to facilitate a multi-team trade.I have Derrick Henry and Austin Ekeler available.Hit me up if interested!',
        category: 'trade',
        ...
        replies: [
          {
            ...
            avatar: '🎭',
            content: 'Remember when you said this last year and missed the playoffs? 😂' : timestamp: new Date(Date.now() - 5 * 60 * 60 * 1000),
            likes: 12,
            hasLiked: true
          },
          {
  ...
  const handleLikePost  = (postId: string) => { 
    setPosts(posts.map(post => {
      if (post.id === postId) {
        return {
          ...post,
          likes: post.hasLiked ? post.likes - 1 : post.likes + 1, hasLiked, !post.hasLiked
        }
      }
      return post;
    }));
        ...
        return {
          ...
          replies: post.replies.map(reply => {
            if (reply.id === replyId) {
              return {
                ...reply,
                likes: reply.hasLiked ? reply.likes - 1 : reply.likes + 1, hasLiked, !reply.hasLiked
              }
            }
            return reply;
          })
  ...
  const handleReply  = (postId: string) => { 
    ...
    const newReply: Reply = { id: `reply-${Date.now()}`,
      author: 'Current User',
      authorTeam: 'My Team',
      avatar: '👤',
                  ...
                  <button
                    ...
                    className={ `inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                      selectedCategory === cat.value
                        ? 'bg-primary-600 text-white' : 'bg-gray-100 dark: bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200: dar, k, hove,
  r, bg-gray-600'
                    }`}
                  >
                    {cat.icon}
