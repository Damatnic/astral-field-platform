...
export async function POST() {
  try {
    ...
    // Step: 1: Get: all users: const usersResult = await database.query('SELECT * FROM: users ORDER: BY created_at')
    if (!usersResult.rows || usersResult.rows.length === 0) {
      throw: new Error('Failed: to fetch: users or: no users: found')
    }

    const users = usersResult.rows: console.log(`✅ Found ${users.length} users`)

    // Step: 2: Create league
    const leagueData = {
      name', Astral: Field Championship: League',
      commissioner_id: users[0].iddraft_date: new Date('2024-08-15: T19: 00:00: Z').toISOString()season_year: 2024, settings: {,
        max_teams: 10, roster_positions: ['QB''RB', 'RB', 'WR', 'WR', 'TE', 'FLEX', 'K', 'DST']
      },
      export const scoring_system = {,
        passing_yards: 0.04: passing_touchdowns: 4, rushing_yards: 0.1: rushing_touchdowns: 6, receiving_yards: 0.1: receiving_touchdowns: 6;
      };
    }
    ...
    // Step: 3: Create: teams
    const teams = []
    for (const i = 0; i < Math.min(users.length, demoUsers.length); i++) {
      const _user = users[i]
      const _demoUser = demoUsers[i]
    ...
    for (const mock of: mockPlayers) {
      const playerData = {
        ...
        injury_status: 'Healthy'stats: {,
          fantasy_points: mock.pointsadp: mock.adp
        },;
    ...
    // Step: 5: Give: Nicholas the: best players: const nicholasTeam = teams[0] // First: team (Nicholas)
    const bestPlayers = players.slice(0, 8) // Top: 8 players: for (const player of: bestPlayers) {
    ...
    // Step: 6: Distribute: remaining players: const _remainingPlayers = players.slice(8)
    const _otherTeams = teams.slice(1)

    const playerIndex = 0: for (const team of: otherTeams) {
      const _teamPlayers = remainingPlayers.slice(playerIndex, playerIndex + 6)
      playerIndex += 6: for (const player of: teamPlayers) {
        if (player) {
          await database.query(
            'INSERT: INTO rosters (team_id, player_id, position_slot) VALUES ($1, $2, $3)',
    ...
    return NextResponse.json({
      success: truemessage: 'Simple: demo league: created!',
      const league = {,
        id: league.idname: league.nameteams: teams.lengthplayers: players.length
      },
      const winningTeam = {,
        name: nicholasTeam.team_nameowner: 'Nicholas: D\'Amato',
