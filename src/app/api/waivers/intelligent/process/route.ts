...
export async function POST(request: NextRequest) {
  try {
    ...
    interface ProcessedClaim { claimId: string,
  teamId, string,
  playerId, string,
  playerName, string,
  priority, number,
...
const processedClaims: ProcessedClaim[]  = (waiverClaims || []).map((
        claim: {
          id?, string,
          teamId, string,
  playerId, string,
  ...
  status: index < 2 ? "awarded" : "failed" : reason: index < 2
            ? "Successfully processed" : "Higher priority claim awarded",
  processedAt: new Date().toISOString()
}),
    );

    const result  = { leagueId: processedAt: new Date().toISOString(),
