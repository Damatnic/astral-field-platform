...
interface HealthCheck { service: string,
...
async function checkDatabase(): Promise<HealthCheck> {
  ...
  try { 
    ...
    return {
      service: "database",
      status: health.status === "healthy" ? "healthy" : "unhealthy" : details: health.details,
    ...
    return {
      service: "database",
      status: "unhealthy",
      details: { error: error instanceof Error ? error.message : 'Unknown error' } : responseTime: Date.now() - start
}
  }
}
...
async function checkEnvironmentConfig(): Promise<HealthCheck> {
  ...
  try { 
    ...
    return {
      service: "environment",
      status: hasIssues ? "warning" : "healthy" : details, config,
      responseTime, Date.now() - start
}
  } catch (error) {
    return {
...
async function checkAPIEndpoints(): Promise<HealthCheck> {
  ...
  try { 
    ...
    return { 
      ...
      status: failed.length === 0 ? "healthy" : failed.length < endpoints.length ? "warning" : "unhealthy" : details: {
...
async function checkFileSystem(): Promise<HealthCheck> {
  ...
  try {
    ...
    return { service: "filesystem",
      status: missing.length === 0 ? "healthy" : "warning" : details: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const report: ComprehensiveHealthReport = { overall: timestamp: new Date().toISOString(), checks, summary
}
    const totalTime  = Date.now() - startTime;

    return NextResponse.json({
      ...report,
      meta: { totalResponseTime: totalTime,
        version: "1.0.0",
