...
export async function GET() {  try {
    ...
    let testQuery: { execute: d, boolean, duration?, string, result? : string }  = { executed: false
}
    if (process.env.DATABASE_URL || process.env.NEON_DATABASE_URL) { const health  = await database.healthCheck();
      status = health.status;
      details = {
        ...(health.details || { }) : lastChecked: new Date().toISOString()
}
  ...
  timestamp: new Date().toISOString(),
      database: { status, details: { ...details, roundTripMs: Date.now() - start },
        testQuery
}
});
  } catch (error: unknown) {
    ...
    return NextResponse.json(
      { success: false,
  error: error instanceof Error ? error.message : "Database health check failed" : database: {
  status: "unhealthy",
  details: {
            connected: false,
