...
const mfaSetupLimit = createRateLimit(
  MFA_RATE_LIMITS.setup.maxAttempts,
  MFA_RATE_LIMITS.setup.windowMs
);

// Mock user MFA storage - replace with database
const userMFAStore: { [userId: string]: UserMFASettings } = {};
...
// Helper function to get user MFA settings
function getUserMFA(userId: string): UserMFASettings {
  return userMFAStore[userId] || { isEnabled: false };
}

// Helper function to save user MFA settings
function saveUserMFA(userId: string, mfaSettings: UserMFASettings): void {
  ...
  try {
    ...
    switch (action) {
      case 'status': {
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            needsNewBackupCodes: needsNewBackupCodes(userMFA),
            isLocked: userMFA.lockedUntil ? userMFA.lockedUntil > new Date() : false,
      ...
      case 'setup': {
        ...
        const rateLimitResponse = mfaSetupLimit(request);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResponse = mfaVerificationLimit(request);
    if (rateLimitResponse) {
      return rateLimitResponse;
    }
    ...
    switch (action) {
      case 'verify-setup': {
        ...
        if (!isValid) {
          ...
          await logAuditEvent(
            ...
            { 
              setupStage: 'verification_failed',
              remainingAttempts: Math.max(0, 5 - (updatedMFA.failedAttempts || 0)),
        ...
        await logAuditEvent(
          ...
          { 
            setupStage: 'verification_success',
            backupCodesGenerated: enabledMFA.backupCodes?.length || 0
          },
          {
            userId: user.id,
            userEmail: user.email,
      ...
      case 'verify': {
        ...
        // Success - reset failed attempts and remove backup code if used
        let updatedMFA = resetFailedAttempts(userMFA);
      ...
      case 'disable': {
        ...
        // Disable MFA
        const disabledMFA = disableMFA(userMFA);
        ...
        await logAuditEvent(
          ...
          { 
            ...
            verificationMethod: token ? 'mfa_token' : 'password'
          },
          {
            userId: user.id,
            userEmail: user.email,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: {
        requiresMFA: isMFARequired(userMFA),
        isLocked: userMFA.lockedUntil ? userMFA.lockedUntil > new Date() : false,
        lockoutTime: userMFA.lockedUntil,
      }
    });
  } catch (error) {
