  ...
  try {
    ...
    switch (type) {
      case 'multi_team':
        ...
        const suggestions = await multiTeamTradeEngine.generateMultiTeamTrades(
          leagueId,
      ...
      case 'suggestions':
        ...
        const tradeSuggestions = await tradeAnalyzer.generateTradeSuggestions(
          team1Id,
          team2Id,
    ...
    switch (type) {
      ...
      case 'process':
        ...
        if (!leagueId || !proposingTeamId || !receivingTeamId || 
            !Array.isArray(proposedPlayers) || !Array.isArray(requestedPlayers)) {
          return NextResponse.json(
