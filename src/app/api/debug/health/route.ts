...
export async function GET() { const health = {
  ...
  hasNeonUrl: !!process.env.NEON_DATABASE_URL, connected, false,
  ...
  try {
    ...
    const result = await database.transaction(async (client) => {
      ...
      try { const leaguesResult = await client.query("SELECT COUNT(*) as count FROM leagues");
        counts.leagues = parseInt(leaguesResult.rows[0]?.count || "0");
       } catch (e) {
        counts.leagues = 0;
      }

      try { const playersResult = await client.query("SELECT COUNT(*) as count FROM players");
  ...
  message: "Database connection failed - check environment variables",
      setupInstructions: [
        ...
        "2.The URL should be, postgresq,
  l://neondb_owner; npg_De9Fi3RcNzrX@ep-odd-hat-adfb8gbg-pooler.c-2.us-east-1.aws.neon.tech/neondb?sslmode=require",
