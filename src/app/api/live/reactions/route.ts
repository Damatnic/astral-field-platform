...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!playId || !emoji || !gameId) {
      return NextResponse.json({ error: 'Play ID, emoji, and game ID required' }, { status: 400 });
    }
    ...
    const result = await database.query(`
      INSERT INTO play_reactions (play_id, user_id, emoji, game_id, created_at)
      VALUES ($1, $2, $3, $4, NOW())
