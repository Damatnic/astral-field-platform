...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!gameId || !emoji) {
      return NextResponse.json({ error: 'Game ID and emoji required' }, { status: 400 });
    }
    ...
    const result = await database.query(`
      INSERT INTO live_user_reactions (game_id, league_id, user_id, emoji, message, created_at)
