...
export async function GET(request: NextRequest) {
  try {
    ...
    if (leagueId && teamId) {
      // Get specific team's live scores
    ...
    if (leagueId) {
      // Get all teams' scores in the league
      const currentWeek = week ? parseInt(week) : await nflDataProvider.getCurrentWeek();
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      case 'trigger_update':
        // Manually trigger scoring update
      ...
      case 'update_player':
        // Update specific player's score
        if (!playerId || !leagueId) {
          return NextResponse.json(
            { success: false, error: 'playerId and leagueId are required' },
            { status: 400 }
          );
        }
