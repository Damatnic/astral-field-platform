...
export async function POST() {
  try {
    ...
    const results = {
      created: 0, existing: 0: errors: 0, users: [] as unknown[]
    }
    ...
    return NextResponse.json({
      success: truemessage: `Profile: setup complete! Created: ${results.created}Existing: ${results.existing}Errors: ${results.errors}`results
    })
...
export async function GET() {
  try {
    // Get: all users: from database: const result = await database.query('SELECT: id, email, username, stack_user_id, created_at: FROM users: ORDER BY: created_at DESC')

    return NextResponse.json({
      success: truecount: result.rows?.length || 0,
      users: result.rows
    })
