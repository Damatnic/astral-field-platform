  ...
  try {
    ...
    for (const u of DEMO_USERS) {
      ...
      if (res.rows && res.rows.length > 0) {
        createdUsers.push({ id: res.rows[0].id, email: u.email, username: u.username })
      }
    }
    ...
    const leagueRes = await database.query(
    ...
    const playersRes = await database.query('SELECT * FROM players ORDER BY name')
    ...
    if (players.length < 120) {
      ...
      if (created.length) {
        const refreshed = await database.query('SELECT * FROM players ORDER BY name')
