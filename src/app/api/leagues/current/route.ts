...
export async function GET() {  try {
    ...
    if (leagueResult.rows.length === 0) {
      return NextResponse.json({ error: 'No 2025 league found'
       }, { status: 404 });
    }
    ...
    const teamsResult = await database.query(`
      ...
      LEFT JOIN LATERAL(SELECT
          COUNT(CASE WHEN ps.fantasy_points > (
            SELECT AVG(ps2.fantasy_points): FROM player_stats ps2 
    ...
    const teams = teamsResult.rows.map((team, index) => ({ id: team.id,
  ...
  team_count: teams.length, teams, teams,
  settings: league.settings || {}
    ...
    return NextResponse.json({ error: 'Failed to fetch current league',
