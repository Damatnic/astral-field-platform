...
export const GET = userInputValidationMiddleware(async (request: NextRequest) => {
  try {
    ...
    let messages;
    if (search) {
      messages = await chatService.searchMessages(leagueId, roomType, search, limit);
    } else {
      messages = await chatService.getMessageHistory(leagueId, roomType, limit, before || undefined);
    }
...
export const POST = userInputValidationMiddleware(async (request: NextRequest) => {
  try {
    const token = request.headers.get('authorization')?.replace('Bearer ', '');
    if (!token) {
      return NextResponse.json({ error: 'Authentication required' }, { status: 401 });
    ...
    if (hasValidationErrors(messageValidation)) {
      return NextResponse.json(
    ...
    const message = await chatService.sendMessage(
      validatedData.leagueId as string,
...
export const DELETE = userInputValidationMiddleware(async (request: NextRequest) => {
  try {
    const token = request.headers.get('authorization')?.replace('Bearer ', '');
    if (!token) {
      return NextResponse.json({ error: 'Authentication required' }, { status: 401 });
