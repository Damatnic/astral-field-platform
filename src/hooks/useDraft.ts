...
export function useDraft(leagueId: string, userTeamId: string) {
  ...
  const {
    ...
    sendMessage,
    lastMessage
  } = useWebSocket(`/api/draft/${leagueId}/ws`);
  ...
  useEffect(() => {
    if (draftState.onTheClock && draftState.timeRemaining && draftState.timeRemaining > 0) {
  ...
  const initializeDraft = async () => {
    ...
    try {
      ...
      if (!draftResponse.ok) {
        throw new Error('Failed to load draft state');
      }
  ...
  const handleWebSocketMessage = (message: any) => {
    switch (message.type) {
      ...
      case 'DRAFT_STARTED':
        setDraftState(prev => ({ ...prev, isActive: true, startTime: new Date(message.startTime) }));
        break;
      ...
      case 'DRAFT_COMPLETED':
        setDraftState(prev => ({ ...prev, isActive: false, endTime: new Date(message.endTime) }));
        break;
  ...
  const updateTeamConnection = (teamId: string, isConnected: boolean) => {
  ...
  const makePick = useCallback(async (player: DraftPlayer) => {
    if (!draftState.onTheClock || !draftState.isActive) {
  ...
  const autoPickPlayer = useCallback(async () => {
    if (!draftState.onTheClock || !draftSettings.autoPickEnabled) return;
  ...
  const proposeTrade = useCallback(async (fromPick: number, toPick: number) => {
    if (!draftSettings.tradingEnabled) {
      throw new Error('Trading is not enabled for this draft');
    }

    try {
      const response = await fetch(`/api/draft/${leagueId}/trade`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
