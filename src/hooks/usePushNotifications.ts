...
export function usePushNotifications() {
  ...
  useEffect(() => {
    const initializeNotifications = async () => {
      ...
      try {
        const isSupported = notificationManager.canShowNotifications();
  ...
  const scheduleMatchupReminder = useCallback((matchupData: any, reminderTime: Date) => {
    try {
  ...
  const scheduleWaiverReminder = useCallback((waiverData: any) => {
    try {
  ...
  return {
    ...
    scheduleMatchupReminder,
    scheduleWaiverReminder,
    clearError,
