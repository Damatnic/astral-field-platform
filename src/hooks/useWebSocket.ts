...
export interface UseWebSocketReturn {
  ...
  joinMatchup: (matchupId: string) => void;
  leaveMatchup: (matchupId: string) => void;
...
export function useWebSocket(options: UseWebSocketOptions = {}): UseWebSocketReturn {
  const { autoConnect = true, leagueId, matchupId } = options;
  ...
  // Messaging
  const sendMessage = useCallback((leagueId: string, message: string, type: 'chat' | 'reaction' = 'chat') => {
...
export function useLeagueWebSocket(leagueId: string) {
  const webSocket = useWebSocket({ leagueId, autoConnect: true });
...
export function useMatchupWebSocket(matchupId: string) {
  const webSocket = useWebSocket({ matchupId, autoConnect: true });
...
export function usePlayerWebSocket() {
  ...
  return {
    ...
    getPlayerUpdate: (playerId: string) => playerUpdates.get(playerId)
  };
}