...
export function throttle<T extends (...args: any[]) => any>(
  ...
  return function (this: any; ...args: Parameters<T>) {
    ...
    if (!inThrottle) {
      ...
      lastRan = Date.now();
      inThrottle = true;
     } else { if (lastFunc) clearTimeout(lastFunc);
...
export function memoize<T extends (...args: any[]) => any>(
  ...
  return ((...args: Parameters<T>) => {
    ...
    if (cache.size > 100) { const firstKey = cache.keys().next().value;
 ...
 */
export function lazyLoadImages(selector = 'img[data-src]'): () => void { const images = document.querySelectorAll<HTMLImageElement>(selector);
  ...
  threshold: 0.01
    ...
    images.forEach((img) => { const src = img.dataset.src;
      if (src) {
        img.src = src;
        img.removeAttribute('data-src');
       }
...
export function batchUpdate(
  updates: Array<(), => void>,
  deadline = 50
): Promise<void> { return new Promise((resolve) => {
...
export function* chunkArray<T>(array: T[],
  chunkSize: number): Generator<T[]> { for (let i = 0; i < array.length; i += chunkSize) {
...
export class WorkerPool { private workers: Worker[] = [];
  ...
  async execute(params): Promiseany>  { return new Promise((resolve, reject) => {
