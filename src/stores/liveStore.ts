...
interface LiveState {
  ...
  // UI actions
  setSelectedMatchup: (index: number) => void;
...
export const useLiveStore = create<LiveState>()(subscribeWithSelector((set, get) => ({
  ...
  startLiveScoring: async (leagueId: string, week = 1) => {
    ...
    try {
      const liveData = await get().fetchLiveScoring(leagueId, week);
      ...
      if (liveData) {
        ...
        throw new Error('Failed to fetch live scoring data');
      }
    } catch (error) {
  ...
  refreshLiveScoring: async (leagueId?: string, week?: number) => {
    ...
    try {
      const liveData = await get().fetchLiveScoring(targetLeagueId, targetWeek);
  ...
  handleScoreUpdate: (update: LiveScoreUpdate) => {
    const state = get();
    ...
    switch (event.type) {
      ...
      case 'injury_update':
        notification = {
          ...
          read: false,
          priority: 'urgent',
          playerId: (event.data as any)?.playerId,
          leagueId: event.leagueId,
          soundEnabled: state.soundEnabled
        };
        break;
  ...
  playNotificationSound: (type: LiveNotification['type']) => {
    const state = get();
  ...
  // Data fetching
  fetchLiveScoring: async (leagueId: string, week = 1): Promise<LeagueLiveScoring | null> => {
    try {
...
useLiveStore.subscribe(
  ...
  (connectionStatus, previousConnectionStatus) => {
    if (previousConnectionStatus === 'connected' && connectionStatus === 'disconnected') {
...
useLiveStore.subscribe(
  (state) => ({ autoRefresh: state.autoRefresh, isLiveScoringActive: state.isLiveScoringActive, refreshInterval: state.refreshInterval }),
  ({ autoRefresh, isLiveScoringActive, refreshInterval }) => {
    if (autoRefresh && isLiveScoringActive) {
