...
interface PWAContextType { pwaManager: PWAManager,
...
export default function PWAProvider({ children: config }: PWAProviderProps) { const [pwaManager]  = useState(() => PWAManager.getInstance());
  ...
  backgroundSyncActive: false, cacheSize: 0,
  syncQueueSize, 0
          });
        }
      } catch (error) {
  ...
  useEffect(() => {  if (!isInitialized) return;
  ...
  position: 'bottom-center'
    ...
    const handleNetworkStatusChange  = (isOnline: boolean) => {  if (isOnline) {
  ...
  position: 'bottom-center',
    ...
    const handleSyncSuccess  = (event: CustomEvent) => { const { count } = event.detail;
      if (count > 0) {
  ...
  position: 'bottom-center'
    ...
    const handleOfflineNotification  = (event: CustomEvent) => { const { message } = event.detail;
      toast(message: { 
        duration: 3000,
  position: 'bottom-center',
        icon: '📱'
  ...
  // Context methods
  const installApp = async (): Promise<boolean> => {  try {
  ...
  const requestNotifications  = async (): Promise<NotificationPermission> => {  try {
  ...
  const subscribeToNotifications  = async (): Promise<PushSubscription | null> => {  try {
  ...
  const unsubscribeFromNotifications  = async (): Promise<boolean> => {  try {
  ...
  const contextValue: PWAContextType  = { pwaManager: status,
    isLoading, installApp,
    requestNotifications, subscribeToNotifications,
...
export function usePWA() { const context = useContext(PWAContext);
...
export function useOfflineStatus() { const [isOnline, setIsOnline] = useState(true);
...
export function useInstallStatus() {  const [isInstallable, setIsInstallable] = useState(false);
