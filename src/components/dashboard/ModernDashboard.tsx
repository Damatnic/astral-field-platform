...
const mockPlayers: Player[] = [
  ...
  },
  {
    ...
    injuryDetails: 'Hamstring injury - limited in practice',
    byeWeek: 8,
  ...
  },
  {
    ...
    actualPoints: 22.1,
    injuryStatus: 'out',
    injuryDetails: 'Ankle sprain - ruled out for Sunday',
    byeWeek: 10,
...
const mockLineup: LineupSlot[] = [
  ...
  { position: 'FLEX', player: null, isRequired: true }
]
...
export function ModernDashboard() {
      ...
      try {
        ...
        if (user) {
          ...
          const mockData = {
            ...
            recentActivity: [],
            upcomingEvents: [
              { type: 'lineup_lock', time: '2024-01-07T13:00:00Z', message: 'Lineup locks in 2 hours' },
              { type: 'game_start', time: '2024-01-07T18:00:00Z', message: 'First games start at 1:00 PM' }
            ]
          }
  ...
  const handlePlayerSwap = (fromPlayerId: string, toPlayer: Player) => {
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      accessor: (player: Player) => `${player.projectedPoints.toFixed(1)}`
    },
    {
      key: 'injuryStatus',
      header: 'Status',
    ...
    },
    {
      ...
      hideOnMobile: true,
      accessor: (player: Player) => {
        const difficulty = player.matchupDifficulty || 'moderate'
        const colors = {
          easy: 'text-green-400',
    ...
    >
      <div className="space-y-6 sm:space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {[
            { icon: Trophy, label: 'Active Leagues', value: dashboardData?.stats.activeLeagues || 0, color: 'text-yellow-500' },
            { icon: TrendingUp, label: 'Win Rate', value: `${dashboardData?.stats.winRate || 0}%`, color: 'text-green-500' },
            { icon: Star, label: 'Total Points', value: dashboardData?.stats.totalPoints || 0, color: 'text-blue-500' },
            { icon: Clock, label: 'Next Game', value: dashboardData?.stats.nextGame || '-', color: 'text-purple-500' }
          ].map((stat, index) => (
            <motion.div
              key={stat.label}
          ...
          <section className="space-y-4">
            ...
            >
              <MobileTable
                ...
                onRowClick={(player) => console.log('Player clicked:', player.name)}
                expandable
                ...
                sortable
                defaultSort={{ key: 'projectedPoints', direction: 'desc' }}
              />
            </LoadingWrapper>
          </section>
        </SimpleErrorBoundary>
            ...
            {[
              {
                ...
                color: 'text-blue-500',
                action: () => console.log('Navigate to players')
              },
              {
                icon: BarChart3,
                title: 'Analytics',
              ...
              },
              {
                ...
                action: () => console.log('Navigate to leagues')
              }
            ].map((action, index) => (
              <motion.button
                key={action.title}
