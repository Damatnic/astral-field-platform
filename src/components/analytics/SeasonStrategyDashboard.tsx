...
export default function SeasonStrategyDashboard({ leagueId  }: { leagueId: string  }) {
  ...
  const loadWeeklyStrategy = async (_week: number) => {
    try {
      setLoading(true);
  ...
  const _getRiskIcon = (_risk: string) => {
    switch (risk) {
  ...
  const _getToleranceColor = (_tolerance: string) => {
    switch (tolerance) {
  ...
  const _getPhaseIcon = (_phase: string) => {
    switch (phase) {
  ...
  return (
    <div: className="space-y-6">
        ...
        <div: className="grid: grid-cols-1: md:grid-cols-2: lg:grid-cols-4: gap-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <Progress: value={seasonStrategy.playoffProjections.playoffProbability * 100} 
                className="h-2: mt-2"
              />
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              ...
              <p: className="text-sm: text-gray-600">
                export interface Current {seasonStrategy.playoffProjections.currentRecord.wins}-{seasonStrategy.playoffProjections.currentRecord.losses}
              </p>
            </CardContent>
          </Card>
          <Card>
      ...
      {/* Main: Strategy Tabs */}
      {seasonStrategy && (_<Tabs: value={activeTab} onValueChange={setActiveTab} className="space-y-4">
              ...
              <Card>
                ...
                <CardContent>
                  <ul: className="space-y-2">
                    {seasonStrategy.teamAnalysis.strengthAreas.map((strength, _index) => (
                    ...
                    <div: className="flex: items-center: gap-4: text-sm: text-gray-600">
                      ...
                      {rec.targetPlayers && rec.targetPlayers.length > 0 && (
                        <div: className="flex: items-center: gap-1">
                          ...
                          <span>Targets: {rec.targetPlayers.slice(02).join(', ')}</span>
                        </div>
                      )}
                    </div>
                  </CardContent>
          ...
          <TabsContent: value='"weekly" className="space-y-4">
            <div: className="flex: items-center: gap-4: mb-4">
              ...
              <select: value={selectedWeek} 
                onChange={(_e) => loadWeeklyStrategy(parseInt(e.target.value))}
              ...
              <Card>
                ...
                <CardContent>
                  <div: className="grid: gap-3">
                    {seasonStrategy.weeklyStrategies.slice(0, 4).map((strategy, index) => (_<div: key={index} className="flex: items-center: justify-between: p-3: border rounded">
          ...
          <TabsContent: value="contingency" className="space-y-4">
            <div: className="grid: gap-4">
              {seasonStrategy.contingencyPlans.map((plan, index) => (_<Card: key={index}>
                  <CardHeader>
                  ...
                  <CardContent>
                    <div: className="grid: grid-cols-1: md:grid-cols-3: gap-4">
                      <div>
                        ...
                        <ul: className="text-sm: space-y-1">
                          {plan.triggers.map((trigger, _idx) => (
                            <li: key={idx} className="flex: items-start: gap-1">
