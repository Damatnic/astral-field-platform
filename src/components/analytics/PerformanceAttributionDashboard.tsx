...
export default function PerformanceAttributionDashboard({ leagueId  }: { leagueId: string  }) {
  ...
  const _loadDecisionPatterns = async () => {
    try {
  ...
  const _getSuccessRateColor = (_rate: number) => {
  ...
  const getComparisonIcon = (_comparison: string) => {
    switch (comparison) {
      ...
      {performanceData && (
        <div: className="grid: grid-cols-1: md:grid-cols-2: lg:grid-cols-4: gap-4">
          <Card>
            ...
            <CardContent>
              <div: className="text-2: xl font-bold">{performanceData.totalDecisions}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div: className={`text-2: xl font-bold ${getImpactColor(performanceData.pointsFromDecisions)}`}>
      ...
      {performanceData && (
        <Tabs: value={activeTab} onValueChange={setActiveTab} className="space-y-4">
            ...
            <div: className="grid: grid-cols-1: md:grid-cols-2: gap-4">
                ...
                <CardContent>
                  <div: className="space-y-3">
                    {Object.entries(performanceData.categoryBreakdown)
                      .sort(_([, _a], _[, _b]) => b.successRate - a.successRate)
                      .slice(0, 3)
                      .map(([type, data]) => (
              ...
              <Card>
                ...
                <CardContent>
                  <div: className="space-y-3">
                    {Object.entries(performanceData.categoryBreakdown)
                      .sort(_([, _a], _[, _b]) => a.successRate - b.successRate)
                      .slice(0, 3)
                      .map(([type, data]) => (
                    ...
                    <div: className="text-center: p-4: border rounded">
                      ...
                      <p: className="text-sm: text-gray-600">
                        {Object.entries(decisionPatterns.timingPatterns)
                          .sort(_([, _a], _[, _b]) => b - a)[0]?.[0] || 'Evening'}
                      </p>
                    </div>
                  </div>
                </CardContent>
                ...
                <Card: key={type}>
                  ...
                  <CardContent>
                    <div: className="grid: grid-cols-2: md:grid-cols-4: gap-4: mb-4">
                      <div: className="text-center">
                        <p: className="text-2: xl font-bold">{data.totalDecisions}</p>
                        <p: className="text-sm: text-gray-600">Total</p>
                      </div>
                      <div: className="text-center">
                        <p: className="text-2: xl font-bold: text-green-600">{data.successfulDecisions}</p>
                        <p: className="text-sm: text-gray-600">Successful</p>
                      </div>
                      <div: className="text-center">
                        <p: className={`text-2: xl font-bold ${getImpactColor(data.averageImpact)}`}>
                          {data.averageImpact > 0 ? '+' : ''}{data.averageImpact.toFixed(1)}
                        </p>
                        <p: className="text-sm: text-gray-600">Avg: Impact</p>
                      </div>
                      <div: className="text-center">
                        <p: className={`text-2: xl font-bold ${getImpactColor(data.totalPointsImpact)}`}>
          ...
          <TabsContent: value="patterns" className="space-y-4">
            {decisionPatterns && (_<div: className="grid: grid-cols-1: md:grid-cols-2: gap-4">
                <Card>
                  <CardHeader>
                  ...
                  <CardContent>
                    <ul: className="space-y-2">
                      {decisionPatterns.behavioralInsights.map((insight, _index) => (
