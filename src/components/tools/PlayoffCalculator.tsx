...
import { 
  Trophy, Target, TrendingUp, Calendar, Calculator,
  BarChart3, Percent, Users, Crown, Star, AlertTriangle,
  CheckCircle, XCircle, Clock, Zap, RefreshCw, Info
} from 'lucide-react';
...
export default function PlayoffCalculator({ 
  ...
  const sortedTeams = [...(teams || MOCK_TEAMS)].sort((a, b) => {
  ...
  useEffect(() => {
    if (sortedTeams.length > 0 && !selectedTeam) {
    ...
    try {
      ...
      if (teamRank <= playoffSpots) {
        ...
        mockScenarios.push({
          ...
          seedRange: { min: Math.max(1, teamRank - 2), max: Math.min(playoffSpots, teamRank + 1) },
        ...
        mockScenarios.push({
          ...
          seedRange: { min: Math.max(4, playoffSpots - 1), max: playoffSpots },
      ...
      if (!team.isEliminated && team.remainingSchedule.length > 0) {
        mockScenarios.push({
          ...
          seedRange: { min: 1, max: Math.min(playoffSpots, 3) },
  ...
  const generateKeyGames = (team: Team, scenarioType: string): KeyGame[] => {
    const games: KeyGame[] = [];
  ...
  const getTeamStatus = (team: Team) => {
    if (team.isClinched) return { status: 'Clinched', color: 'text-green-600 bg-green-100 dark:bg-green-900 dark:text-green-300' };
    if (team.isEliminated) return { status: 'Eliminated', color: 'text-red-600 bg-red-100 dark:bg-red-900 dark:text-red-300' };
    
    const rank = sortedTeams.findIndex(t => t.id === team.id) + 1;
    if (rank <= playoffSpots) return { status: 'In Playoffs', color: 'text-blue-600 bg-blue-100 dark:bg-blue-900 dark:text-blue-300' };
    return { status: 'Outside', color: 'text-yellow-600 bg-yellow-100 dark:bg-yellow-900 dark:text-yellow-300' };
  };

  const getScenarioProbabilityColor = (probability: number) => {
                ...
                return (
                  <div 
                    ...
                    className={`p-3 rounded-lg border dark:border-gray-700 ${
                      team.id === selectedTeam ? 'ring-2 ring-primary-500 bg-primary-50 dark:bg-primary-900/20' : ''
                    }`}
                  >
                  ...
                  <div className="space-y-4">
                    ...
                    {scenarios.length > 2 && (
                      <button
                        ...
                        className="w-full py-2 text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg transition-colors"
                      >
