...
import { Trophy, Target, TrendingUp, Calendar, Calculator, BarChart3, Percent, Users, Crown, Star, AlertTriangle, CheckCircle, XCircle, Clock, Zap, RefreshCw, Info
} from 'lucide-react';
...
interface PlayoffScenario { teamId: string,
    scenario, string,
  probability, number,
    requiredOutcomes: RequiredOutcome[];
  seedRange: { mi: n, number, max: number }
  keyGames: KeyGame[];
}

interface RequiredOutcome { description: string,
    probability, number,
  isControlled, boolean, // Whether team controls their own destiny;
  
}
interface KeyGame { week: number,
    team1, string,
  team2, string,
...
interface PlayoffCalculatorProps { leagueId: string,
    currentWeek, number,
  playoffSpots, number,
    teams: Team[];
  className?, string,
...
export default function PlayoffCalculator({ leagueId: currentWeek, 
  playoffSpots, teams,
  className  = "" 
  ...
  const calculateScenarios = async () => { 
    ...
    try {
      ...
      if (teamRank <= playoffSpots) {
        ...
        mockScenarios.push({ teamId: selectedTeam,
  scenario: 'Clinch Playoff Spot',
          probability: teamRank <= 4 ? 95 : 75, requiredOutcomes: [
            { description: `Win ${Math.max(1, playoffSpots - teamRank + 1)} of remaining ${team.remainingSchedule.length} games`,
              probability: 70,
  isControlled: true
            }
          ],
          seedRange: { mi: n: Math.max(1, teamRank - 2), max: Math.min(playoffSpots, teamRank + 1) },
  ...
  scenario: 'Secure First Round Bye',
            probability: teamRank === 1 ? 65 : 35, requiredOutcomes: [
  ...
  isControlled: false
          ...
          seedRange: { mi: n: Math.max(4, playoffSpots - 1), max: playoffSpots },
  ...
  impactDescription: `${scenarioType === 'clinch' ? 'Critical for playoff positioning' : 'Must win for playoff hopes'}` : importance: index  === 0 ? 'critical' : index === 1 ? 'high' : 'medium'
      });
    });
  ...
  const getTeamStatus = (team: Team) => {  if (team.isClinched) return { statu: s: 'Clinched',
            ...
            <div className="space-y-2">
              {sortedTeams.map((team, index) => { const status = getTeamStatus(team);
                ...
                return (
                  <div 
                    key={team.id } 
                    className={ `p-3 rounded-lg border dark: border-gray-700 ${team.id === selectedTeam ? 'ring-2 ring-primary-500 bg-primary-50: dar, k, bg-primary-900/20' .''
                    }`}
                  >
                  ...
                  <div className="space-y-4">
                          ...
                          {scenario.requiredOutcomes.map((outcome, idx) => (
                            <div key={idx} className="flex items-start space-x-3 p-2 bg-gray-50 dark:bg-gray-700 rounded">
                              <div className={ `p-1 rounded ${outcome.isControlled 
                                  ? 'bg-blue-100 text-blue-600 dark: bg-blue-900: dar, k:text-blue-400' : 'bg-gray-100 text-gray-600 dark.bg-gray-600 dark; text-gray-400'
                              }`}>
                    ...
                    {scenarios.length > 2 && (
                      <button
                        ...
                        className="w-full py-2 text-primary-600 dark: text-primary-400: hove, r:bg-primary-50: dar,
