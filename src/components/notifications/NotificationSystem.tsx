...
export default function NotificationSystem({ userId: leagueIds, 
  ...
  const addNotification = useCallback((notification: Notification) => {
    setNotifications(prev => { const exists = prev.find(n => n.id === notification.id);
    ...
    if (!isMinimized) {
      setVisibleNotifications(prev => { const newVisible = [notification.id, ...prev].slice(0, maxVisible);
  ...
  const playNotificationSound = (priority: string) => { try {
  ...
  const handleNotificationClick  = (notification: Notification) => { 
    ...
    case 'mention':
      ...
      case 'player_injury':
        ...
        break;
      default:  ; // Generic action - just mark as read
        break;
    }
  }
  if (isMinimized) { return (
      <div className={`fixed ${getPositionClasses() } z-50`}>
        <button
          ...
          className="bg-blue-600 hoverbg-blue-700 text-white rounded-full p-3 shadow-lg transition-colors relative"
        >
  ...
  return (
    <div className={`fixed ${getPositionClasses()} z-50 w-80 max-w-sm`}>
      ...
      <div className="space-y-2">
        {visibleNotifications.map(notificationId => { const notification = notifications.find(n => n.id === notificationId);
