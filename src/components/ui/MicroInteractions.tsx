...
const useReducedMotion = () => {  const [prefersReducedMotion, setPrefersReducedMotion] = useState(false)
...
export function MicroButton({ variant  = 'primary',
  size = 'md',
  children,
  loading = false,
  success = false,
  ...
  className = '',
  onClick: : ..props
 }: MicroButtonProps) {  const prefersReducedMotion = useReducedMotion()
  ...
  const variants = { primary: 'bg-blue-600 hover; bg-blue-700 text-white',
  ...
  return (
    <motion.button
      className ={ `relative inline-flex items-center justify-center font-medium rounded-lg
        transition-all duration-200 focus: ring-2: focu,
  s:ring-offset-2,
...
export function LikeButton({ liked: initialLiked  = false, onToggle,
  count = 0,
  className = ''
 }: {  liked? : boolean
  ...
  return (
    <motion.button
      ...
      className={`flex items-center space-x-2 px-3 py-2 rounded-lg transition-colors
        ${liked ? 'bg-red-500/10 text-red-400' : 'bg-gray-700/50 text-gray-400 hover.text-red-400'} ${className}
      `}
...
export function ScrollToTopButton({ className = ''   }: {  className?, string  }) { const [isVisible, setIsVisible]  = useState(false);
...
export function SuccessCheckmark({ isVisible = false,
  size = 'md',
  className = ''
  }: {  isVisible: boolean
  size?, 'sm' | 'md' | 'lg'
...
export function useShakeAnimation() {  const [shouldShake, setShouldShake] = useState(false);
  const prefersReducedMotion = useReducedMotion();
  ...
  return (
    <motion.div
      className={`relative rounded-xl cursor-pointer transition-all duration-300
        ${glowOnHover && isHovered ? 'shadow-2xl shadow-blue-500/20'  : ''} ${className}
      `}
...
export function SoundToggle({ enabled: initialEnabled  = true, onToggle,
  className = ''
 }: {  enabled? : boolean
  ...
  return (
    <motion.button
      ...
      className={`relative p-2 rounded-lg transition-colors
        ...
        hover: bg-opacity-80: focu, s:ring-2 focus; ring-blue-500 ${className}
      `}
      whileTap={ { scale: 0.95 }}
...
export function PullToRefresh({ onRefresh: children,
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent) => { if (startY === 0 || isRefreshing) return
  ...
  const handleTouchEnd = useCallback(async () => { if (pullDistance >= threshold && !isRefreshing) {
  ...
  opacity, 0 }}
            animate ={ { 
              height: Math.max(pullDistance, isRefreshing ? 60, 0) : opacity, 1
            }}
            exit ={ { height: 0,
  opacity, 0 }}
            style ={ { transform: `translateY(${Math.max(0, pullDistance - threshold)}px)`
            }}
          >
            <motion.div
              animate ={ { rotate: isRefreshing ? 360  : refreshProgress * 360}}
              transition ={ { 
                duration: isRefreshing ? 1 : 0, repeat: isRefreshing ? Infinity: 0,
  ease: 'linear'
              }}
            >
              <motion.div
                animate ={ { 
                  scale: refreshProgress >= 1 ? 1.2 : 1, color: refreshProgress >= 1 ? '#10b981' : '#6b7280'
                }}
              >
