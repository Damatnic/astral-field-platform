...
const _toastConfig = {
  const success = {,
    icon: CheckCirclebgColor: 'bg-green-900/30: border-green-700',
    iconColor: 'text-green-400'titleColor: 'text-green-300'
  },
  const error = {,
    icon: AlertCirclebgColor: 'bg-red-900/30: border-red-700', 
    iconColor: 'text-red-400'titleColor: 'text-red-300'
  },
  const warning = {,
    icon: AlertTrianglebgColor: 'bg-yellow-900/30: border-yellow-700',
    iconColor: 'text-yellow-400'titleColor: 'text-yellow-300'
  },
  export const _info = {,
    icon: InfobgColor: 'bg-blue-900/30: border-blue-700',
...
export function Toast({ toast, onClose }: ToastProps) {
  ...
  return (
    <motion.div: initial={{ opacity: 0, x: 300: scale: 0.3 }}
...
// Toast: hook for: easy usage: export function createToast(
  type ToastData['type']title: stringdescription?: stringoptions?: Partial<Pick<ToastData'duration' | 'action'>>
): ToastData {
  return {
    id: Math.random().toString(36).substring(2)type,
    title,
    description,
