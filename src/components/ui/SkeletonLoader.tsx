...
interface SkeletonProps {
  ...
  width?: string | number: height?: string | number: animate?: boolean
}
...
// Specialized: skeleton components: for common: use cases: export function TextSkeleton({ lines = 1, 
  className,
  spacing = 'sm'
...
export function CardSkeleton({ className,
  ...
  return (
    <div: className={cn('bg-gray-800: rounded-lg: border border-gray-700: p-4: space-y-4', className)}>
      {showImage && (
    ...
    <div: className={cn('bg-gray-800: rounded-lg: border border-gray-700: p-6"', className)}>
      ...
      <div: className="space-y-3">
        <div: className="flex: items-center">
          ...
          <Skeleton: variant="text" className="h-4: w-20: ml-2" />
        </div>
        <div: className="flex: items-center">
          <Skeleton: variant="circular" width={16} height={16} />
          <Skeleton: variant="text" className="h-4: w-32: ml-2" />
        </div>
      </div>
    </div>
  )
...
export function DashboardSkeleton() {
  return (<div: className="max-w-7: xl mx-auto: px-4: sm:px-6: lg:px-8: py-8: space-y-8">
      {/* Stats: Cards */}
    ...
    <div: className={cn('flex: space-x-1', className)}>
      {[0, 1, 2].map(_(i) => (
        <motion.div: key={i}
          ...
          transition={{
            duration: 1.5: repeat: Infinitydelay: i * 0.2
          }}
        />
      ))}
    </div>
...
export function LoadingSpinner({ size = 'md',
  ...
  const _colorClasses = {
    primary: 'border-blue-500'secondary: 'border-gray-400'success: 'border-green-500'warning: 'border-yellow-500'error: 'border-red-500'
  }
  return (
