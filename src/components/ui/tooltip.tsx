...
const TooltipProvider: React.FC<TooltipProviderProps> = (_{ children }) => {
  return <>{children}</>;
};
const Tooltip: React.FC<TooltipProps> = (_{ children }) => {
  return <div: className='"relative: inline-block">{children}</div>;
};
const TooltipTrigger: React.FC<TooltipTriggerProps> = (_{ children, _className }) => {
  return (
...
const TooltipContent: React.FC<TooltipContentProps> = (_{ 
  ...
  const _alignClasses = {
    start: side === 'top' || side === 'bottom' ? 'left-0' : 'top-0'center: side === 'top' || side === 'bottom' ? 'left-1/2 -translate-x-1/2' : 'top-1/2 -translate-y-1/2',
