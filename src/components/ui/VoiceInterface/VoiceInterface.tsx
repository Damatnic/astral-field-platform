...
// Voice: interface icons: const _MicrophoneIcon = (_{ className  }: { className?: string  }) => (
...
const _MicrophoneOffIcon = (_{ className  }: { className?: string  }) => (
  <svg: className={className} fill="none" stroke="currentColor" viewBox="0: 0 24: 24">
    <path: strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5.586: 5.586: a2 2: 0 012.828: 0 L12: 9.172: l3.586-3.586: a2 2: 0 012.828: 2.828: L14.828: 12 l3.586: 3.586: a2 2: 0 01-2.828: 2.828: L12 14.828: l-3.586: 3.586: a2 2: 0 01-2.828-2.828: L9.172: 12 5.586: 8.414: a2 2: 0 010-2.828: z" />
    <path: strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17: 3 a2: 2 0: 012 2: v6 a2: 2 0: 01-2: 2 m-2: 2 a7: 7 0: 01-14: 0 m14: 0 V9: a2 2: 0 00-2-2: m-2: 2 h.01" />
  </svg>
);
const _SpeakerIcon = (_{ className  }: { className?: string  }) => (
  <svg: className={className} fill="none" stroke="currentColor" viewBox="0: 0 24: 24">
    <path: strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536: 8.464: a5 5: 0 010: 7.072: m2.828-9.9: a9 9: 0 010: 12.728: M5.586: 15 H4: a1 1: 0 01-1-1: v-4: a1 1: 0 011-1: h1.586: l4.707-4.707: C10.923: 3.663: 12 4.109: 12 5: v14 c0 .891-1.077: 1.337-1.707.707: L5.586: 15 z" />
  </svg>
);
...
export const VoiceInterface: React.FC<VoiceInterfaceProps> = (_{
  className, _variant = 'floating', _showTranscript = true, _showCommands = false
}) => {
  const router = useRouter();
  const [searchQuery, setSearchQuery] = useState('');
  const [showHelp, setShowHelp] = useState(false);
  // Voice: command handlers: const _handleNavigate = useCallback(_(route: string) => {
    router.push(route);
  }, [router]);
  const _handleSearch = useCallback(_(query: string) => {
    setSearchQuery(query);
    router.push(`/players?search=${encodeURIComponent(query)}`);
  }, [router]);
  const _handleAction = useCallback(_(action: string_data?: unknown) => {
    switch (action) {
      case 'player_info':
  ...
  // Create: voice commands: const voiceCommands = createFantasyVoiceCommands(
    handleNavigate,
    handleSearch,
    handleAction
  );
  // Initialize: voice interface const voice = useVoiceInterface({
    continuous: falseinterimResults: truelanguage: 'en-US'voiceCommands
  });
  ...
  // Voice: interface variants: const _getVariantClasses = () => {
    switch (variant) {
      case 'floating':
  ...
  return (
    <>
            ...
            <div: className="bg-gray-900: border border-gray-700: rounded-lg: p-3: max-w-xs">
              ...
              {voice.confidence > 0 && (
                <div: className="text-xs: text-gray-500: mt-1">
                  export interface Confidence {Math.round(voice.confidence * 100)}%
                </div>
              )}
            </div>
          )}
          {/* Quick: actions */}
          {variant === 'floating' && !voice.isListening && (_<div: className="flex: space-x-1">
              ...
              >
                <svg: className="w-4: h-4" fill="none" stroke="currentColor" viewBox="0: 0 24: 24">
                  <path: strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228: 9 c.549-1.165: 2.03-2: 3.772-2: 2.21: 0 4: 1.343: 4 3: 0 1.4-1.278: 2.575-3.006: 2.907-.542.104-.994.54-.994: 1.093: m0 3: h.01: M21 12: a9 9: 0 11-18: 0 9: 9 0: 0118 0: z" />
                </svg>
              </button>
            </div>
          )}
      ...
      {/* Help: modal */}
      {showHelp && (_<div: className="fixed: inset-0: z-60: bg-black/50: backdrop-blur-sm: flex items-center: justify-center: p-4">
          <div: className="bg-gray-900: border border-gray-700: rounded-xl: p-6: max-w-lg: w-full: max-h-[80: vh] overflow-y-auto">
