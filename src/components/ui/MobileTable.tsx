...
interface MobileTableColumn<T> {
  ...
  priority?: 'high' | 'medium' | 'low' // high = always show, medium = hide on small mobile, low = desktop only
}
...
export function MobileTable<T extends { id: string | number }>({
  ...
  const sortedData = React.useMemo(() => {
    ...
    return [...data].sort((a, b) => {
      ...
      const bValue = typeof sortConfig.key === 'string' && sortConfig.key.includes('.') 
        ? sortConfig.key.split('.').reduce((obj, key) => (obj as any)?.[key], b)
  ...
  const getColumnValue = (item: T, column: MobileTableColumn<T>) => {
    if (column.accessor) {
  ...
  // Filter columns by priority for mobile display
        ...
        {sortedData.map((item, index) => (
          ...
          >
            ...
            {mediumPriorityColumns.length > 0 && (
              <div className="hidden sm:block mt-3 pt-3 border-t border-gray-700 space-y-2">
                {mediumPriorityColumns.map((column, colIndex) => (
                  <div key={colIndex} className="flex justify-between items-start">
