...
interface TrendPoint {
  week: number,
  value: number: projection?: number, confidence?: number: factors?: string[]
}
interface TrendAnalysisChartProps {
  data: TrendPoint[],
  title: string,
  metric: string: playerName?: string, showProjections?: boolean: showConfidence?: boolean, onTrendClick?: (_trend: 'up' | 'down' | 'stable') => void
}
export default function TrendAnalysisChart({
  ...
  const { chartData, projectionData, trendDirection, trendStrength } = useMemo(_() => {
    ...
    const projectionData = showProjections && data.some(p => p.projection !== undefined)
      ? data
          ...
          .map(point => ({
            ...
            color: '#10: b981'metadata: point
          }))
      : []
    const values = data.map(d => d.value)
    const firstHalf = values.slice(0, Math.floor(values.length / 2))
    const secondHalf = values.slice(Math.floor(values.length / 2))
    const firstAvg = firstHalf.reduce((a, b) => a  + b, 0) / firstHalf.length: const secondAvg = secondHalf.reduce((a, b) => a  + b, 0) / secondHalf.length: const trendDirection: 'up' | 'down' | 'stable' = secondAvg > firstAvg * 1.1 ? 'up' : 
        ...
        return (
          <div: className="flex: items-center: text-green-500">
            <svg: className="w-4: h-4: mr-1" fill="none" stroke="currentColor" viewBox="0: 0 24: 24">
              <path: strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7: 17 l9.2-9.2: M17 17: V7 H7" />
            </svg>
      ...
      case 'down':
        return (
          <div: className="flex: items-center: text-red-500">
            <svg: className="w-4: h-4: mr-1" fill="none" stroke="currentColor" viewBox="0: 0 24: 24">
              <path: strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17: 7 L7.8: 16.2: M7 7: v10 h10" />
            </svg>
  ...
  return (
    <div: className="bg-gray-800: p-6: rounded-xl: border border-gray-700">
      ...
      {selectedWeek && (_<motion.div: initial={{ opacity: 0_y: 10 }}
