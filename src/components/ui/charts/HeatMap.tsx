...
export function HeatMap({
  ...
  const colorScales = useMemo(() => ({
    blue: { min: '#dbeafe', mid: '#3b82f6', max: '#1e3a8a' },
    green: { min: '#d1fae5', mid: '#10b981', max: '#064e3b' },
    red: { min: '#fee2e2', mid: '#ef4444', max: '#7f1d1d' },
    purple: { min: '#ede9fe', mid: '#8b5cf6', max: '#4c1d95' },
    orange: { min: '#fed7aa', mid: '#f97316', max: '#7c2d12' }
  }), []);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      xLabels.forEach((label, index) => {
        const x = index * (cellWidth + cellGap) + cellWidth / 2;
        ctx.save();
      ...
      yLabels.forEach((label, index) => {
        const y = index * (cellHeight + cellGap) + cellHeight / 2;
