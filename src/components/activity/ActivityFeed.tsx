...
import { 
  ...
  ArrowUpDown, Zap, Shield, Crown, Star,
...
export default function ActivityFeed({ 
  ...
  const filterOptions = [
    ...
    { value: 'trade', label: 'Trades', icon: ArrowUpDown },
    { value: 'injury', label: 'Injuries', icon: AlertTriangle },
    { value: 'waiver', label: 'Waivers', icon: DollarSign },
    { value: 'performance', label: 'Performances', icon: Zap },
    { value: 'commissioner', label: 'Announcements', icon: Crown }
  ];
  ...
  const getActivityIcon = (activity: ActivityItem) => {
    const iconClass = `h-5 w-5 ${compact ? 'h-4 w-4' : ''}`;
    ...
    <div className={`bg-white dark:bg-gray-800 rounded-lg shadow ${className}`}>
            ...
            {filteredActivities.map((activity) => (
              <div
                ...
                className={`p-4 border-l-4 ${getPriorityColor(activity.priority)} ${
                  !activity.isRead ? 'bg-primary-50/50 dark:bg-primary-900/10' : ''
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <h4 className={`text-sm font-medium ${
                        !activity.isRead ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300'
                      }`}>
                        {activity.title}
                    ...
                    {activity.actionable && activity.actionUrl && (
                      <div className="mt-2">
                        <button
                          ...
                          className="inline-flex items-center space-x-1 text-xs text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300"
                        >
      ...
      {filteredActivities.length >= maxItems && (
        <div className="p-4 border-t dark:border-gray-700 text-center">
          <button className="text-sm text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 flex items-center justify-center space-x-1">
