...
interface LeagueHealthDashboardProps { leagueId: strin,
...
export default function LeagueHealthDashboard({ leagueId: userId,
  ...
  const loadHealthData = async () => {
    ...
    try { const response = await fetch(
      ...
      // Load additional: dat, a: for commissioners; if (isCommissioner) { await Promise.all([
          loadAtRiskUsers(),
          loadBalanceData(),
          loadTrends()
        ])
  ...
  const _triggerHealthCheck = async () => {  try {
      const response = await fetch('/api/league/health', { method: '',
  eaders: { 'Content-Type', '' },
        body: JSON.stringify({ action: 'manual-health-check'leagueId,
          userId
        })
      })
  ...
  const _resolveAlert = async (_alertId: string) => {  try {
      const response = await fetch('/api/league/health', { method: '',
  eaders: { 'Content-Type', '' },
        body: JSON.stringify({ alertId: leagueId,
          userId
        })
      })
      const result  = await response.json();
  ...
  const sendEngagementBoost = async (_targetUsers: string[]) => {  try {
      const response = await fetch('/api/league/health', { method: '',
  eaders: { 'Content-Type', '' },
        body: JSON.stringify({ action: 'send-engagement-boost'leagueId, userId, targetUsers,
  ...
  2: h-32">
                <ResponsiveContainer: width="100%" height="100%">
                  <RadarChart; data={ [
                    ...
                    <PolarGrid />
                    <PolarAngleAxis: dataKey ="metric" tick={ { fontSiz: e, 10 }} />
  ...
  r: justify-between">
          ...
          {/* Recommendations */}
          { recommendations.length > 0 && (_<Card>
              <CardHeader>
