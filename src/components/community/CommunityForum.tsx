...
export default function CommunityForum({ 
  ...
  const [activeView, setActiveView] = useState<'categories' | 'category' | 'thread'>('categories');
            ...
            <div className="text-right">
              ...
              {category.lastActivityAt && (
                <div className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                  Last: {new Date(category.lastActivityAt).toLocaleDateString()}
                </div>
              )}
            </div>
...
function PostCard({
  ...
  return (
    <div className={`p-4 border border-gray-200 dark:border-gray-700 rounded-lg ${
      post.isSolution ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-700' : ''
    }`}>
