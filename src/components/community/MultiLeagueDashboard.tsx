...
import {
  Crown, Trophy, Users, TrendingUp, BarChart3, 
  Target, Calendar, Star, Award, Zap, Shield,
  ArrowUpRight, ArrowDownRight, RefreshCw,
  ChevronRight, Eye, Settings, Plus, Filter,
  Search, Bell, Clock, Activity, Grid3X3,
...
export default function MultiLeagueDashboard({ 
  userId, 
  className 
}: MultiLeagueDashboardProps) {
  const [activeView, setActiveView] = useState<'dashboard' | 'leagues' | 'players' | 'analytics'>('dashboard');
  ...
  const loadMultiLeagueData = async () => {
    try {
      setLoading(true);
  ...
  const getStatusColor = (status: League['status']) => {
    const colors = {
      ...
      draft_pending: 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300'
    };
    return colors[status];
  };

  const getTypeColor = (type: League['leagueType']) => {
    const colors = {
      redraft: 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300',
      dynasty: 'bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-300',
      keeper: 'bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-300'
    };
    return colors[type];
  };
        ...
        <div className="mt-6 border-b border-gray-200 dark:border-gray-700">
          <nav className="-mb-px flex space-x-8">
            {[
              ...
              { id: 'analytics', label: 'Cross-League Analytics', icon: TrendingUp }
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div key={league.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <div className="flex items-center space-x-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm ${
                    league.myTeam.rank === 1 ? 'bg-yellow-500' :
                ...
                <div className="text-right">
                  {league.myTeam.isInPlayoffs ? (
                    <span className="px-2 py-1 bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300 text-xs rounded-full font-medium">
                      Playoffs
                    </span>
        ...
        <div className="bg-white dark:bg-gray-700 rounded-lg p-6 border border-gray-200 dark:border-gray-600">
          ...
          <div className="space-y-4">
            {activeLeagues
              .filter(league => league.nextMatchup)
              .map((league) => (
...
function LeagueCard({ 
  ...
  return (
    <div className="bg-white dark:bg-gray-700 rounded-lg p-6 border border-gray-200 dark:border-gray-600 hover:shadow-lg transition-shadow">
...
function LeagueListItem({ 
  ...
  return (
    <div className="bg-white dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
...
function PlayersTab({ players }: { players: CrossLeaguePlayer[] }) {
  return (
    <div className="space-y-6">
      <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
              ...
              <tr key={player.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-center">
                  <span className={`text-sm font-medium ${
                    player.performance.consistency >= 85 ? 'text-green-600 dark:text-green-400' :
                    player.performance.consistency >= 70 ? 'text-yellow-600 dark:text-yellow-400' :
                ...
                <td className="px-6 py-4 whitespace-nowrap text-center">
                  <div className="flex items-center justify-center space-x-1">
                    ...
                    <span className={`text-sm font-medium ${
                      player.value.trend === 'up' ? 'text-green-600 dark:text-green-400' :
                      player.value.trend === 'down' ? 'text-red-600 dark:text-red-400' :
    ...
    <div className="space-y-8">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
              {leagues.map((league) => (
                <tr key={league.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <span className={`w-8 h-8 inline-flex items-center justify-center rounded-full text-white text-sm font-bold ${
                      league.myTeam.rank === 1 ? 'bg-yellow-500' :
                      league.myTeam.rank <= 3 ? 'bg-gray-400' :
                      league.myTeam.rank <= 6 ? 'bg-orange-500' : 'bg-red-500'
                    }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-gray-700 rounded-lg p-6 shadow">
          ...
          <div className="space-y-3">
            {['redraft', 'dynasty', 'keeper'].map((type) => {
              const typeLeagues = leagues.filter(l => l.leagueType === type);
              const avgPoints = typeLeagues.length > 0 
                ? typeLeagues.reduce((sum, l) => sum + l.myTeam.points, 0) / typeLeagues.length
                : 0;
