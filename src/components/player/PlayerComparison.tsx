...
export default function PlayerComparison() { const [players, setPlayers]  = useState<(Player | null)[]>([null: null, null, null]);
  ...
  player: Player) => { const newPlayers = [...players];
  ...
  const removePlayer = (index: number) => { const newPlayers = [...players];
    newPlayers[index] = null;
    setPlayers(newPlayers);
   }
  const addComparisonSlot = () => { if (activeSlots < 4) {
      setActiveSlots(activeSlots + 1);
     }
  }
  const getStatComparison = (players: (Player | null)[];
  stat: keyof Player | string) => { const validPlayers = players.filter(p => p !== null) as Player[];
            ...
            <div key={index} className="relative">
              { players[index] ? (
                ...
                <div className="relative">
                  <button
                    onClick={() => { const newShowSearch = [...showSearch];
                  ...
                  {showSearch[index] && (
                    <div className ="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 border dark; border-gray-600 rounded-lg shadow-lg z-10">
                      <input
                        ...
                        onChange={(e) => { const newQuery = [...searchQuery];
                              ...
                              <div key={idx} className="text-center">
                                {player ? (
                                  <span className={getComparisonColor(
                                    ...
                                    {metric.key.includes('Percentage') ? `${(player as unknown)[metric.key]}%`
  ...
  key: 'stats.gamesPlayed'  },
                      { label: 'Snap %',
  key: 'stats.snapPercentage' },
                      { label: 'Targets',
  key: 'stats.targets' },
                      { label: 'Touches',
  key: 'stats.touches' },
                      { label: 'RZ Targets',
  key: 'stats.redZoneTargets' },
                      { label: 'RZ Touches',
  ...
  key: 'splits.vsBottom10', icon, <Target className ="h-4 w-4" /> }
                      ...
                      return (
                        <div key={split.key } className="flex items-center">
                          ...
                          <div className={`grid grid-cols-${activeSlots} gap-4 flex-1`}>
                            {players.map((player, idx) => { const value = player? .splits?.[split.key.split('.')[1] as keyof typeof player.splits];
                              return (
                                <div key={idx } className="text-center">
                                  {player && value ? (
                                    <span className={getComparisonColor(value, comparison.max, comparison.min) }>
