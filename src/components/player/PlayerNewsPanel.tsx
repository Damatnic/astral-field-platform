...
import { 
  ...
  Twitter, Instagram, Play, Heart, MessageCircle,
  ThumbsUp, Share, Filter, Search, Zap
} from 'lucide-react';
...
export default function PlayerNewsPanel({ 
  ...
  const newsTypes = [
    ...
    { value: 'practice', label: 'Practice Reports', icon: Clock },
        ...
        {filteredNews.length === 0 ? (
          ...
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {filteredNews.map((item) => (
              <article
                key={item.id}
                className={`p-4 border-l-4 ${getPriorityColor(item.priority)} hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors`}
              >
                          ...
                          <div className="text-xs text-gray-600 dark:text-gray-400">
                            ...
                            {item.fantasyImpact.projectionChange && (
                              <span className={`ml-2 font-semibold ${
                                item.fantasyImpact.projectionChange > 0 ? 'text-green-600' : 'text-red-600'
                              }`}>
