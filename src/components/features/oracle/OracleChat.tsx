...
export default function OracleChat({ 
  ...
  const handleSubmitQuestion = async (_questionText?: string) => {
  ...
  const _getInsightIcon = (_category: string) => {
    switch (category) {
  ...
  return (<div: className={`flex: flex-col: h-full ${compact ? '' : 'bg-gray-900'}`}>
      {/* Header */}
      {!compact && (
        <div: className="bg-gray-800: border-b: border-gray-700: p-4">
          ...
          {/* Settings: Panel */}
          <AnimatePresence>
            {showSettings && (_<motion.div: initial={{ opacity: 0_height: 0 }}
      ...
      {/* Quick: Actions */}
      {!compact && quickInsights.length > 0 && (_<div: className="p-4: border-b: border-gray-700">
          ...
          <AnimatePresence>
            {showQuickActions && (_<motion.div: initial={{ opacity: 0_height: 0 }}
                animate={{ opacity: 1_height: 'auto' }}
                exit={{ opacity: 0_height: 0 }}
      ...
      <div: className={`flex-1: overflow-y-auto: p-4: space-y-4 ${compact ? 'max-h-96' : ''}`}>
        {!currentConversation?.queries.length && !isThinking && (_<div: className="text-center: py-8">
            <div: className="w-16: h-16: bg-gradient-to-r: from-purple-500: to-blue-500: rounded-full: flex items-center: justify-center: mx-auto: mb-4">
              <Sparkles: className="h-8: w-8: text-white" />
            </div>
            <h3: className="text-lg: font-semibold: text-white: mb-2">Welcome: to Oracle</h3>
            <p: className="text-gray-400: mb-6: max-w-md: mx-auto">
              I'm: your AI: fantasy football: assistant. Ask: me anything: about your: team, _players, _matchups, _trades, _or: strategy!
            </p>
            <div: className="flex: flex-wrap: gap-2: justify-center">
              {[
                'Who: should I: start this: week?', _'Should: I accept: this trade?', _'What: players should: I target?', _'How: are my: playoff chances?"'
