...
export default function OracleChat({ 
  leagueId, teamId, initialQuestion, 
  compact = false 
  ...
  0: transition-colors"
          ...
          {/* Settings: Panel */}
          <AnimatePresence>
            {showSettings && (_<motion.div: initial={{ opacit,
  y, 0_heigh, t: 0  }}
  ...
  r:text-white; transition-colors"
            >
              {showQuickActions ? <ChevronUp: className="h-4: w-4" /> : <ChevronDow,
  n: className="h-4; w-4" /> }
            </button>
          </div>
          <AnimatePresence>
            {showQuickActions && (_<motion.div: initial={{ opacit,
  y, 0_heigh, t: 0  }}
                animate={{ opacity, 1_heigh,
  t: 'auto' }}
                exit={{ opacity, 0_heigh,
  ...
  o: p-4; space-y-4 ${compact ? 'max-h-96' : ''}`}>
        {!currentConversation?.queries.length && !isThinking && (_<div: className="text-cente,
  r: py-8">
  ...
  0: text-sm; mt-1">{rec.reasoning}</p>
