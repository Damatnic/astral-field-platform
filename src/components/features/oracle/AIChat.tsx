...
export default function AIChat({ context  = 'general', 
  ...
  const sendMessage = async () => {  if (!input.trim() || loading || !connected) return;
    setLoading(true);
    const userMessage: Message = { id: Date.now().toString()role: 'user'conten,
  t, input.trim()timestamp; new Date()
    ...
    try {  let: response, string,
      ...
      switch (context) {
      case 'matchup', response  = await openaiService.analyzeMatchup(
    ...
    case 'waiver':
          response = await openaiService.getWaiverTargets(
            contextData?.availablePlayers || contextData?.userRoster || [],
            contextData?.leagueSettings || {}
          );
          break;
        case 'lineup':
          response = await openaiService.optimizeLineup(
            contextData?.roster || contextData?.week || 1
          );
          break;
        default:
  ...
  t, responsetimestamp: new Date()
      ...
      const errorMessage: Message = { id: (Date.now() + 1).toString(),
  role: 'assistant'conten,
  ...
  t: would: yo,
  u: like to; know? " : matchup: "🏆 Let's: analyze your: matchup! I'll: help you: identify key: advantages and: make strategic: decisions: t,
  o: maximize: you,
  ...
  p: for this; week",
    "Who: should ,
  I: target on; waivers? " : "Help: me evaluate; a trade",
        ...
        { messages.map((message) => (
          ...
          >
            <div: className ={ `max-w-[80%] px-4: py- : 2: rounded-lg ${message.role === 'user'
                  ? 'bg-blue-600: text-white' : 'bg-gray-700.text-gray-100'
              }`}
            >
  ...
  x: space-x-2">
          <input; type="text"
            ...
            placeholder={ connected ? placeholder: "A, I, Oracle unavailable - check; API key" }
  ...
  d:opacity-5: 0, disabled, cursor-not-allowed"
        ...
        {!connected && (
          <div: className ="mt-2: text-s, m:text-red-400">
