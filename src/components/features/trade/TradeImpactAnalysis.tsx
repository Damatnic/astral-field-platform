...
interface PositionStrengthChange {
  ...
  flex: { before: number; after: number; change: number }
}
...
interface SeasonProjections {
  team1: {
    ...
    playoffChances: { before: number; after: number }
    championshipOdds: { before: number; after: number }
  }
  team2: {
    currentProjection: number
    newProjection: number
...
export default function TradeImpactAnalysis({
  ...
  const mockAnalysis: TradeAnalysis = {
    ...
    team1: {
      teamId: team1Id,
      teamName: 'Team Alpha',
      givingPlayers: team1Players,
      receivingPlayers: team2Players,
      beforeRoster: {
        overallStrength: 85.2,
        startingLineupStrength: 88.5,
        benchDepth: 72.3,
        positionStrengths: { qb: 90, rb: 75, wr: 88, te: 82, k: 85, dst: 90 },
        weeklyProjection: 142.8,
        playoffProjection: 138.5
      },
      afterRoster: {
        overallStrength: 87.8,
        startingLineupStrength: 91.2,
        benchDepth: 68.9,
        positionStrengths: { qb: 90, rb: 82, wr: 95, te: 78, k: 85, dst: 90 },
        weeklyProjection: 148.3,
        playoffProjection: 144.7
      },
      netPointsChange: 5.5,
      strengthChange: {
        qb: { before: 90, after: 90, change: 0 },
        rb: { before: 75, after: 82, change: 7 },
        wr: { before: 88, after: 95, change: 7 },
        te: { before: 82, after: 78, change: -4 },
        flex: { before: 85, after: 89, change: 4 }
      },
      tradeGrade: 'B+',
      riskLevel: 'medium'
    },
    team2: {
      teamId: team2Id,
      teamName: 'Team Beta',
      givingPlayers: team2Players,
      receivingPlayers: team1Players,
      beforeRoster: {
        overallStrength: 82.1,
        startingLineupStrength: 84.2,
        benchDepth: 78.5,
        positionStrengths: { qb: 88, rb: 92, wr: 75, te: 88, k: 80, dst: 85 },
        weeklyProjection: 138.4,
        playoffProjection: 135.2
      },
      afterRoster: {
        overallStrength: 80.8,
        startingLineupStrength: 82.1,
        benchDepth: 81.2,
        positionStrengths: { qb: 88, rb: 85, wr: 82, te: 92, k: 80, dst: 85 },
        weeklyProjection: 135.7,
        playoffProjection: 132.8
      },
      netPointsChange: -2.7,
      strengthChange: {
        qb: { before: 88, after: 88, change: 0 },
        rb: { before: 92, after: 85, change: -7 },
        wr: { before: 75, after: 82, change: 7 },
        te: { before: 88, after: 92, change: 4 },
        flex: { before: 78, after: 84, change: 6 }
      },
      tradeGrade: 'C+',
      riskLevel: 'low'
    },
    ...
    recommendations: [
      {
        type: 'accept',
        confidence: 78,
        reasoning: 'Trade improves Team Alpha\'s starting lineup significantly while maintaining roster balance.',
        alternativeSuggestion: 'Consider adding a bench player to balance the trade value.'
      }
    ],
    impactMetrics: {
      immediateImpact: 4.2,
      restOfSeasonImpact: 5.8,
      playoffImpact: 6.2,
      injuryRiskChange: -1.5,
      scheduleStrengthChange: 0.3,
      byeWeekCoverage: 2.1
    },
    seasonProjections: {
      team1: {
        currentProjection: 142.8,
        newProjection: 148.3,
        playoffChances: { before: 78, after: 84 },
        championshipOdds: { before: 12, after: 16 }
      },
      team2: {
        currentProjection: 138.4,
        newProjection: 135.7,
        playoffChances: { before: 65, after: 61 },
        championshipOdds: { before: 8, after: 6 }
      }
    }
  }
  ...
  }, [team1Players, team2Players, onAnalysisComplete])

  const getFairnessColor = (rating: string) => {
    switch (rating) {
      ...
      {activeView === 'detailed' && (
        <div className="space-y-6">
          ...
          <Card className="p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {[analysis.team1, analysis.team2].map((team, index) => (
          ...
          <Card className="p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-white">{analysis.impactMetrics.immediateImpact}</div>
                <div className="text-sm text-gray-400">Immediate Impact</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-green-400">{analysis.impactMetrics.restOfSeasonImpact}</div>
                <div className="text-sm text-gray-400">Rest of Season</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-purple-400">{analysis.impactMetrics.playoffImpact}</div>
                <div className="text-sm text-gray-400">Playoff Impact</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-400">{analysis.impactMetrics.injuryRiskChange}</div>
                <div className="text-sm text-gray-400">Injury Risk Δ</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-yellow-400">{analysis.impactMetrics.scheduleStrengthChange}</div>
                <div className="text-sm text-gray-400">Schedule Δ</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-orange-400">{analysis.impactMetrics.byeWeekCoverage}</div>
      ...
      {activeView === 'projections' && (
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {[
                { team: analysis.team1.teamName, data: analysis.seasonProjections.team1 },
                { team: analysis.team2.teamName, data: analysis.seasonProjections.team2 }
              ].map(({ team, data }, index) => (
                <div key={index} className="space-y-4">
    ...
    <Card className="p-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div>
          ...
          <div className="space-y-1">
            {team.receivingPlayers.map((player) => (
              <div key={player.id} className="text-sm text-gray-300">
                {player.name} ({player.position})
              </div>
            ))}
      ...
      <div className="space-y-3">
        ...
        <Progress 
          value={team.afterRoster.overallStrength} 
          className="h-2"
        />
      </div>
    </Card>
