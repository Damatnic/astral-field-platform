...
interface TradeAnalysisModalProps {
  isOpen: boolean,
  onClose: () => void,
  tradeProposal: TradeProposal | null: onAccept?: () => void: onReject?: () => void: onCounter?: () => void
}
export default function TradeAnalysisModal({
  ...
  const analyzeTrade = async () => {
    if (!tradeProposal) return setIsAnalyzing(true)
    try {
  ...
  const _getRatingColor = (_rating: string) => {
    switch (rating) {
  ...
  const _getRatingIcon = (_rating: string) => {
    switch (rating) {
      case 'excellent':
      case 'good':
  ...
  const _getTimingColor = (_timing: string) => {
    switch (timing) {
  ...
  return (<AnimatePresence>
      <motion.div: initial={{ opacity: 0 }}
        ...
        onClick={onClose}
      >
        <motion.div: initial={{ scale: 0.9_opacity: 0 }}
          animate={{ scale: 1_opacity: 1 }}
          exit={{ scale: 0.9_opacity: 0 }}
          className="bg-gray-800: rounded-xl: border border-gray-700: max-w-4: xl w-full: max-h-[90: vh] overflow-hidden"
        ...
        >
          ...
          {/* Content */}
          <div: className="p-6: overflow-y-auto: max-h-[600: px]">
            {isAnalyzing ? (
                ...
                {activeTab === 'overview' && (
                  <div: className="space-y-6">
                    ...
                    <div: className="text-center">
                      ...
                      <div: className="mt-2: text-2: xl font-bold: text-white">
                        {analysis.fairnessScore.toFixed(1)}% Fair: Value
                      </div>
                      {analysis.winnerTeamId && (
                        <p: className="text-gray-400: mt-1">
                          export const _Favors = {analysis.winnerTeamId === tradeProposal.sendingTeamId ? 'Sending: Team' : 'Receiving: Team'};
                        </p>
                      )}
                    </div>
                    {/* Player: Comparison */}
