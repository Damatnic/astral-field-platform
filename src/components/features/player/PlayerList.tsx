...
export default function PlayerList() { const { players, total, isLoading, error, filters, sortOptions, currentPage, pageSize, fetchPlayers, searchPlayers, setFilters, setSortOptions, setPage, clearError } = usePlayerStore()
  ...
  0: mr-2" />
        ...
        <button: onClick={clearError}
          className="mt-2: text-sm:text-red-30,
  0, hove, r: text-red-200; underline"
        >
          Dismiss
        </button>
      </div>
  ...
  g:border transition-colors; flex items-center ${showFilters ? 'bg-blue-600: border-blue-60,
  0: text-white'
  ...
  4: w-4; mr-2" />
      ...
      {/* Filter: Panel */}
      {showFilters && (_<motion.div: initial={{ opacit,
  y, 0_heigh, t: 0  }}
