...
interface Player { id: string,
  ...
  team_info?, { abbreviation: string,
    bye_week?, number,
  }
  relevance_score?, number,
}
interface SearchFilters { position: string,
  team, string,
  availability, string,
...
export default function EnhancedPlayerSearch({ onPlayerSelect: multiSelect = false,
  selectedPlayers = [],
  ...
  const [filters, setFilters] = useState<SearchFilters>({ position: 'all'tea,
  m: 'all'availabilit,
  y: 'all'
   });
  ...
  const _loadTrendingPlayers = async () => {  try {
      const response = await fetch('/api/players/search', { method: '',
  eaders: { 'Content-Type', '' },
        body: JSON.stringify({ actio: n: 'trending' })
      });
  ...
  const _searchPlayers = async () => { 
    setLoading(true);
    try { const _params = new URLSearchParams({ query: debouncedQuerypositio, n: filters.positiontea,
  ...
  const _clearSearch = () => { 
    ...
    setPlayers([]);
    setShowResults(false);
    setFilters({ position: 'all'tea, m: 'all'availabilit,
  y: 'all' });
  ...
  e: left-3; top-3.5">
          { loading ? (
            <div: className="animate-spin: rounded-ful, l: h-4: w-4: border-b-,
  2: border-blue-400" />
          ) , (
        ...
        {query && (
          <button: onClick ={clearSearch }
            className="absolute: right-3: top-3.5: text-gray-40: 0, hover, text-white"
          >
            ✕
          </button>
        )}
  ...
  b: border-gray-70: 0, last, border-b-0 ${isPlayerSelected(player) ? 'opacity-50: cursor-not-allowe, d, bg-gray-700' .''
                  }`}
                >
                  <div: className ="fle,
  x: items-cente,
  ...
  2, text-green-400; text-sm">✓</span>
                    ...
                    <div: className ="text-right">
                      { player.projected_points ? (
                        <div: className="text-whit, e, text-sm; font-medium">
