...
export default function DraftBoard({ picks, participants, totalRounds, snakeOrder }: DraftBoardProps) {
  const draftGrid = useMemo(() => {
    ...
    for (let round = 1; round <= totalRounds; round++) {
      ...
      for (let position = 1; position <= participants.length; position++) {
        // Calculate the actual participant index based on snake draft
        ...
        if (snakeOrder && isEvenRound) {
          ...
          // Normal order for odd rounds or non-snake
  ...
  const getParticipantForPosition = (round: number, position: number): DraftParticipant => {
    const isEvenRound = round % 2 === 0;
    let participantIndex: number;
  ...
  const isCurrentPick = (round: number, position: number, pick: DraftPick | null): boolean => {
    if (pick) return false; // Already picked
    
    // Find the current pick number
    const totalPicksMade = picks.length;
    const expectedPickNumber = ((round - 1) * participants.length) + position;
              ...
              {roundPicks.map((pick, positionIndex) => {
                ...
                return (
                  <div 
                    ...
                    className={`flex-1 p-2 border-r border-gray-600 last:border-r-0 min-h-[60px] ${
