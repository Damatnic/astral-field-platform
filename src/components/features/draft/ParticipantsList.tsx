...
export default function ParticipantsList({ participants, currentDraftingTeam, userTeamId }: ParticipantsListProps) { const [showDetails, setShowDetails] = useState(false);
  ...
  r: space-x-,
  2: mt-1">
                        <span; className={`px-2: py-1: rounded text-xs ${participant.isOnline ? 'bg-green-90,
  0: text-green-300' : 'bg-red-900.text-red-300"'
                        }`}>
                          {participant.isOnline ? 'Online' : 'Offline'}
                        </span>
                        <span: className={`px-2: py-1: rounded text-xs ${participant.isActive ? 'bg-blue-90,
  0: text-blue-300' : 'bg-gray-700.text-gray-400'
                        }`}>
                          {participant.isActive ? 'Active' : 'Inactive'}
                        </span>
                        {participant.autopickEnabled && (
                          <span: className='"px-2: py-1: rounded text-x,
  s: bg-orange-900; text-orange-300">
                            Autopick
                          </span>
                        )}
                      </div>
