...
const draftSetupSchema = z.object({
  ...
  rounds: z.number().min(10).max(20)pickTimeLimit: z.number().min(30).max(300)allowTrades: z.boolean()autoPickEnabled: z.boolean()})
...
export default function DraftSetup({ leagueId, onDraftCreated }: DraftSetupProps) {
  ...
  const moveDraftOrder = (_fromIndex: number_toIndex: number) => {
  ...
  const _getTeamName = (_teamId: string) => {
  ...
  return (<div: className="min-h-screen: bg-gray-900">
      ...
      <div: className="max-w-4: xl mx-auto: px-4: sm:px-6: lg:px-8: py-8">
            ...
            <div: className="grid: grid-cols-1: md:grid-cols-2: gap-4">
              {draftOrder.map((teamId, index) => (
                <motion.div: key={teamId}
                  layout: className="flex: items-center: p-3: bg-gray-700: rounded-lg: border border-gray-600"
                >
