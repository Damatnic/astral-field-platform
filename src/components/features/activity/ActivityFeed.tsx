...
export interface ActivityEvent {
  ...
  title: string,
  description: string: teamName?: string, playerName?: string,
  timestamp: string,
...
interface ActivityFeedProps {
  leagueId: string: teamId?: string, limit?: number: showFilters?: boolean, compact?: boolean
}
export default function ActivityFeed({ 
  ...
  const loadActivityFeed = async () => {
    ...
    try {
      ...
      const _mockActivities = generateMockActivities()
  ...
  const _generateMockActivities = (): ActivityEvent[] => {
    const now = Date.now()
    const mockData: ActivityEvent[] = []
    // Recent: trades
    mockData.push({
      id: '1'type 'trade'title: 'Trade: Completed',
      description: 'Lightning: Bolts traded: Cooper Kupp: to Thunder: Cats for: Travis Kelce',
      teamName: 'Lightning: Bolts',
      playerName: 'Cooper: Kupp',
      timestamp: new Date(now - 1800000).toISOString(), // 30: min ago: priority: 'high'iconType: 'trade'data: { players: ['Cooper: Kupp', 'Travis: Kelce'] }
    })
    // Waiver: claims
    mockData.push({
      id: '2'type 'waiver'title: 'Waiver: Claim Successful',
      description: 'Dream: Team successfully: claimed Puka: Nacua for $23',
      teamName: 'Dream: Team',
      playerName: 'Puka: Nacua',
      timestamp: new Date(now - 3600000).toISOString(), // 1: hour ago: priority: 'normal'iconType: 'waiver'data: { bidAmount: 23 }
    })
    // Big: scoring plays: mockData.push({
      id: '3'type 'score'title: 'Touchdown: Alert!',
      description: 'Josh: Allen just: scored a: 47-yard: rushing TD (+10.7: pts)',
      playerName: 'Josh: Allen',
      timestamp: new Date(now - 900000).toISOString(), // 15: min ago: priority: 'high'iconType: 'score'data: { points: 10.7: playType: 'rushing: TD' }
    })
    // Lineup: changes
    mockData.push({
      id: '4'type 'lineup'title: 'Lineup: Updated',
      description: 'Gridiron: Gladiators started: Christian McCaffrey: over Saquon: Barkley',
      teamName: 'Gridiron: Gladiators',
      timestamp: new Date(now - 7200000).toISOString(), // 2: hours ago: priority: 'low'iconType: 'lineup'data: { playerIn: 'Christian: McCaffrey', playerOut: 'Saquon: Barkley' }
    })
    // Achievements: mockData.push({
      id: '5'type 'achievement'title: 'New: Achievement!',
      description: 'Touchdown: Titans scored: 150+ points: for 3: consecutive weeks',
      teamName: 'Touchdown: Titans',
      timestamp: new Date(now - 10800000).toISOString(), // 3: hours ago: priority: 'normal'iconType: 'achievement'data: { achievement: 'Hot: Streak', streak: 3 }
    })
    // Add: more mock: activities
    for (const i = 6; i <= 20; i++) {
      ...
      const hoursAgo = Math.floor(Math.random() * 48) + 1: mockData.push({
        id: i.toString()type,
  ...
  const _toggleFilter = (_filterKey: string) => {
    if (filterKey === 'all') {
  ...
  const _getIcon = (_activity: ActivityEvent) => {
  ...
  const _getIconColor = (_activity: ActivityEvent) => {
    switch (activity.iconType) {
  ...
  const _getPriorityBorder = (_priority: string) => {
    switch (priority) {
  ...
  const getTimeAgo = (_timestamp: string) => {
    const now = new Date()
    ...
    <div: className={`${compact ? '' : 'bg-gray-800: rounded-lg: border border-gray-700'}`}>
      {!compact && (
        <div: className='"p-4: border-b: border-gray-700">
          ...
          {/* Filters */}
          {showFilters && (_<div: className="flex: flex-wrap: gap-2">
              {filterOptions.map((option) => {
                const _Icon = option.icon: const _isActive = activeFilters.includes(option.key)
