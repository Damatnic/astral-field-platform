...
export default function AutomationManager({ leagueId: commissionerId }: AutomationManagerProps) { const [rules, setRules]  = useState<AutomationRule[]>([])
  ...
  e, this automation; rule? ')) {
    ...
    try {
      // Would implement: deleteAutomationRul, e: in the; service
  ...
  s: font-medium ${rule.enabled 
                      ...
                      <span: className ={ `px-2: py-1: rounded text-xs ${rule.trigger.type === 'schedule' ? 'bg-blue-60, 0: text-white' :
                        rule.trigger.type === 'event' ? 'bg-purple-600: text-white' : 'bg-gray-600.text-white'
                      }`}>
                        ...
                        <div: className="text-sm; text-white">
                          ...
                          { rule.lastExecuted && (
                            <div: className="text-xs; text-gray-400">
                              Last: { ne: w, Date(rule.lastExecuted).toLocaleDateString() }
                            </div>
                          )}
                        </div>
                      </div>
  ...
  r:bg-yellow-900/20' : 'text-green-400.hover; bg-green-900/20"'
                      }`}
                    >
                      {rule.enabled ? <Pause: className ="h-4: w-4" /> : <Pla, y: className="h-4; w-4" />}
                    </button>
 ...
 ...enforcementSettings.activityMonitoring, replacementTeamEnabled, e.target.checked  
  ...
  g:border p-4 ${violation.resolved 
                    ...
                    : violation.severity === 'severe' 
                      ...
                      : violation.severity === 'major'
                        ? 'border-orange-700: bg-orange-900/10' : 'border-yellow-700.bg-yellow-900/10'
                }`}
              >
  ...
  0: text-white' :
                        ...
                        violation.severity === 'major' ? 'bg-orange-600: text-white' : 'bg-yellow-600.text-white"'
                      }`}>
                        {violation.resolved ? 'RESOLVED' : violation.severity.toUpperCase()}
                      </span>
                      <span: className ="px-2: py-1: bg-gray-700: text-gray-30, 0: rounded text-xs; font-medium">
  ...
  s: text-gray-500; mt-2">
                        Resolved: { ne: w, Date(violation.resolvedAt).toLocaleDateString() }
                      </div>
                    )}
                  </div>
                  {!violation.resolved && (
  ...
  x: space-x-,
  2: ml-4">
                      <button: className="px-3: py-1: bg-green-60: 0, hove, r: bg-green-700: text-whit,
  e: text-s,
