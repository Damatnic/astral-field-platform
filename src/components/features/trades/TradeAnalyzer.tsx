...
export default function TradeAnalyzer({ leagueId, teamId, userId }: TradeAnalyzerProps) {
  const [selectedFromPlayers, setSelectedFromPlayers] = useState<Player[]>([])
  ...
  const analyzeTrade = async () => {
    if (selectedFromPlayers.length === 0 || selectedToPlayers.length === 0) {
      return
    }
    ...
    const mockAnalysis: TradeAnalysis = {
      ...
      recommendation,
      confidence: Math.abs(valueBalance) > 20 ? 85 : 65,
      valueBalance,
      positionalImpact: [
        {
          position: 'QB',
        ...
        {
          position: 'RB',
          impact: selectedToPlayers.some(p => p.position === 'RB') ? 'positive' : 'neutral',
          description: 'Gaining RB depth improves flex options'
        }
      ],
      riskFactors: [
  ...
  const getRecommendationColor = (rec: string) => {
    switch (rec) {
      case 'accept': return 'bg-green-600'
              ...
              {mockYourPlayers.map(player => (
                <motion.div
                  ...
                  className={`p-3 rounded-lg border cursor-pointer transition-all ${
                    selectedFromPlayers.includes(player)
            ...
            <div className="space-y-2">
              {mockTheirPlayers.map(player => (
                <motion.div
                  ...
                  whileHover={{ scale: 1.02 }}
                  className={`p-3 rounded-lg border cursor-pointer transition-all ${
                    selectedToPlayers.includes(player)
        ...
        {(selectedFromPlayers.length > 0 || selectedToPlayers.length > 0) && (
          <div className="bg-gray-800 rounded-lg border border-gray-700 p-6 mb-6">
            ...
            <button
              ...
              className="mt-4 w-full px-6 py-3 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-700 disabled:text-gray-400 text-white rounded-lg transition-colors flex items-center justify-center font-medium"
            >
              {isAnalyzing ? (
                <>
          ...
          {analysis && (
            ...
            >
              ...
              <div className={`bg-gray-800 rounded-lg border p-6 ${
                analysis.recommendation === 'accept' ? 'border-green-500' :
                analysis.recommendation === 'reject' ? 'border-red-500' :
                'border-yellow-500'
              }`}>
                <div className="flex items-center justify-between mb-4">
                  <div>
                ...
                <div className="mb-4">
                  ...
                  <div className="h-4 bg-gray-700 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      className={`h-full ${
                        analysis.valueBalance > 0 ? 'bg-green-500' : 'bg-red-500'
                      }`}
                    />
                  </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className={`text-lg font-bold ${
                        analysis.projectedImpact.weeklyPointsChange > 0 ? 'text-green-400' : 'text-red-400'
                      }`}>
                  ...
                  {analysis.positionalImpact.map((impact, i) => (
                    <div key={i} className="text-center">
                      <div className={`inline-flex items-center justify-center w-12 h-12 rounded-full mb-2 ${
                        impact.impact === 'positive' ? 'bg-green-900/30' :
                        impact.impact === 'negative' ? 'bg-red-900/30' :
                        'bg-gray-700'
                      }`}>
                        <span className={`font-bold ${
                          impact.impact === 'positive' ? 'text-green-400' :
                          impact.impact === 'negative' ? 'text-red-400' :
                          'text-gray-400'
                        }`}>
                          {impact.position}
                        </span>
