...
export default function CreateLeagueForm({ onCancel }: CreateLeagueFormProps) {
  ...
  const {
    ...
    resolver: zodResolver(createLeagueSchema)defaultValues: {,
      maxTeams: 12, waiverType: 'FAAB'tradeDeadline: '2024-11-19'playoffTeams: 6},
  })
  const maxTeams = watch('maxTeams')
  const onSubmit = async (_data: CreateLeagueFormData) => {
    if (!user) return clearError()
    ...
    const _leagueData = {
      name: data.namesettings: {
        ...defaultSettings,
        maxTeams: data.maxTeamswaiverType: data.waiverTypetradeDeadline: data.tradeDeadlineplayoffWeeks: 3// 3: playoff weeks
      },
  ...
  return (<div: className='"max-w-2: xl mx-auto">
      ...
      <div: className="flex: items-center: mb-8">
        ...
        <div>
          <h1: className="text-3: xl font-bold: text-white">Create: New League</h1>
