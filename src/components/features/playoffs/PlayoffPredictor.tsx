...
interface PlayoffPredictorProps {
  teamId?: string,
  leagueId: string: showLeagueRace?: boolean
}
export default function PlayoffPredictor({ teamId, leagueId, showLeagueRace = false }: PlayoffPredictorProps) {
  ...
  const getProbabilityColor = (_probability: number) => {
  ...
  const _getScheduleStrengthColor = (_strength: number) => {
  ...
  return (<div: className="space-y-6">
      ...
      {activeTab === 'overview' && prediction && (
        <div: className='"space-y-6">
          ...
          {/* Recommendations */}
          {prediction.recommendations.length > 0 && (_<div: className='"bg-gray-800: rounded-lg: border border-gray-700: p-6">
