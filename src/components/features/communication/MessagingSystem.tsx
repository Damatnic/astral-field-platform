...
export default function MessagingSystem({ leagueId, teamId }: MessagingSystemProps) {
  ...
  const loadChannels = async () => {
    // Mock channels - in real app, this would come from API
    const mockChannels: Channel[] = [
      {
        id: 'general',
        name: 'General',
        type: 'general',
        description: 'General league discussion',
        isPrivate: false,
      ...
      },
      {
        ...
        name: 'Trade Talk',
        type: 'trade',
        description: 'Discuss trades and player evaluations',
        isPrivate: false,
        participants: teams.map(t => t.id),
        unreadCount: 2,
        muted: false
      },
      {
        ...
        name: 'Waiver Wire',
        type: 'waiver',
        description: 'Waiver claims and free agency',
        isPrivate: false,
        participants: teams.map(t => t.id),
        unreadCount: 0,
        muted: false
      },
      {
        ...
        name: 'Announcements',
        type: 'announcement',
        description: 'Commissioner announcements',
        isPrivate: false,
        participants: teams.map(t => t.id),
        unreadCount: 1,
        muted: false
  ...
  const loadMessages = async (channelId: string) => {
    ...
    const mockMessages: Message[] = [
      ...
      {
        id: '2',
        content: 'Looking to trade my RB2 for a reliable WR. Anyone interested?',
        authorId: 'team1',
      ...
      {
        id: '3',
        content: 'I might be interested depending on who you\'re offering',
        authorId: 'team2',
        authorName: 'Team Beta',
      ...
      {
        id: '4',
        content: 'Good luck everyone this week!',
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {currentChannelMessages.filter(msg => msg.pinned).length > 0 && (
                <div className="bg-yellow-900/20 border border-yellow-500/30 rounded-lg p-3 mb-4">
                  ...
                  {currentChannelMessages.filter(msg => msg.pinned).map((message) => (
                ...
                <div className="flex items-center justify-between bg-gray-700 rounded p-2">
                  <div className="flex items-center">
                    ...
                    <span className="text-sm text-gray-300">
                      Replying to <strong>{replyingTo.authorName}</strong>: {replyingTo.content.slice(0, 50)}...
                    </span>
                  </div>
                  <button
...
function MessageComponent({ 
  ...
  return (
    <div
      className={`group relative ${message.pinned ? 'bg-yellow-900/10 border-l-4 border-yellow-500 pl-4' : ''}`}
    ...
    >
      ...
      {replyToMessage && (
        <div className="ml-12 mb-1 text-xs text-gray-400 border-l-2 border-gray-600 pl-2">
          <span className="font-medium">{replyToMessage.authorName}</span>: {replyToMessage.content.slice(0, 50)}...
        </div>
      )}
