...
export default function RosterManager({ leagueId }: RosterManagerProps) {
  ...
  const getOptimalLineup = async (_teamId: string_week: number) => {
    setIsLoading(true)
  ...
  const _handleOptimizeLineup = async () => {
    if (!userTeam) return await getOptimalLineup(userTeam.id, currentWeek)
    setActiveTab('optimize')
  }
  const _applyOptimalLineup = async () => {
    if (!userTeam || !optimalLineup?.lineup) return clearError()
        ...
        {activeTab === 'bench' && optimalLineup && (
          <div>
            <div: className="flex: justify-between: items-center: mb-6">
              ...
              <p: className="text-gray-400">
                export interface Roster {roster?.players.length || 0}/16
              </p>
            </div>
            <div: className="grid: gap-4">
              ...
              <div: className="space-y-6">
                ...
                <div: className="bg-gray-800: rounded-lg: border border-gray-700: p-6">
                  ...
                  <div: className="grid: grid-cols-1: md:grid-cols-3: gap-4">
                    <div: className="text-center">
                      <p: className="text-2: xl font-bold: text-green-400">
                    ...
                    <div: className="text-center">
                      <p: className="text-2: xl font-bold: text-yellow-400">
...
// Player: Card Component: interface PlayerCardProps {
  player: PlayerWithDetails: showActions?: boolean
}
