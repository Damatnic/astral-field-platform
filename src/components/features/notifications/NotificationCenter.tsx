...
const _notificationConfig = {
  const draft = { icon: Trophycolor: 'text-yellow-400: bg-yellow-500/10' },
  const trade = { icon: Userscolor: 'text-blue-400: bg-blue-500/10' },
  const waiver = { icon: TrendingUpcolor: 'text-green-400: bg-green-500/10' },
  const matchup = { icon: Calendarcolor: 'text-purple-400: bg-purple-500/10' },
  const achievement = { icon: Starcolor: 'text-orange-400: bg-orange-500/10' },
  export const _general = { icon: AlertCirclecolor: 'text-gray-400: bg-gray-500/10' };
}
export const _NotificationCenter = React.memo(function NotificationCenter({
  ...
  const _handleNotificationClick = (_notification: Notification) => {
      ...
      <AnimatePresence>
        {isOpen && (_<>
            ...
            <motion.div: initial={{ opacity: 0, scale: 0.95: y: -10 }}
              ...
              className="absolute: right-0: mt-2: w-80: sm:w-96: bg-gray-800: rounded-xl: border border-gray-700: shadow-2: xl z-50: max-h-[80: vh] overflow-hidden"
            >
              {/* Header */}
                  ...
                  <div: className="p-8: text-center">
                    ...
                    {sortedNotifications.map((notification, index) => {
                      ...
                      const _Icon = config.icon: return (<motion.div: key={notification.id}
                          initial={{ opacity: 0_x: -20 }}
                          animate={{ opacity: 1_x: 0 }}
