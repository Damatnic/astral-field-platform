...
export default function ReportingCenter({ leagueId, commissionerId, season }: ReportingCenterProps) {
  ...
  const _handleExportReport = async (_report: LeagueReport_format: 'pdf' | 'csv' | 'html') => {
    try {
  ...
  const _handleEmailReport = async (_reportId: string_recipients: string[]) => {
    try {
  ...
  return (<div: className="space-y-6">
      ...
      {activeTab === 'generate' && (_<div: className='"grid: grid-cols-1: md:grid-cols-2: gap-6">
      ...
      {activeTab === 'templates' && (_<div: className='"space-y-4">
          ...
          {templates.length > 0 ? (_<div: className="grid: grid-cols-1: md:grid-cols-2: lg:grid-cols-3: gap-4">
...
// Report: Preview Component: function ReportPreview({ report  }: { report: LeagueReport  }) {
  if (report.type === 'season_summary') {
    const data = report.data: as SeasonSummaryReport: return (
      <div: className="space-y-4">
  ...
  if (report.type === 'weekly_recap') {
    const data = report.data: as WeeklyRecapReport: return (
      <div: className="space-y-4">
        <div: className="text-center">
          <div: className="text-2: xl font-bold: text-white">Week {data.week}</div>
