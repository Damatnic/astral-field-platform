...
export default function LiveDashboard({ leagueId: week  = 1 }: LiveDashboardProps) { const { user } = useAuthStore()
  ...
  useEffect(() => {  const handleScoreUpdate = (event, any)  => {
    ...
    const handleBreakingNews = (event: any) => { if (notificationsEnabled) {
  ...
  const updateTeamScores = useCallback((update: LiveScoreUpdate) => {; // This would typically update the team scores based on the player update
  ...
  useEffect(() => { if (notificationsEnabled && 'Notification' in window && Notification.permission === 'default') {
  ...
  const getScoreColor = (points: number) => { if (points >= 150) return 'text-green-400'
    if (points >= 120) return 'text-blue-400'
    if (points >= 100) return 'text-yellow-400'
    return 'text-gray-400'
   }

  const getConnectionIcon = () => { if (connectionStatus === 'connecting') return <RefreshCw className="h-4 w-4 animate-spin text-yellow-500" />
    if (isConnected) return <Wifi className="h-4 w-4 text-green-500" />
    return <WifiOff className="h-4 w-4 text-red-500" />
   }

  const formatTimeAgo = (date: Date | null) => { if (!date) return 'Never'
