...
interface SmartLineupSuggestionsProps {
  ...
  bench: Player[]
  currentWeek: number
  onPlayerSwap?: (fromSlot: string, toPlayer: Player) => void
  className?: string
}
...
export function SmartLineupSuggestions({
  ...
  // Generate smart suggestions based on player data
  const suggestions = useMemo(() => {
    const allSuggestions: SmartSuggestion[] = []

    // Check each lineup slot
    lineup.forEach((slot, slotIndex) => {
      ...
      if (player.injuryStatus && player.injuryStatus !== 'healthy') {
        ...
        allSuggestions.push({
          ...
          impact: replacement ? replacement.projectedPoints - player.projectedPoints : 0,
          confidence: player.injuryStatus === 'out' ? 95 : 
                     player.injuryStatus === 'doubtful' ? 70 : 50,
          action: player.injuryStatus === 'out' ? 'replace' : 'consider'
        })
      }
      ...
      // Matchup difficulty warnings
      if (player.matchupDifficulty === 'hard' && player.vsPositionRank && player.vsPositionRank > 25) {
        const replacement = findBestReplacement(player, bench, slot.position)
        
        if (replacement && replacement.projectedPoints > player.projectedPoints + 2) {
          allSuggestions.push({
            id: `matchup-${player.id}`,
            type: 'matchup',
            severity: 'medium',
            title: `${player.name} - Tough Matchup`,
            description: `${player.name} faces a tough defense (${player.vsPositionRank}th vs ${player.position})`,
            player,
            suggestedReplacement: replacement,
            impact: replacement.projectedPoints - player.projectedPoints,
            confidence: 60,
            action: 'consider'
          })
        }
      ...
      // Poor recent form
      if (player.recentForm && player.recentForm < 8) {
        const replacement = findBestReplacement(player, bench, slot.position)
  ...
  const findBestReplacement = (player: Player, availablePlayers: Player[], position: string): Player | undefined => {
...
interface SuggestionCardProps {
  ...
  onApply?: (fromSlot: string, toPlayer: Player) => void
}
...
function SuggestionCard({ suggestion, index, onDismiss, onApply }: SuggestionCardProps) {
  ...
  const icon = getSeverityIcon(suggestion.type, suggestion.severity)
    ...
    >
      ...
      {suggestion.suggestedReplacement && (
        <div className="bg-black/20 rounded-lg p-3 space-y-2">
          ...
          {/* Action Button */}
          {onApply && suggestion.action === 'replace' && (
            <button
              onClick={() => onApply(suggestion.player.id, suggestion.suggestedReplacement!)}
