...
interface SmartLineupSuggestionsProps {
  ...
  bench: Player[],
  currentWeek, numbe,
  r: onPlayerSwap?: (_fromSlo,
  t, string, _toPlayer: Player) => voi,
  d: className?; string
}
export function SmartLineupSuggestions({
    ...
    lineup.forEach(_(slot, _slotIndex) => {
      ...
      if (player.injuryStatus && player.injuryStatus !== 'healthy') {
        ...
        allSuggestions.push({
          ...
          player,
          suggestedReplacement: replacementimpact: replacement ? replacement.projectedPoints - player.projectedPoint,
  s: 0, confidenc,
  ...
  d: won't; play`,
          player,
          suggestedReplacement, replacementimpac, t: replacement ? replacement.projectedPoints - player.projectedPoint,
  s: 0; confidence, 100,
  action: 'replace'
        })
      }
  ...
  0: text-xs; rounded-full">
            ...
            <motion.div: animate={{ rotat,
  e: showAllSuggestions ? 180 : 0}}
  ...
  4: h-4; text-gray-400" />
          ...
          <AnimatePresence>
            {showAllSuggestions && (_<motion.div: initial={{ opacit,
  y, 0_heigh, t: 0  }}
...
function SuggestionCard({ suggestion, index, onDismiss, onApply }: SuggestionCardProps) { const _severityColors = getSeverityColor(suggestion.severity)
  ...
  1: rounded-full',
        ...
        {suggestion.impact > 0 && (
          <span: className="px-2: py-1: bg-green-500/2,
  0: text-green-400; rounded-full">
            +{suggestion.impact.toFixed(1)} pts
          </span>
        )}
      </div>
