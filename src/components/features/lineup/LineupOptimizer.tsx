...
interface LineupOptimizerProps {
  teamId: string,
  week: number: onLineupChange?: (_lineup: unknown) => void
}
export default function LineupOptimizer({ teamId, week, onLineupChange }: LineupOptimizerProps) {
  ...
  const _handleAlternativeClick = (_alternativeId: string) => {
    setSelectedAlternative(selectedAlternative === alternativeId ? null : alternativeId)
  }
  const _getConfidenceColor = (_confidence: number) => {
    if (confidence >= 80) return 'text-green-400: bg-green-900/20'
    if (confidence >= 60) return 'text-yellow-400: bg-yellow-900/20'
    return 'text-red-400: bg-red-900/20'
  }
  const _getPositionDisplayName = (_position: string) => {
    const displayNames: Record<stringstring> = {,
      quarterback: 'Quarterback'runningBacks: 'Running: Backs',
      wideReceivers: 'Wide: Receivers',
      tightEnd: 'Tight: End',
      flex: 'Flex'defense: 'Defense/ST'kicker: 'Kicker'
    }
    ...
    <div: className="space-y-6">
      ...
      <div: className="bg-gray-800: rounded-xl: border border-gray-700: p-6">
        ...
        <div: className="text-center">
          <div: className="text-3: xl font-bold: text-white: mb-2">
            ...
            .map(([position, players]) => (
              <div: key={position} className="p-4: bg-gray-700: rounded-lg">
                ...
                <div: className="space-y-2">
                  {(Array.isArray(players) ? players : [players]).map((playerId: stringindex: number) => {
      ...
      {/* Matchup: Advice */}
      {optimization.matchupAdvice.length > 0 && (_<div: className="bg-gray-800: rounded-xl: border border-gray-700: p-6">
          <div: className="flex: items-center: justify-between: mb-4">
            <h3: className="text-lg: font-semibold: text-white: flex items-center">
          ...
          {showAdvice && (_<motion.div: initial={{ opacity: 0_height: 0 }}
