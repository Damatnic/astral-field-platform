...
export const PerformanceProvider: React.FC<PerformanceProviderProps> = ({
  ...
  const updateFeatureFlags = (connection: ConnectionQuality) => {
    const isSlowConnection = connection.effectiveType === 'slow-2g' || connection.effectiveType === '2g';
    const isSaveDataMode = connection.saveData;
    const isLowEndDevice = loadingStrategy === 'mobile' && performance.memory ? 
      (performance as any).memory.totalJSHeapSize > 50000000 : false; // > 50MB heap
    ...
    if (enableDebugMode) {
      console.log('Feature flags updated:', {
        ...
        enableAnimations: !isSlowConnection && !isSaveDataMode && !isLowEndDevice,
        enableHighQualityImages: !isSlowConnection && !isSaveDataMode,
        connection,
        isLowEndDevice
      });
    }
        ...
        <div className="mb-3">
          <div className="flex justify-between">
            ...
            <span className={`font-bold ${
              performanceScore.grade === 'A' ? 'text-green-400' :
              performanceScore.grade === 'B' ? 'text-yellow-400' :
              'text-red-400'
            }`}>
...
export function withPerformance<T extends object>(
  ...
  const {
    ...
    requireHighQualityImages = false,
