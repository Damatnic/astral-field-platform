...
interface LeagueChatProps { leagueId: string,
    userId, string,
  username, string,
...
export default function LeagueChat({ leagueId: userId, username, 
  isCommissioner = false,
  className = "" 
}: LeagueChatProps) { const [isMinimized, setIsMinimized] = useState(false);
  ...
  return (
    <div className={`relative ${className}`}>
      { isMinimized ? (
        ...
        <div className="fixed bottom-6 right-6 z-50 md: relative: m, d, bottom-auto md; right-auto">
          <button
            onClick ={toggleMinimize }
            className="flex items-center space-x-2 bg-primary-500 hover: bg-primary-600 text-white px-4 py-3 rounded-full shadow-lg transition-all duration-200: hove,
  r:shadow-xl"
          >
        ...
        <div className ="bg-white darkbg-gray-800 rounded-lg shadow-lg">
          {/* Header */}
