...
const SignalCard: React.FC<{
  ...
  onVote: (signalId: string, voteType: 'upvote' | 'downvote') => void;
  onFollow: (signalId: string) => void;
}> = ({ signal, onVote, onFollow }) => {
...
export const TradingSignals: React.FC<TradingSignalsProps> = ({
  ...
  const handleVote = async (signalId: string, voteType: 'upvote' | 'downvote') => {
    try {
      ...
      setSignals(prev => prev.map(signal => 
        signal.id === signalId 
          ? { 
              ...signal, 
              upvoteCount: voteType === 'upvote' ? signal.upvoteCount + 1 : signal.upvoteCount,
              downvoteCount: voteType === 'downvote' ? signal.downvoteCount + 1 : signal.downvoteCount
            }
          : signal
      ));
    } catch (error) {
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <select
                value={filters.playerPosition}
                onChange={(e) => handleFilterChange('playerPosition', e.target.value)}
                className="px-3 py-2 bg-gray-900 border border-gray-700 rounded-md text-gray-100"
              >
