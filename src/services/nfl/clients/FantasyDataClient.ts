...
export class FantasyDataClient extends BaseAPIClient {
  constructor(apiKey: string) {
    const config: APIClientConfig = {
      name: 'FantasyData',
      baseURL: 'https://api.fantasydata.net/v3/nfl',
      apiKey,
      timeout: 12000,
      retryAttempts: 3,
      retryDelay: 1200,
   ...
   */
  async getSeasonProjections(season: number = 2025): Promise<any[]> {
    const projections = await this.makeRequest<FantasyProjection[]>(
   ...
   */
  async getFantasyRankings(week: number, season: number = 2025): Promise<Array<{
    playerId: string;
    playerName: string;
    position: string;
    team: string;
   ...
   */
  async getOwnershipData(week: number, season: number = 2025): Promise<Array<{
    playerId: string;
    playerName: string;
    position: string;
    team: string;
   ...
   */
  async getDefenseProjections(week: number, season: number = 2025): Promise<Array<{
    teamId: string;
    team: string;
    projectedPoints: number;
    projectedStats: {
   ...
   */
  async getDFSData(week: number, season: number = 2025): Promise<Array<{
    playerId: string;
    playerName: string;
    position: string;
    team: string;
    ...
    return projections.map(proj => {
      ...
      return {
        ...
        value: {
          draftKings: dfs.draftKingsSalary > 0 ? dfs.draftKingsPoints / (dfs.draftKingsSalary / 1000) : 0,
   ...
   */
  async getAdvancedMetrics(week: number, season: number = 2025): Promise<Array<{
    playerId: string;
    playerName: string;
    position: string;
    team: string;
   ...
   */
  async getWaiverRecommendations(week: number, season: number = 2025): Promise<Array<{
    playerId: string;
    playerName: string;
    position: string;
    team: string;
   ...
   */
  async getTradeAnalysis(playerIds: string[]): Promise<{
    players: Array<{
      playerId: string;
      playerName: string;
      position: string;
    ...
    const players = playerIds.map(playerId => {
      const proj = projections.find(p => p.playerId === playerId);
      return {
        playerId,
  ...
  // Helper methods
  private generateWaiverReason(projection: any): string {
    if (projection.projectedPoints > 15) {
      return 'High projected points, immediate starter potential';
    } else if (projection.projectedPoints > 10) {
      return 'Good bench depth with flex appeal';
    } else {
      return 'Deep league or injury replacement option';
    }
  }

  private calculateFAABBid(projectedPoints: number, ownership: number): number {
    let baseBid = Math.round(projectedPoints * 2);
