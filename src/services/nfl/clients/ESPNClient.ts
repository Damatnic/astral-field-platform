...
export class ESPNClient extends BaseAPIClient {
  ...
  async getCurrentWeek(): Promise<number> {
    const scoreboard = await this.makeRequest<ESPNScoreboardResponse>('/scoreboard');
  ...
  async getTeams(): Promise<Array<{
    ...
    const response = await this.makeRequest<{ sports: Array<{ leagues: Array<{ teams: any[] }> }> }>('/teams');
   ...
   * Get standings
   */
  async getStandings(season: number = 2025): Promise<Array<{
    team: string;
    teamId: string;
    wins: number;
    losses: number;
   ...
   */
  async getPlayerStats(playerId: string, season: number = 2025): Promise<any> {
    try {
   ...
   */
  async getNews(limit: number = 10): Promise<Array<{
    id: string;
    headline: string;
    description: string;
    published: Date;
   ...
   */
  async getWeatherByWeek(week: number, season: number = 2025): Promise<Array<{
    gameId: string;
    temperature: number;
    conditions: string;
    details: string;
  ...
  // Transform methods
  private transformGame(event: ESPNEvent): NFLGame {
    const competition = event.competitions[0];
  ...
  private transformPlayer(athlete: any): NFLPlayer {
