...
export class ClientManager extends EventEmitter {
  ...
  private loadBalancingStrategy: LoadBalancingStrategy = { type: 'priority' };
  ...
  private async initializeClients(config: ClientConfig): Promise<void> {
    // Initialize SportsIO client
    if (config.sportsIO?.enabled && config.sportsIO.apiKey) {
      const client = new SportsIOClient(config.sportsIO.apiKey);
    ...
    if (config.nflOfficial?.enabled) {
      const client = new NFLOfficialClient(config.nflOfficial.apiKey);
    ...
    if (config.fantasyData?.enabled && config.fantasyData.apiKey) {
      const client = new FantasyDataClient(config.fantasyData.apiKey);
  ...
  private setupClientEventListeners(clientName: string, client: any): void {
  ...
  async executeRequest<T>(
    operation: string,
    requestFunction: (client: any) => Promise<T>,
    options: {
  ...
  async getCurrentWeek(): Promise<number> {
    return this.executeRequest(
      ...
      { preferredClient: 'sportsIO', fallbackClients: ['espn'] }
    );
  }
   ...
   */
  async getGamesByWeek(week: number, season: number = 2025): Promise<NFLGame[]> {
    return this.executeRequest(
      'getGamesByWeek',
      async (client: any) => {
  ...
  async getLiveGames(): Promise<NFLGame[]> {
    return this.executeRequest(
      ...
      { preferredClient: 'nflOfficial', fallbackClients: ['sportsIO', 'espn'] }
    );
  }
   ...
   */
  async getPlayerStats(playerId: string, week: number, season: number = 2025): Promise<PlayerStats | null> {
    return this.executeRequest(
      'getPlayerStats',
      async (client: any) => {
        if (client.getPlayerStatsByPlayerAndWeek) {
          return await client.getPlayerStatsByPlayerAndWeek(playerId, week, season);
        } else if (client.getPlayerStats) {
          const stats = await client.getPlayerStats(playerId, season, week);
          return stats[0] || null;
        }
   ...
   */
  async getFantasyProjections(week: number, season: number = 2025): Promise<any[]> {
    return this.executeRequest(
  ...
  async getInjuryReports(): Promise<any[]> {
    ...
    for (const clientName of ['sportsIO', 'espn', 'nflOfficial']) {
      try {
        const client = this.clients.get(clientName);
   ...
   */
  async getClientsHealth(): Promise<ClientHealth[]> {
  ...
  private getExecutionOrder(preferredClient?: string, fallbackClients?: string[]): string[] {
    const order: string[] = [];
  ...
  private isClientHealthy(clientName: string): boolean {
    const client = this.clients.get(clientName);
  ...
  private recordSuccess(clientName: string, responseTime: number): void {
  ...
  private recordFailure(clientName: string, error: string): void {
    this.metrics.totalRequests++;
  ...
  private getAverageResponseTime(clientName: string): number {
    const times = this.responseTimes.get(clientName) || [];
    return times.length > 0 ? times.reduce((sum, time) => sum + time, 0) / times.length : 0;
  }

  private getRateLimitStatus(clientName: string): string {
    const client = this.clients.get(clientName);
    ...
    try {
      ...
      if (rateLimiter) {
        const minuteUsage = rateLimiter.requestsThisMinute / 100; // Assuming 100 rpm limit
  ...
  private incrementConnectionCount(clientName: string): void {
    const current = this.connectionCounts.get(clientName) || 0;
    this.connectionCounts.set(clientName, current + 1);
  }

  private decrementConnectionCount(clientName: string): void {
    const current = this.connectionCounts.get(clientName) || 0;
    this.connectionCounts.set(clientName, Math.max(0, current - 1));
  }

  private updateClientMetrics(clientName: string, metrics: any): void {
    // Update health score based on client metrics
    const clientsCount = this.clients.size;
    let totalHealthScore = 0;
    
    for (const name of this.clients.keys()) {
      const isHealthy = this.isClientHealthy(name);
  ...
  private mergeInjuryReports(reports: any[][]): any[] {
