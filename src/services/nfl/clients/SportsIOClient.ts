...
export class SportsIOClient extends BaseAPIClient {
  ...
  private setupAuthentication(): void {
    ...
    this.makeRequest = async <T>(endpoint: string, options: RequestOptions = {}): Promise<T> => {
  ...
  async getCurrentWeek(): Promise<number> {
    const response = await this.makeRequest<number>('/scores/json/CurrentWeek');
    return response;
  }
  ...
  async getGamesByWeek(week: number, season: number = 2025): Promise<NFLGame[]> {
    const games = await this.makeRequest<SportsIOGame[]>(
  ...
  async getLiveGames(): Promise<NFLGame[]> {
    const games = await this.makeRequest<SportsIOGame[]>('/scores/json/AreAnyGamesInProgress');
    return games.map(this.transformGame);
  }
   ...
   */
  async getPlayerStatsByWeek(week: number, season: number = 2025): Promise<PlayerStats[]> {
   ...
   */
  async getPlayerStatsByPlayerAndWeek(
    playerId: string, 
    week: number, 
    season: number = 2025
   ...
   */
  async getActivePlayers(): Promise<NFLPlayer[]> {
   ...
   * Get players by team
   */
  async getPlayersByTeam(teamAbbreviation: string): Promise<NFLPlayer[]> {
    const players = await this.makeRequest<SportsIOPlayer[]>(`/scores/json/Players/${teamAbbreviation}`);
   ...
   */
  async getFantasyProjections(week: number, season: number = 2025): Promise<Array<{
    playerId: string;
    playerName: string;
    position: string;
    team: string;
   ...
   * Get team standings
   */
  async getTeamStandings(season: number = 2025): Promise<Array<{
    team: string;
    conference: string;
    division: string;
    wins: number;
   ...
   */
  async getWeatherByGame(gameId: string): Promise<WeatherData | null> {
    try {
   ...
   */
  async getRealTimeBoxScores(): Promise<any[]> {
    return this.makeRequest<any[]>('/stats/json/BoxScores');
  }

  // Transform methods
  private transformGame(game: SportsIOGame): NFLGame {
    return {
  ...
  private transformPlayer(player: SportsIOPlayer): NFLPlayer {
    return {
  ...
  private transformPlayerStats(stats: SportsIOPlayerStats): PlayerStats {
    return {
