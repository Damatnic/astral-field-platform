...
export class EnhancedWebSocketService extends EventEmitter {
  ...
  private setupDefaultFilters(): void {
    ...
    this.addBroadcastFilter({
      ...
      priority: 2,
      condition: (message: BroadcastMessage, user: SocketUser) => {
        if (message.type === 'player_stats' && message.data.pointsChange !== undefined) {
    ...
    this.addBroadcastFilter({
      ...
      priority: 3,
      condition: (message: BroadcastMessage, user: SocketUser) => {
        if (message.data.position && user.preferences.preferredPositions.length > 0) {
    ...
    this.addBroadcastFilter({
      ...
      priority: 4,
      condition: (message: BroadcastMessage, user: SocketUser) => {
        if (message.data.playerId && user.preferences.mutedPlayers.length > 0) {
    ...
    this.addBroadcastFilter({
      ...
      condition: (message: BroadcastMessage, user: SocketUser) => {
        ...
        if (message.type === 'breaking_news' || message.priority === 'critical') {
          return true;
        }
        
        // Check if message is relevant to user's leagues
        if (message.targetLeagues && message.targetLeagues.length > 0) {
          return message.targetLeagues.some(leagueId => user.leagueIds.includes(leagueId));
        }
        
        // Check if message is relevant to user's teams
        if (message.data.teamId && user.teamIds.length > 0) {
          return user.teamIds.includes(message.data.teamId);
        }
        
        // Check if message is relevant to user's players
        if (message.data.playerId && user.playerIds.length > 0) {
          return user.playerIds.includes(message.data.playerId);
        }
  ...
  registerUser(user: SocketUser): void {
    ...
    for (const leagueId of user.leagueIds) {
      if (!this.usersByLeague.has(leagueId)) {
   ...
   */
  updateUserPreferences(userId: string, preferences: Partial<UserBroadcastPreferences>): boolean {
    const user = this.connectedUsers.get(userId);
   ...
   */
  removeBroadcastFilter(filterName: string): boolean {
    const index = this.broadcastFilters.findIndex(f => f.name === filterName);
   ...
   */
  broadcastScoreUpdate(gameUpdate: LiveGameUpdate): void {
  ...
  broadcastInjuryAlert(alert: {
    ...
    const priority = alert.severity === 'major' || alert.fantasyImpact === 'high' ? 'critical' : 'high';
  ...
  broadcastTradeNotification(trade: {
    ...
    players: Array<{ playerId: string; playerName: string; fromTeam: string; toTeam: string }>;
  ...
  broadcastGameEvent(event: {
    ...
    const priority = event.type === 'touchdown' || event.fantasyRelevant ? 'high' : 'medium';
  ...
  private queueMessage(message: BroadcastMessage): void {
    ...
    if (message.deduplicationKey && this.messageDeduplication.has(message.deduplicationKey)) {
      const lastSent = this.messageDeduplication.get(message.deduplicationKey)!;
  ...
  private async processMessage(message: BroadcastMessage): Promise<void> {
    ...
    try {
      ...
      if (message.targetUsers && message.targetUsers.length > 0) {
      ...
      } else if (message.targetLeagues && message.targetLeagues.length > 0) {
        ...
        targetUsers = Array.from(this.connectedUsers.values());
      }
      ...
      const eligibleUsers = targetUsers.filter(user => {
        ...
        for (const filter of this.broadcastFilters) {
          if (!filter.condition(message, user)) {
      ...
      for (const user of eligibleUsers) {
        await this.sendMessageToUser(user, message);
  ...
  private async sendMessageToUser(user: SocketUser, message: BroadcastMessage): Promise<void> {
    try {
      const payload = {
        ...message.data,
        type: message.type,
      ...
      switch (message.type) {
        ...
        case 'breaking_news':
          ...
          break;
        case 'fantasy_milestone':
          // For milestones, we might use a generic broadcast method
  ...
  private isUserThrottled(user: SocketUser, message: BroadcastMessage): boolean {
    const prefs = user.preferences;
    ...
    if (throttle.count >= prefs.throttling.maxUpdatesPerMinute) {
      ...
      if (prefs.throttling.priority === 'important_only' && (message.priority === 'high' || message.priority === 'critical')) {
        return false;
      }
      
      if (prefs.throttling.priority === 'critical_only' && message.priority === 'critical') {
        return false;
      }
  ...
  private convertToUserTimezone(date: Date, timezone: string): Date {
    try {
      ...
      return date; // Fallback to original date if timezone conversion fails
    }
  }
  ...
  private recordBroadcastTime(time: number): void {
    ...
    this.metrics.averageBroadcastTime = this.broadcastTimes.length > 0
      ? this.broadcastTimes.reduce((a, b) => a + b, 0) / this.broadcastTimes.length
      : 0;
  }
