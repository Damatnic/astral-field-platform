...
expect.extend({
  toBeValidNFLTeam(received: string) {
    const validTeams = [
  ...
  toBeValidNFLPosition(received: string) {
  ...
  toBeValidGameStatus(received: string) {
  ...
  toBeValidFantasyPoints(received: number) {
    const pass = typeof received === 'number' && received >= -10 && received <= 100;
...
global.testUtils = {
  ...
  randomNFLTeam: () => {
    const teams = ['KC', 'BUF', 'SF', 'DAL', 'GB', 'NE', 'PIT', 'SEA'];
  ...
  randomFantasyPoints: (min = 0, max = 30) => {
  ...
  // Mock API response
  mockAPIResponse: (data: any, status = 200, ok = true) => {
  ...
  mockDate: new Date('2025-09-15T20:30:00.000Z'),
  
  // Generate test data
  generateTestGames: (count: number) => {
  ...
  generateTestPlayers: (count: number) => {
    return Array.from({ length: count }, (_, index) => ({
  ...
  generateTestStats: (playerId: string, gameId: string) => {
    const position = global.testUtils.randomPosition();
    const baseStats = {
      playerId,
      gameId,
...
let performanceMetrics = {
  ...
  slowTests: new Array<{name: string, duration: number}>(),
  memoryUsage: new Array<{test: string, usage: number}>()
};
...
afterAll(() => {
  ...
  if (performanceMetrics.memoryUsage.length > 0) {
    const maxMemory = Math.max(...performanceMetrics.memoryUsage.map(m => m.usage));
...
global.performanceUtils = {
  ...
  measureMemoryUsage: (fn: () => any): { result: any; memoryDelta: number } => {
...
declare global {
  ...
  var testUtils: {
    ...
    generateTestStats(playerId: string, gameId: string): any;
  };
  
  var performanceUtils: {
    measureExecutionTime(fn: () => Promise<any>): Promise<{ result: any; duration: number }>;
    measureMemoryUsage(fn: () => any): { result: any; memoryDelta: number };
  };
}