...
class NFLDataProvider {
  ...
  private async pollGameUpdates(game: NFLGame): Promise<void> {
    try {
      const previousGame = this.getFromCache(`game_${game.id}`)?.data;
      if (!previousGame) return;

      // Check for score changes
      if (previousGame.homeScore !== game.homeScore || previousGame.awayScore !== game.awayScore) {
  ...
  private async fetchFromPrimaryAPI(): Promise<any> {
    if (!this.apiKeys.sportsIO) return null;
  ...
  private async fetchFromSecondaryAPI(): Promise<any> {
    try {
  ...
  private async fetchFromTertiaryAPI(): Promise<any> {
    if (!this.apiKeys.nflAPI) return null;
  ...
  private async fetchFromDatabase(): Promise<any> {
    try {
      const result = await database.query(`
  ...
  private async makeRateLimitedRequest(apiType: string, url: string, options: any = {}): Promise<Response> {
    ...
    if (!this.rateLimiter.has(apiType)) {
      this.rateLimiter.set(apiType, { count: 0, resetTime: now + windowMs });
    }
