...
export class FallbackChain<T = any> extends EventEmitter {
   ...
   */
  addProvider(provider: FallbackProvider<T>): void {
    if (this.providers.length >= this.config.maxProviders) {
  ...
  async execute(
    ...
    try {
      ...
      if (this.config.enableCaching && !options.skipCache && options.cacheKey) {
        const cachedData = await this.tryCache(options.cacheKey);
        if (cachedData !== null) {
      ...
      for (const provider of availableProviders) {
        ...
        if (providerResult.success) {
          ...
          if (this.config.enableCaching && options.cacheKey && result.data) {
            await this.cacheResult(options.cacheKey, result.data, options.cacheTTL);
          }
      ...
      if (this.config.enableDatabaseFallback && !options.skipDatabase) {
        const dbResult = await this.tryDatabase(operation, params);
        if (dbResult !== null) {
      ...
      if (this.config.enableMockData && !options.skipMockData) {
        const mockResult = await this.tryMockData(operation, params);
        if (mockResult !== null) {
  ...
  private async tryCache(cacheKey: string): Promise<T | null> {
    try {
  ...
  private async cacheResult(cacheKey: string, data: T, ttl?: number): Promise<void> {
    try {
  ...
  private getAvailableProviders(requiredProviders?: string[], excludedProviders?: string[]): FallbackProvider<T>[] {
    let available = this.providers.filter(p => p.enabled);
    ...
    if (excludedProviders && excludedProviders.length > 0) {
      available = available.filter(p => !excludedProviders.includes(p.name));
    }
  ...
  private async tryProvider(
    ...
    while (attempts <= provider.retryAttempts) {
      try {
        ...
        if (provider.validate && !provider.validate(transformedData)) {
  ...
  private async tryDatabase(operation: string, params: any): Promise<T | null> {
    try {
  ...
  private async tryMockData(operation: string, params: any): Promise<T | null> {
    try {
  ...
  private async getDatabaseFallback(operation: string, params: any): Promise<T | null> {
    ...
    switch (operation) {
      case 'getCurrentWeek':
        try {
          const result = await database.query(`
            ...
            WHERE season_year = 2025 AND start_date <= NOW() AND end_date >= NOW()
      ...
      case 'getPlayerStats':
        try {
          const { playerId, week, season } = params;
          const result = await database.query(`
      ...
      default:
        console.warn(`No database fallback available for operation: ${operation}`);
        return null;
    }
  }

  private async generateMockData(operation: string, params: any): Promise<T | null> {
    ...
    switch (operation) {
      ...
      default:
        console.warn(`No mock data available for operation: ${operation}`);
        return null;
    }
  }

  // Circuit breaker methods
  private isCircuitBreakerAllowed(providerName: string): boolean {
  ...
  private updateCircuitBreaker(providerName: string, result: 'success' | 'failure'): void {
    const breaker = this.circuitBreakers.get(providerName);
    if (!breaker) return;
   ...
   */
  toggleProvider(providerName: string, enabled: boolean): boolean {
    const provider = this.providers.find(p => p.name === providerName);
    if (!provider) return false;
