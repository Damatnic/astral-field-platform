...
export class EnhancedDataProvider extends EventEmitter {
  private config: EnhancedDataProviderConfig;
  ...
  private setupFallbackProviders(): void {
    ...
    this.fallbackChain.addProvider({
      ...
      retryAttempts: 2,
      retryDelay: 1000,
      fetch: async (params: { week: number; season: number }) => 
    ...
    this.fallbackChain.addProvider({
      ...
      retryAttempts: 2,
      retryDelay: 1000,
      fetch: async (params: { playerId: string; week: number; season: number }) =>
  ...
  private setupEventListeners(): void {
    ...
    if (this.realTimeSyncService) {
      this.realTimeSyncService.on('game:changed', (data) => {
        if (this.config.webSocket.enableBroadcast) {
      ...
      this.realTimeSyncService.on('player_stats:changed', (data) => {
        if (this.config.webSocket.enableBroadcast) {
  ...
  async getGamesByWeek(week: number, season: number = 2025): Promise<NFLGame[]> {
    ...
    try {
      ...
      if (this.config.validation.enabled) {
        // Validate each game
        games = games.filter((game: NFLGame) => {
          const validation = this.dataValidator.validateGame(game);
  ...
  async getPlayerStats(
    ...
    try {
      ...
      if (stats && this.config.validation.enabled) {
        const validation = this.dataValidator.validatePlayerStats(stats);
        if (!validation.isValid) {
  ...
  async getInjuryReports(): Promise<Array<{
    ...
    try {
      ...
      if (this.config.webSocket.enableBroadcast) {
        validatedInjuries
          .filter(injury => injury.severity === 'major' || injury.fantasyImpact === 'high')
          .forEach(injury => {
  ...
  async getHealthStatus(): Promise<{
    ...
    if (healthyCount === totalComponents) {
      overall = 'healthy';
    } else if (healthyCount >= Math.ceil(totalComponents * 0.7)) {
      overall = 'degraded';
  ...
  private recordFailure(startTime: number, error: Error): void {
    ...
    this.emit('request:failed', { error: error.message, timestamp: new Date() });
  }
  ...
  private classifyInjurySeverity(status: string): 'minor' | 'moderate' | 'major' {
  ...
  private assessFantasyImpact(position: string, injuryStatus: string): 'low' | 'medium' | 'high' {
    ...
    if (criticalPositions.includes(position) && 
      ...
      return 'high';
    } else if (criticalPositions.includes(position)) {
      return 'medium';
    } else {
      return 'low';
    }
