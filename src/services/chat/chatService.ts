...
class ChatService {
  private typingIndicators = new Map<string, Set<string>>();
  private activeUsers = new Map<string, Set<string>>();

  // Chat Room Management
  async getChatRooms(leagueId: string): Promise<ChatRoom[]> {
    try {
  ...
  async createChatRoom(leagueId: string, name: string, type: ChatRoomType, description?: string, createdBy?: string): Promise<ChatRoom> {
    try {
      const result = await database.query(`
        INSERT INTO chat_rooms (league_id, name, description, type, created_by)
        VALUES ($1, $2, $3, $4, $5)
  ...
  async searchMessages(leagueId: string, roomType: ChatRoomType, query: string, limit: number = 20): Promise<ChatMessage[]> {
    try {
      const roomResult = await database.query(`
        SELECT id FROM chat_rooms WHERE league_id = $1 AND type = $2
      `, [leagueId, roomType]);
  ...
  // Typing Indicators
  async setTypingIndicator(leagueId: string, roomType: ChatRoomType, userId: string, isTyping: boolean): Promise<void> {
    try {
      ...
      if (isTyping) {
        ...
        await database.query(`
          ...
          WHERE room_id = (SELECT id FROM chat_rooms WHERE league_id = $1 AND type = $2)
          AND user_id = $3
        `, [leagueId, roomType, userId]);
  ...
  // Message Moderation
  async moderateMessage(messageId: string, moderatorId: string, action: 'hide' | 'delete' | 'warn', reason?: string): Promise<void> {
    try {
      ...
      if (moderatorId !== commissionerId) {
        throw new Error('Unauthorized: Only league commissioner can moderate messages');
      }

      // Record moderation action
      await database.query(`
        INSERT INTO chat_moderation (message_id, moderator_id, action, reason)
        VALUES ($1, $2, $3, $4)
  ...
  async notifyMention(mentionedUserId: string, mentioningUser: string, leagueId: string, message: string): Promise<void> {
    try {
  ...
  // Analytics
  async updateChatAnalytics(leagueId: string, userId: string): Promise<void> {
    try {
      const today = new Date().toISOString().split('T')[0];

      // Update daily analytics
      await database.query(`
        INSERT INTO chat_analytics (league_id, date, total_messages, active_users, created_at)
        VALUES ($1, $2, 1, 1, NOW())
  ...
  async getChatAnalytics(leagueId: string, days: number = 30): Promise<ChatAnalytics[]> {
    try {
      const result = await database.query(`
  ...
  // User Preferences
  async getUserChatPreferences(userId: string, leagueId: string): Promise<any> {
    try {
      const result = await database.query(`
  ...
  async updateUserChatPreferences(userId: string, leagueId: string, preferences: any): Promise<void> {
    try {
      await database.query(`
        INSERT INTO user_chat_preferences 
        (user_id, league_id, notifications_enabled, sound_enabled, mention_notifications, private_message_notifications)
        VALUES ($1, $2, $3, $4, $5, $6)
  ...
  // Real-time Features
  public getTypingUsers(leagueId: string, roomType: ChatRoomType): string[] {
    const roomId = `${leagueId}:${roomType}`;
    const indicators = this.typingIndicators.get(roomId);
    return indicators ? Array.from(indicators) : [];
  }

  public getActiveUsers(leagueId: string, roomType: ChatRoomType): string[] {
    const roomId = `${leagueId}:${roomType}`;
    const users = this.activeUsers.get(roomId);
    return users ? Array.from(users) : [];
  }

  public addActiveUser(leagueId: string, roomType: ChatRoomType, userId: string): void {
    const roomId = `${leagueId}:${roomType}`;
  ...
  public removeActiveUser(leagueId: string, roomType: ChatRoomType, userId: string): void {
    const roomId = `${leagueId}:${roomType}`;
    const users = this.activeUsers.get(roomId);
    if (users) {
      users.delete(userId);
  ...
  async cleanupStaleTypingIndicators(): Promise<void> {
    try {
