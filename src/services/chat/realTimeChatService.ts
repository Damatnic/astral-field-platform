...
class RealTimeChatService {
    ...
    try {
      ...
      await database.query(`
        INSERT INTO chat_rooms (
          ...
          members, moderators, created_at, last_activity,
          message_count, settings
  ...
  async addReaction(
    ...
    try {
      ...
      const messageResult = await database.query(
        'SELECT room_id, league_id FROM chat_messages WHERE id = $1',
        [messageId]
      );

      if (messageResult.rows.length > 0) {
        const { room_id, league_id } = messageResult.rows[0];
  ...
  private async checkSlowMode(roomId: string, userId: string): Promise<void> {
    const room = await this.getRoom(roomId);
  ...
  private async processMentions(message: ChatMessage): Promise<void> {
  ...
  private addToCache(roomId: string, message: ChatMessage): void {
    const cached = this.messageCache.get(roomId) || [];
    cached.push(message);
  ...
  private async getMessageAuthor(messageId: string): Promise<string> {
    const result = await database.query(
  ...
  private async createNotification(notification: Omit<ChatNotification, 'id'>): Promise<void> {
  ...
  private async notifyRoomMembers(
    ...
    for (const memberId of room.members) {
      if (memberId !== actorId) {
        webSocketManager.sendToUser(memberId, {
          type: eventType,
          room: room,
          actorId
        });
  ...
  private getDefaultRoomSettings(type: ChatRoom['type']): ChatRoomSettings {
    const defaults: Record<ChatRoom['type'], ChatRoomSettings> = {
      general: {
        allowReactions: true,
        allowThreads: true,
  ...
  private async verifyModeratorPermissions(messageId: string, userId: string): Promise<void> {
    const messageResult = await database.query(
      'SELECT room_id FROM chat_messages WHERE id = $1',
      [messageId]
    );
    ...
    if (!room || !room.moderators.includes(userId)) {
  ...
  private async extractPopularTopics(
    ...
    for (const keyword of keywords) {
      ...
      `, roomId ?
        [leagueId, since, `%${keyword}%`, roomId] :
        [leagueId, since, `%${keyword}%`]
      );
