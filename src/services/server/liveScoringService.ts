...
class LiveScoringServerService {
  ...
  private simulateGamesUsingTeams(teamsBasic: Awaited<ReturnType<typeof sportsDataService.getTeamsBasic>>): LiveGame[] {
    const teams = [...teamsBasic].sort(() => Math.random() - 0.5)
  ...
  private calcLiveFantasyPoints(position: string, gameStatus: 'scheduled' | 'live' | 'final'): number {
  ...
  private computeFantasyPointsFromStats(stats: Record<string, number>, ppr = 0.5): number {
    const passYds = stats.passingYards ?? stats.PassingYards ?? 0
  ...
  private generateLiveStats(position: string, gameStatus: 'scheduled' | 'live' | 'final'): Record<string, number> {
  ...
  async getLeagueLiveScoring(leagueId: string, week: number): Promise<LeagueLiveScoring> {
    ...
    for (const team of teams) {
      ...
      if (Array.isArray(lineupRes.rows)) {
        for (const entry of lineupRes.rows) {
          ...
          const fantasyPoints = latestStats?.fantasyPoints ?? this.calcLiveFantasyPoints(p.position, gameStatus)
          ...
          if (['QB','RB','WR','TE','FLEX','D/ST','DST','K'].includes(entry.position_slot)) starters.push(live)
    ...
    if (gamesInProgress) {
      try {
        ...
        games = (scores || []).slice(0, 8).map((g: any, idx: number) => ({
          id: `game_${idx}`,
          awayTeam: g.AwayTeam,
          homeTeam: g.HomeTeam,
          ...
          timeRemaining: g.TimeRemaining || '00:00',
  ...
  async refreshLeagueLiveStats(leagueId: string, week: number, ppr?: number): Promise<{ updated: number; skipped: number; ppr: number }> {
