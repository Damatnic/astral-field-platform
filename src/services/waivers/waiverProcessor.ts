...
export interface WaiverSettings {
  waiverType: 'faab' | 'rolling' | 'reverse';
  processDay: string; // 'tuesday', 'wednesday', etc.
  processTime: string; // '03:00:00'
  faabBudget?: number;
  minBid?: number;
  bidIncrement?: number;
...
class WaiverProcessor {
  ...
  async cancelWaiverClaim(claimId: string, teamId: string): Promise<void> {
    ...
    if (claim.status !== 'pending') {
      throw new Error('Cannot cancel processed waiver claim');
    }
  ...
  async processWaivers(leagueId: string): Promise<WaiverProcessResult> {
    // Prevent concurrent processing
    if (this.processingLocks.get(leagueId)) {
    ...
    try {
      ...
      switch (settings.waiverType) {
        ...
        case 'reverse':
          ...
          break;
        default:
          throw new Error(`Unsupported waiver type: ${settings.waiverType}`);
      }
  ...
  private async processFaabWaivers(
    leagueId: string,
    claims: WaiverClaim[],
    settings: WaiverSettings
  ): Promise<WaiverProcessResult> {
    ...
    for (const [playerId, playerClaims] of claimsByPlayer) {
      const playerResult = await this.processFaabPlayerClaims(
        playerId,
        playerClaims,
        teamBudgets,
        settings
  ...
  private async processFaabPlayerClaims(
    playerId: string,
    claims: WaiverClaim[],
    ...
    for (const claim of sortedClaims) {
      const teamBudget = teamBudgets.get(claim.teamId) || 0;
  ...
  private async processRollingWaivers(
    leagueId: string,
    claims: WaiverClaim[],
    settings: WaiverSettings
  ): Promise<WaiverProcessResult> {
    ...
    for (const [playerId, playerClaims] of claimsByPlayer) {
      const sortedClaims = this.sortClaimsByPriority(playerClaims, priorities);
  ...
  private async processReverseWaivers(
    leagueId: string,
    claims: WaiverClaim[],
    settings: WaiverSettings
  ): Promise<WaiverProcessResult> {
    ...
    for (const [playerId, playerClaims] of claimsByPlayer) {
      const sortedClaims = this.sortClaimsByStandings(playerClaims, standings);
  ...
  private async sortFaabClaims(claims: WaiverClaim[], settings: WaiverSettings): Promise<WaiverClaim[]> {
  ...
  private sortClaimsByPriority(claims: WaiverClaim[], priorities: Map<string, WaiverPriority>): WaiverClaim[] {
    return claims.sort((a, b) => {
  ...
  private sortClaimsByStandings(claims: WaiverClaim[], standings: Map<string, number>): WaiverClaim[] {
    return claims.sort((a, b) => {
  ...
  private async calculateNextProcessDate(leagueId: string, settings: WaiverSettings): Promise<Date> {
    const now = new Date();
  ...
  private generateId(): string {
    return `waiver_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  }
  ...
  private async getWaiverSettings(leagueId: string): Promise<WaiverSettings> {
    ...
    return {
      ...
      processTime: league.waiver_process_time || '03:00:00',
  ...
  private async storeWaiverClaim(claim: WaiverClaim): Promise<void> {
    // Create transaction record
    const transactionResult = await database.query(`
      INSERT INTO transactions (league_id, transaction_type, status, initiated_by, details, created_at)
      VALUES ($1, 'waiver', 'pending', (SELECT user_id FROM teams WHERE id = $2), $3, $4)
      RETURNING id
    `, [
      claim.leagueId,
      claim.teamId,
    ...
    await database.query(`
      INSERT INTO waiver_claims (
        id, transaction_id, team_id, player_add_id, player_drop_id,
        waiver_priority, faab_amount, process_date, status, created_at
  ...
  private async validateWaiverClaim(claim: any, settings: WaiverSettings): Promise<void> {
    // Implementation for claim validation
  }

  private async scheduleWaiverProcessing(leagueId: string, processDate: Date): Promise<void> {
  ...
  private async getTeamFaabBudgets(leagueId: string): Promise<Map<string, number>> {
    // Implementation to fetch team FAAB budgets
    return new Map();
  }

  private async getWaiverPriorities(leagueId: string): Promise<Map<string, WaiverPriority>> {
    // Implementation to fetch waiver priorities
    return new Map();
  }

  private async getTeamStandings(leagueId: string): Promise<Map<string, number>> {
    // Implementation to fetch team standings
    return new Map();
  }

  private async hasRosterSpace(teamId: string): Promise<boolean> {
    // Implementation to check roster space
    return true;
  }

  private async persistProcessingResults(result: WaiverProcessResult): Promise<void> {
    // Implementation to save processing results to database
  }

  private createEmptyProcessResult(startTime: number): WaiverProcessResult {
    return {
      processedClaims: [],
      updatedPriorities: [],
      budgetUpdates: [],
  ...
  private broadcastWaiverResults(leagueId: string, result: WaiverProcessResult): void {
  ...
  // Cleanup method
  cleanup(): void {
    for (const timer of this.scheduledProcessing.values()) {
      clearTimeout(timer);
    }
