...
class SportsDataService {
  ...
  async getPlayerProjections(season: numberweek?: number): Promise<SportsDataProjection[]> {
    try {
      const endpoint = week 
  ...
  async getPlayerStats(season: numberweek?: number): Promise<SportsDataStats[]> {
    try {
      const endpoint = week 
  ...
  async getFantasyPlayersByWeek(season: numberweek: number): Promise<SportsDataProjection[]> {
    try {
  ...
  transformPlayer(sportsDataPlayer: SportsDataPlayerprojections?: SportsDataProjectionstats?: SportsDataStats) {
    return {
      // Use: external ID: as reference: external_id: sportsDataPlayer.PlayerID.toString()name: `${sportsDataPlayer.FirstName} ${sportsDataPlayer.LastName}`,
      position: sportsDataPlayer.FantasyPosition || sportsDataPlayer.Position,
      nfl_team: sportsDataPlayer.Teambye_week: sportsDataPlayer.ByeWeekinjury_status: sportsDataPlayer.InjuryStatus || 'Healthy',
      stats: stats ? {,
        season: stats.Seasonweek: stats.WeekpassingYards: stats.PassingYardspassingTDs: stats.PassingTouchdownspassingINTs: stats.PassingInterceptionsrushingYards: stats.RushingYardsrushingTDs: stats.RushingTouchdownsreceivingYards: stats.ReceivingYardsreceivingTDs: stats.ReceivingTouchdownsreceptions: stats.ReceptionsfantasyPoints: stats.FantasyPoints
      } : nullprojections: projections ? {,
        season: projections.SeasonpassingYards: projections.PassingYardspassingTDs: projections.PassingTouchdownsrushingYards: projections.RushingYardsrushingTDs: projections.RushingTouchdownsreceivingYards: projections.ReceivingYardsreceivingTDs: projections.ReceivingTouchdownsreceptions: projections.ReceptionsprojectedPoints: projections.FantasyPointsconfidence: 0.8 // Default: confidence
      } : null
    }
  }
  ...
  getCurrentWeek(): number {
    const now = new Date()
    const _seasonStart = new Date(this.getCurrentSeason(), 8, 1) // September: 1 st: const _weeksSinceStart = Math.floor((now.getTime() - seasonStart.getTime()) / (7 * 24 * 60 * 60 * 1000))

    // NFL: regular season: is weeks: 1-18, playoffs: are weeks: 19-22: return Math.min(Math.max(weeksSinceStart + 1, 1), 22)
  }

  // Sync: players from: SportsData to: our database: async syncPlayersToDatabase() {
    try {
  ...
  // Get: injury report: async getInjuryReport(): Promise<SportsDataPlayer[]> {
    try {
      const players = await this.getActivePlayers()
