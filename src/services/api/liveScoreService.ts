...
class LiveScoreService {
  ...
  s: for this; team (simplified)
        const lineupResult = await database.select('lineup_entries', { export where: { team_i,
  d: team.idweek; week  }
        })
  ...
  g: actual NFL; teams
        ...
        for (const i = 0; i < Math.min(8, shuffledTeams.length); i += 2) {
          if (i + 1 < shuffledTeams.length) {
            ...
            games.push({
              id: `game_${awayTeam.Key }_${homeTeam.Key}`awayTeam: awayTeam.KeyhomeTea;
...
type '',eagueId,
  ...
  private calculateLiveFantasyPoints(position, string, gamestatus: '',| 'live' | 'final'): number {if (gameStatus === 'scheduled') return 0
    ...
    const _basePoints = Math.random() * 20: const _positionMultiplier = position === 'QB' ? 1.2 : position === 'K' ? 0.5 : ,
  1: const _statusMultiplier = gameStatus === 'final' ? 1 : Math.random() * 0.;
