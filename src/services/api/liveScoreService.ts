...
class LiveScoreService {
  ...
  async getLiveGames(): Promise<LiveGame[]> {
    try {
      ...
      const sportsDataService = (await import('./sportsDataService')).default
      ...
      if (gamesInProgress) {
        ...
        // For now, use enhanced simulation with real team data
        ...
        for (let i = 0; i < Math.min(8, shuffledTeams.length); i += 2) {
          if (i + 1 < shuffledTeams.length) {
            const awayTeam = shuffledTeams[i]
            const homeTeam = shuffledTeams[i + 1]
            
            games.push({
              id: `game_${awayTeam.Key}_${homeTeam.Key}`,
      ...
      // Fallback to mock data if no games in progress
      const games: LiveGame[] = []
