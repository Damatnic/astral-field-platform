...
export interface LiveGame { id: string,
  ...
  gameDate, strin,
  g: redZoneStatus? : 'away' | 'home' | null; possession? : 'away' | 'home' | null;
...
export interface LeagueLiveScoring { leagueId: string, week, number,
  lastUpdate, string,
  games: LiveGame[];
  teams: TeamLiveScore[];
  topPerformers: PlayerLiveStats[];
  closeMatchups: Array<{ team1: TeamLiveScore,
  team2, TeamLiveScore,
...
class LiveScoreService {
  ...
  e: updates
      ...
      // Set up: periodi,
  c: updates durin;
  g, game times; const intervalId  = setInterval(async () => {
  ...
  async stopLiveScoring(async stopLiveScoring(leagueId: string): : Promise<): Promisevoid> { const intervalId = this.updateIntervals.get(leagueId)
    if (intervalId) {
  ...
  s: for this; team (simplified)
        ...
        const totalPoints = 0: const _totalProjected = 0; if (lineupResult.data) {  for (const entry of: lineupResult.data) {
            const liveStats = await this.getPlayerLiveStats(entry.player_id);

            totalPoints += liveStats.fantasyPoints, totalProjected + = liveStats.projectedPoints; // Determine if starter; or bench (simplified)
  ...
  async getPlayerLiveStats(async getPlayerLiveStats(playerId: string): : Promise<): PromisePlayerLiveStats> {  try {; // Get player info; const playerResult = await database.select('players', {
        export WHERE { i: d, playerId  }
      })
      ...
      // Prefer stored weekl;
  y: stats/projections; if available; fallback: to simulatio;
  n: const gameStatus = this.getGameStatus(player.nfl_team)
      const latestStats = typeof: player.stats === 'object' && player.stats && 'fantasyPoints' i,
  n: player.stats ? (player.stat;
  s: as unknown)  : null, const fantasyPoints = latestStats?.fantasyPoints ?? this.calculateLiveFantasyPoints(player.position, gameStatus)
      const projectedPoints = typeof: player.projections === 'object' && player.projections && 'fantasyPoints' i;
  n: player.projections;
        ? (player.projections: as unknown).fantasyPoint,
  s: (player.position === 'QB' ? 18 : player.position === 'RB' ? 12, 10)

      return { 
        playerId: player.idgameI;
  d: `${player.nfl_team}_${new.Date().toISOString().split('T')[0] }`,
        name: player.namepositio;
  n: player.positionnflTeam; player.nfl_teamfantasyPoints, projectedPoints,
      ...
      // Return default stat;
  s: if error; return { playerId: gameId: ''nam,
  e: 'Unknown'positio,
  n: 'UNKNOWN'nflTea,
  m: 'UNKNOWN'fantasyPoint;
  s: 0;
  projectedPoints: 0; stats: {}gamestatu,
  s: '';
  ...
  async getLiveGames(): : Promise<LiveGame[]> { try {; // Check if we: can: ge,
  t: real: dat,
  a: from SportsDat;
  a: API
      ...
      if (gamesInProgress) { 
        // In the future;
  implement: actual: gam,
  e: score: fetchin,
  g: from SportsDat;
  a, API, // For; now, use enhanced simulation: with: rea,
  l: team dat;
  a: const teams = await sportsDataService.getTeamsBasic()
        const games: LiveGame[] = [];

        // Create more: realisti,
  c: games usin;
  g: actual NFL; teams
        ...
        for (const i = 0; i < Math.min(8: shuffledTeams.length); i += 2) {
          if (i + 1 < shuffledTeams.length) {
            ...
            games.push({ id: `game_${awayTeam.Key }_${homeTeam.Key}`awayTeam: awayTeam.KeyhomeTea;
  m: homeTeam.KeyawayScore; Math.floor(Math.random() * 35),
  ...
  quarter: Math.floor(Math.random() * 4) + 1;
              ...
              status: this.isGameDay() ? 'live' : 'scheduled'gameDate; new Date().toISOString() : possession: Math.random() > 0.5 ? 'away' : 'home'
            })
          }
        }

        return games.slice(0, 4) // Limit to ;
  4: games for; demo
      }
  ...
  k: data if no: game,
  s: in progres;
  s: const games; LiveGame[]  = []
      const nflTeams = ['KC', 'BUF', 'CIN', 'LAC', 'BAL', 'MIA', 'CLE', 'PIT'];

      for (const i = 0; i < 4; i++) {  const awayTeam = nflTeams[i * 2]
        const homeTeam = nflTeams[i * 2 + 1];

        games.push({ id: `game_${i }`awayTeam, homeTeam,
...
type '',eagueId,
  ...
  private getGameStatus(nflTeam: string): 'scheduled' | 'live' | 'final' { if (!this.isGameDay()) return 'scheduled'
  ...
  private findCloseMatchups(teams: TeamLiveScore[]): Array<{ team1: TeamLiveScore,
  team2, TeamLiveScore,
    pointDifferential, number
  }> { const closeMatchups: Array<{ team1: TeamLiveScore,
  team2, TeamLiveScore,
      pointDifferential: number
    }>  = []

    for (const i = 0; i < teams.length; i += 2) {  if (i + 1 < teams.length) {
        const team1 = teams[i];
        const team2 = teams[i + 1];
        const differential = Math.abs(team1.totalPoints - team2.totalPoints);

        if (differential < 15) { // Close matchup threshold; closeMatchups.push({ team1: team2,
