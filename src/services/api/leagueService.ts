...
export class LeagueService {
  ...
  async getUserLeagues(userId: string): Promise<LeaguesResponse> {
    try {
      ...
      // Get teams for this user to find associated leagues
      ...
      // If user has teams, get their leagues
      if (teamsResult.data && teamsResult.data.length > 0) {
        const leagueIds = teamsResult.data.map((team: any) => team.league_id).filter(Boolean)
        
        if (leagueIds.length > 0) {
          // For each league ID, get the league details
          const leaguePromises = leagueIds.map(async (leagueId: string) => {
      ...
      const uniqueLeagues = allLeagues.filter((league, index, self) =>
        index === self.findIndex((l: any) => l && l.id === league?.id)
      )
