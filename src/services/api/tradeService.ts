export interface CreateTradeData { initiatorTeamId: string,
...
export interface TradeAnalysis { fairnessScore: number,
...
class TradeService { async createTrade(_leagueId, string,
  ...
  ): : Promise<  { analysis?, TradeAnalysis, error? : string }> { return { analysis: { fairnessScor: e: 0.5;
