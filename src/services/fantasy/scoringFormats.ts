...
export class ScoringFormatLibrary {
  private static readonly BASE_POSITION_SCORING: BasePositionScoring = {
   ...
   */
  static getStandardScoring(): AdvancedScoringRules {
    return {
   ...
   */
  static getPPRScoring(): AdvancedScoringRules {
    return {
      ...
      qb: {
        ...this.BASE_POSITION_SCORING,
        receptions: 0, // No PPR for QBs typically
      },
      rb: {
        ...this.BASE_POSITION_SCORING,
   ...
   * Half-PPR Scoring
   */
  static getHalfPPRScoring(): AdvancedScoringRules {
    return {
   ...
   */
  static getDynastyScoring(): AdvancedScoringRules {
    return {
      ...
      rookieBonuses: {
        ...
        rookieThresholdBonuses: {
          qb_passing_yards: { threshold: 4000, bonus: 10 },
          rb_rushing_yards: { threshold: 1000, bonus: 8 },
          wr_receiving_yards: { threshold: 1000, bonus: 8 },
          te_receiving_yards: { threshold: 800, bonus: 6 }
        }
      }
    };
  }
   ...
   */
  static getSuperflexScoring(): AdvancedScoringRules {
    return {
   ...
   */
  static getIDPScoring(): AdvancedScoringRules {
    return {
   ...
   * Best Ball Scoring
   */
  static getBestBallScoring(): AdvancedScoringRules {
    return {
   ...
   */
  static getStandardWeatherModifiers(): WeatherModifiers {
    return {
      enabled: true,
      temperatureThresholds: {
        extreme_cold: { threshold: 20, modifier: -0.05 }, // -5% in extreme cold
        cold: { threshold: 40, modifier: -0.02 },         // -2% in cold
        hot: { threshold: 85, modifier: -0.01 },          // -1% in hot weather
        extreme_hot: { threshold: 95, modifier: -0.03 }   // -3% in extreme heat
      },
      windThresholds: {
        moderate: { threshold: 15, modifier: -0.02 },     // -2% for 15+ mph winds
        strong: { threshold: 25, modifier: -0.05 },       // -5% for 25+ mph winds
        extreme: { threshold: 35, modifier: -0.10 }       // -10% for 35+ mph winds
      },
   ...
   */
  static getStandardInjuryModifiers(): InjuryModifiers {
    return {
      enabled: true,
   ...
   */
  static getStandardMatchupModifiers(): MatchupModifiers {
    return {
      enabled: true,
      difficultyTiers: {
  ...
  static createCustomFormat(
    ...
    const baseRules = this.getFormatByType(baseFormat);
   ...
   * Get format by type
   */
  static getFormatByType(format: ScoringFormat): AdvancedScoringRules {
    switch (format) {
      ...
      case ScoringFormat.BEST_BALL:
        return this.getBestBallScoring();
      default:
   ...
   */
  static getAllFormats(): { format: ScoringFormat; rules: AdvancedScoringRules }[] {
    return [
      { format: ScoringFormat.STANDARD, rules: this.getStandardScoring() },
      { format: ScoringFormat.PPR, rules: this.getPPRScoring() },
      { format: ScoringFormat.HALF_PPR, rules: this.getHalfPPRScoring() },
      { format: ScoringFormat.DYNASTY, rules: this.getDynastyScoring() },
      { format: ScoringFormat.SUPERFLEX, rules: this.getSuperflexScoring() },
      { format: ScoringFormat.IDP, rules: this.getIDPScoring() },
      { format: ScoringFormat.BEST_BALL, rules: this.getBestBallScoring() }
    ];
  }
   ...
   */
  static createTEPremiumFormat(baseFormat: ScoringFormat = ScoringFormat.PPR): AdvancedScoringRules {
    const baseRules = this.getFormatByType(baseFormat);
   ...
   */
  static createSixPointPassingFormat(baseFormat: ScoringFormat = ScoringFormat.PPR): AdvancedScoringRules {
    const baseRules = this.getFormatByType(baseFormat);
   ...
   */
  static createBonusHeavyFormat(baseFormat: ScoringFormat = ScoringFormat.PPR): AdvancedScoringRules {
    const baseRules = this.getFormatByType(baseFormat);
   ...
   */
  static validateFormat(rules: AdvancedScoringRules): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    for (const position of positions) {
      ...
      if (position !== 'kicker' && position !== 'defense') {
        const baseRules = positionRules as BasePositionScoring;
  ...
  static compareFormats(
    ...
    for (const position of positions) {
      const rules1 = format1[position] as BasePositionScoring;
