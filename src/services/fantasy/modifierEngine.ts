...
export class FantasyModifierEngine {
  ...
  async applyWeatherModifiers(
    ...
    try {
      ...
      if (tempModifier !== 0) {
        const adjustment = basePoints * tempModifier;
      ...
      if (this.positionAffectedByWind(position)) {
        const windModifier = this.calculateWindModifier(weatherData.windSpeed, weatherModifiers);
        if (windModifier !== 0) {
      ...
      if (precipModifier !== 0) {
        const adjustment = basePoints * precipModifier;
        adjustedPoints += adjustment;
      ...
      // Apply dome bonus
      if (gameContext.isDomeGame && weatherModifiers.domeBonus) {
        const adjustment = basePoints * weatherModifiers.domeBonus;
        adjustedPoints += adjustment;
   ...
   */
  private calculateTemperatureModifier(temperature: number, modifiers: WeatherModifiers): number {
    if (temperature <= modifiers.temperatureThresholds.extreme_cold.threshold) {
      return modifiers.temperatureThresholds.extreme_cold.modifier;
    }
    if (temperature <= modifiers.temperatureThresholds.cold.threshold) {
      return modifiers.temperatureThresholds.cold.modifier;
    }
    if (temperature >= modifiers.temperatureThresholds.extreme_hot.threshold) {
      return modifiers.temperatureThresholds.extreme_hot.modifier;
    }
    if (temperature >= modifiers.temperatureThresholds.hot.threshold) {
      return modifiers.temperatureThresholds.hot.modifier;
    }
    return 0;
  }
   ...
   */
  private calculateWindModifier(windSpeed: number, modifiers: WeatherModifiers): number {
    if (windSpeed >= modifiers.windThresholds.extreme.threshold) {
      return modifiers.windThresholds.extreme.modifier;
    }
    if (windSpeed >= modifiers.windThresholds.strong.threshold) {
      return modifiers.windThresholds.strong.modifier;
    }
    if (windSpeed >= modifiers.windThresholds.moderate.threshold) {
      return modifiers.windThresholds.moderate.modifier;
    }
    return 0;
  }
   ...
   */
  private calculatePrecipitationModifier(weather: WeatherData, modifiers: WeatherModifiers): number {
    ...
    if (conditions.includes('snow')) {
      return modifiers.precipitationModifiers.snow;
    }
    if (conditions.includes('heavy rain') || conditions.includes('thunderstorm')) {
      return modifiers.precipitationModifiers.heavy_rain;
    }
    if (conditions.includes('rain') || conditions.includes('drizzle')) {
      return modifiers.precipitationModifiers.light_rain;
    }
   ...
   */
  private positionAffectedByWind(position: Position): boolean {
    return [Position.QB, Position.WR, Position.TE, Position.K].includes(position);
  }
  ...
  async applyInjuryModifiers(
    ...
    try {
      ...
      if (injuryContext.isReturnFromInjury) {
        const returnModifier = injuryModifiers.returnFromInjuryModifier;
      ...
      if (Math.abs(injuryModifier) > 0.01) {
        const adjustment = basePoints * injuryModifier;
        adjustedPoints += adjustment;
  ...
  async applyMatchupModifiers(
    ...
    try {
      // Get matchup context
      const matchupContext = await this.getMatchupContext(
        gameContext.isHomeGame ? gameContext.awayTeam : gameContext.homeTeam,
        position,
        gameContext.week,
        gameContext.season
      );
      ...
      const strengthModifier = this.calculateOpponentStrengthModifier(
        matchupContext.opponentRank.vsPosition,
        matchupModifiers
      );

      if (strengthModifier !== 0) {
        const adjustment = basePoints * strengthModifier;
        adjustedPoints += adjustment;
      ...
      if (gameContext.isHomeGame && matchupModifiers.homeFieldAdvantage) {
        const homeAdvantage = basePoints * matchupModifiers.homeFieldAdvantage;
      ...
      if (gameContext.isDivisionalGame && matchupModifiers.divisionalRivalryBonus) {
        const rivalryBonus = basePoints * matchupModifiers.divisionalRivalryBonus;
      ...
      if (gameContext.isPrimetime && matchupModifiers.primetime_bonus) {
        const primetimeBonus = basePoints * matchupModifiers.primetime_bonus;
  ...
  private calculateOpponentStrengthModifier(
    ...
    if (opponentRank <= 5) {
      ...
      return modifiers.difficultyTiers.strong_defense;
    } else if (opponentRank <= 22) {
      return modifiers.difficultyTiers.average_defense;
    } else if (opponentRank <= 27) {
      return modifiers.difficultyTiers.weak_defense;
    } else {
      return modifiers.difficultyTiers.worst_defense;
    }
  }
   ...
   */
  private getStrengthDescription(rank: number): string {
   ...
   */
  private async getWeatherData(gameId: string): Promise<WeatherData | null> {
    if (this.weatherCache.has(gameId)) {
   ...
   */
  private async getInjuryContext(playerId: string): Promise<InjuryContext | null> {
    if (this.injuryCache.has(playerId)) {
   ...
   */
  private async getMatchupContext(
    opponentTeam: string,
    position: Position,
    week: number,
    season: number
  ): Promise<MatchupContext | null> {
    const cacheKey = `${opponentTeam}_${position}_${week}_${season}`;
    
    if (this.matchupCache.has(cacheKey)) {
      return this.matchupCache.get(cacheKey)!;
    }
  ...
  async applyAllModifiers(
    ...
    const breakdown = { weather: 0, injury: 0, matchup: 0 };

    // Apply weather modifiers
    if (weatherModifiers) {
      const weatherResult = await this.applyWeatherModifiers(
        adjustedPoints,
        position,
        gameContext,
        weatherModifiers
    ...
    if (injuryModifiers) {
      const injuryResult = await this.applyInjuryModifiers(
        adjustedPoints,
        playerId,
        position,
        injuryModifiers
    ...
    if (matchupModifiers) {
      const matchupResult = await this.applyMatchupModifiers(
        adjustedPoints,
        position,
        gameContext,
        matchupModifiers
