...
class FantasyScoringEngine {
  private scoringRulesCache = new Map<string, ScoringRules>();
  ...
  calculateFantasyPoints(stats: PlayerStats, rules: ScoringRules): number {
    let totalPoints = 0;
    const breakdown = {
      passing: 0,
      rushing: 0,
  ...
  async processLiveScoring(): Promise<void> {
    ...
    try {
      ...
      const leaguesResult = await database.query(
        'SELECT id FROM leagues WHERE is_active = true AND season_year = 2025'
      );
  ...
  private async processLeagueScoring(leagueId: string, week: number): Promise<void> {
    try {
      const scoringRules = await this.getScoringRules(leagueId);
      
      // Get all teams in the league
      const teamsResult = await database.query(
        'SELECT id, user_id FROM teams WHERE league_id = $1',
        [leagueId]
      );
  ...
  private async processTeamScoring(
    leagueId: string, 
    teamId: string, 
    week: number, 
    rules: ScoringRules
  ...
  private async updatePlayerScore(
    ...
    try {
      await database.query(`
        INSERT INTO live_fantasy_scores (team_id, player_id, week, season_year, current_points, last_updated)
        VALUES ($1, $2, $3, 2025, $4, NOW())
        ON CONFLICT (team_id, player_id, week, season_year)
        DO UPDATE SET 
          current_points = $4,
          last_updated = NOW()
  ...
  private async getPreviousScore(teamId: string, playerId: string, week: number): Promise<FantasyScore | null> {
    try {
      const result = await database.query(`
      ...
      return {
        ...
        breakdown: {
          ...
          defense: 0, bonuses: 0, penalties: 0
        },
  ...
  private getFieldGoalPoints(stats: PlayerStats, rules: ScoringRules): number {
  ...
  private createScoringRules(settings: any): ScoringRules {
    return {
  ...
  async getTeamScore(teamId: string, week: number): Promise<number> {
    try {
      const result = await database.query(`
  ...
  // Get matchup scores
  async getMatchupScores(matchupId: string): Promise<{
    homeScore: number;
    awayScore: number;
    homeTeamId: string;
    awayTeamId: string;
    ...
    try {
      ...
      return { homeScore: 0, awayScore: 0, homeTeamId: '', awayTeamId: '' };
    }
  }
  ...
  async triggerPlayerScoreUpdate(playerId: string, leagueId: string, week?: number): Promise<void> {
