...
export class FantasyRuleEngine {
   ...
   */
  getActiveRules(position: Position, week?: number): CustomRule[] {
   ...
   */
  getActiveBonuses(position: Position): PerformanceBonus[] {
  ...
  evaluateRules(
    ...
    ruleAdjustments: Array<{ ruleName: string; points: number; description: string }>;
  } {
    ...
    const ruleAdjustments: Array<{ ruleName: string; points: number; description: string }> = [];
        ...
        if (conditionMet) {
          ...
          if (adjustment !== 0) {
            ...
            appliedModifiers.push({
              type: 'custom',
              name: rule.name,
              multiplier: rule.action.type === 'multiply_points' ? rule.action.value : 1,
    ...
    const result = { adjustedPoints, appliedModifiers, ruleAdjustments };
  ...
  evaluatePerformanceBonuses(
    stats: PlayerStats, 
    position: Position
  ): Array<{ bonusName: string; points: number; trigger: string }> {
    const bonuses: Array<{ bonusName: string; points: number; trigger: string }> = [];
   ...
   */
  private evaluateStatThreshold(condition: RuleCondition, stats: PlayerStats): boolean {
    const statValue = this.getStatValue(stats, condition.field);
  ...
  private evaluateGameSituation(
    ...
    switch (condition.field) {
      ...
      case 'game_type':
        // Could evaluate playoff vs regular season
   ...
   */
  private evaluatePlayerAttribute(condition: RuleCondition, position: Position): boolean {
    switch (condition.field) {
      case 'position':
  ...
  private evaluateBonusCondition(
    ...
    const statValue = this.getStatValue(stats, condition.stat);
   ...
   */
  private getStatValue(stats: PlayerStats, field: string): number | undefined {
   ...
   */
  private getBonusTriggerDescription(condition: any, stats: PlayerStats): string {
    const statValue = this.getStatValue(stats, condition.stat);
   ...
   */
  static createPresetRules(): CustomRule[] {
    return [
      // 40+ yard TD bonus
      {
        id: 'long_td_bonus',
   ...
   */
  static createPresetBonuses(): PerformanceBonus[] {
    return [
      {
        id: 'hat_trick_bonus',
   ...
   */
  validateRule(rule: CustomRule): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    if (rule.condition) {
      if (!['stat_threshold', 'game_situation', 'player_attribute', 'composite'].includes(rule.condition.type)) {
    ...
    // Action validation
    if (rule.action) {
      if (!['add_points', 'multiply_points', 'set_points', 'add_percentage'].includes(rule.action.type)) {
   ...
   */
  testRule(rule: CustomRule, testStats: PlayerStats, position: Position, week: number): {
    triggered: boolean;
    adjustment: number;
    description: string;
  } {
