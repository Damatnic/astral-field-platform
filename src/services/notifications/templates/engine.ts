...
export class TemplateEngine {
  ...
  private async renderTemplate(
    ...
    try {
      ...
      // Process filters ({{variable | filter:arg}})
   ...
   */
  private async resolveVariable(expression: string, context: TemplateContext): Promise<any> {
    // Handle filters (variable | filter:args)
    if (expression.includes('|')) {
      const [variablePath, filterExpression] = expression.split('|').map(s => s.trim());
    ...
    if (this.variableResolvers.has(expression)) {
  ...
  private applyFilter(value: any, filterExpression: string): any {
    const [filterName, ...filterArgs] = filterExpression.split(':').map(s => s.trim());
   ...
   */
  private async processConditionals(content: string, context: TemplateContext): Promise<string> {
   ...
   * Process loop blocks
   */
  private async processLoops(content: string, context: TemplateContext): Promise<string> {
    const loopPattern = /\{\{#each\s+([^}]+)\}\}([\s\S]*?)\{\{\/each\}\}/g;
    
    return content.replace(loopPattern, (match, itemsPath, block) => {
      try {
   ...
   */
  private async processFilters(content: string, context: TemplateContext): Promise<string> {
   ...
   */
  private getDefaultTemplate(type: NotificationType, channel: NotificationChannel): NotificationTemplate {
  ...
  private setupVariableResolvers(): void {
    this.variableResolvers.set('currentTime', async (context) => {
      return new Date().toLocaleString('en-US', { 
        timeZone: context.user.timezone 
      });
    });

    this.variableResolvers.set('gameDay', async (context) => {
      const today = new Date().getDay();
      return today === 0 || today === 1 || today === 4; // Sun, Mon, Thu
    });

    this.variableResolvers.set('weekNumber', async (context) => {
      const now = new Date();
  ...
  private setupFilters(): void {
    ...
    this.filters.set('currency', (value, currency = 'USD') => {
    ...
    this.filters.set('date', (value, format = 'short') => {
    ...
    this.filters.set('time', (value, format = 'short') => {
      const date = new Date(value);
    ...
    this.filters.set('pluralize', (count, singular, plural) => {
  ...
  private async getLeagueContext(leagueId: string): Promise<any> {
    try {
      const result = await database.query(
        'SELECT id, name, size FROM leagues WHERE id = $1',
        [leagueId]
      );
  ...
  private async getTeamContext(teamId: string): Promise<any> {
    try {
      const result = await database.query(
  ...
  private async getPlayerContext(playerId: string): Promise<any> {
    try {
      const result = await database.query(
        'SELECT id, first_name, last_name, position, team FROM nfl_players WHERE id = $1',
        [playerId]
      );
  ...
  private async getGameContext(gameId: string): Promise<any> {
    try {
      const result = await database.query(
        'SELECT id, home_team, away_team, status, game_time FROM games WHERE id = $1',
        [gameId]
      );
  ...
  private getDefaultActions(type: NotificationType): any[] {
    const typeActions: Record<string, any[]> = {
      'trade_proposal': [
        { id: 'view', text: 'View Trade', url: '{{actionUrl}}' },
        { id: 'respond', text: 'Respond', url: '{{actionUrl}}' }
      ],
      'lineup_reminder': [
        { id: 'set_lineup', text: 'Set Lineup', url: '{{actionUrl}}' }
      ]
    };
  ...
  private getDefaultEmailHtml(type: NotificationType): string {
    return `
  ...
  private getDefaultVariables(type: NotificationType): TemplateVariable[] {
    return [
      {
        name: 'title',
        type: 'string',
  ...
  private getFallbackContent(type: NotificationType, field: string, data: Record<string, any>): string {
    if (field === 'title') {
