...
import { NotificationTemplate, NotificationChannel,
...
interface TemplateContext { 
  ...
  game? : { id: string, homeTeam, string,
    awayTeam, string,
    status, string,
    time?, string,
  }
...
interface TemplateProcessingResult { content: string,
...
export class TemplateEngine { private templates: Map<string, NotificationTemplate>  = new Map();
  ...
  r, arg }})
      ...
      return { content: variables, resolvedVariables, processingTime,
        errors
      }
    } catch (error) { 
  ...
  private setupVariableResolvers(): void {
    this.variableResolvers.set('currentTime', async (context)  => {  return new Date().toLocaleString('en-US', { timeZone: context.user.timezone 
       });
    });
  ...
  private setupFilters(): void {
    ...
    this.filters.set('date', (value, format = 'short') => {  const date = new Date(value);
      return date.toLocaleDateString('en-US', { dateStyle: format as any 
       });
    });

    this.filters.set('time', (value, format  = 'short') => {  const date = new Date(value);
      return date.toLocaleTimeString('en-US', { timeStyle: format as any 
       });
    });
