...
const DELIVERY_CONFIG: DeliveryConfig = {
  ...
  priority: {
    low: { maxDelay: 300000, retryAttempts: 1 }, // 5 minutes
    normal: { maxDelay: 180000, retryAttempts: 2 }, // 3 minutes
    high: { maxDelay: 60000, retryAttempts: 3 }, // 1 minute
    urgent: { maxDelay: 10000, retryAttempts: 5 }, // 10 seconds
    critical: { maxDelay: 5000, retryAttempts: 7 } // 5 seconds
  }
};
...
export class DeliveryManager extends EventEmitter {
  ...
  private metrics = {
    ...
    failedDeliveries: 0,
    averageLatency: 0,
    channelPerformance: new Map<NotificationChannel, {
      success: number;
      failure: number;
      avgLatency: number;
    }>()
  ...
  async initialize(): Promise<void> {
    const initPromises = Array.from(this.channels.values()).map(channel => 
      channel.initialize?.()
    );
  ...
  async deliver(notification: Notification): Promise<DeliveryResult[]> {
    ...
    try {
      ...
      if (this.config.enableFallbacks && failedChannels.length > 0 && successfulChannels.length === 0) {
        const fallbackResults = await this.handleFallbacks(notification, failedChannels);
      ...
      const errorResult: DeliveryResult = {
        ...
        timestamp: new Date().toISOString(),
        latency: Date.now() - startTime,
        error: error instanceof Error ? error.message : 'Delivery manager error'
      };
  ...
  private async deliverToChannel(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        // Add timeout wrapper
        const deliveryPromise = channelHandler.deliver(notification, {
          attempt,
          maxAttempts,
          deliveryId
        });
        ...
        lastError = error instanceof Error ? error.message : 'Delivery exception';
  ...
  private async handleFallbacks(
    ...
    const fallbackChannels = this.deliveryConfig.fallbackChannels.filter(
   ...
   */
  private optimizeChannelSelection(notification: Notification): NotificationChannel[] {
    ...
    if (notification.priority === 'urgent' || notification.priority === 'critical') {
      ...
      channels.sort((a, b) => {
        const urgentOrder = ['websocket', 'push', 'in_app', 'sms', 'email'];
      ...
      channels.sort((a, b) => {
        const lowOrder = ['in_app', 'email', 'websocket', 'push', 'sms'];
  ...
  private async checkRateLimit(userId: string): Promise<void> {
    ...
    if (!this.rateLimiter.has(userKey)) {
      this.rateLimiter.set(userKey, { count: 1, resetTime: now + windowMs });
      return;
    }
   ...
   */
  private updateMetrics(results: DeliveryResult[], totalLatency: number): void {
    ...
    results.forEach(result => {
      if (!this.metrics.channelPerformance.has(result.channel)) {
