...
export class EmailDelivery {
  ...
  async deliver(
    ...
    try {
      ...
      const emailEnabled = await this.isEmailEnabledForUser(
        notification.userId, 
        notification.type
      );
   ...
   */
  private async generateEmailContent(notification: Notification): Promise<EmailTemplate> {
    const template = this.config.templates[notification.type] || this.config.templates.default;
   ...
   * Get league name
   */
  private async getLeagueName(leagueId: string): Promise<string> {
    try {
      const result = await database.query(
   ...
   */
  private async isEmailEnabledForUser(userId: string, type: string): Promise<boolean> {
    try {
      const result = await database.query(`
  ...
  private async trackEmailDelivery(
    ...
    try {
      await database.query(`
        ...
        ON CONFLICT (notification_id, email) 
   ...
   * Send test email
   */
  async sendTestEmail(toEmail: string, userId: string): Promise<boolean> {
    try {
   ...
   */
  private getDefaultTemplates(): { [key: string]: EmailTemplate } {
    const baseTemplate = {
      html: `
        <!DOCTYPE html>
        <html>
