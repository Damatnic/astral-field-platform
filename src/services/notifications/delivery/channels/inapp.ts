...
export class InAppDelivery {
  ...
  async deliver(
    ...
    try {
      ...
      return {
        ...
        timestamp: new Date().toISOString(),
        latency: Date.now() - startTime,
        error: error instanceof Error ? error.message : 'In-app delivery error'
      };
    }
  }
  ...
  async getUserNotifications(
    ...
    const { unreadOnly = false, limit = 20, offset = 0, includeExpired = false } = options;
   ...
   */
  async clearOldNotifications(userId: string, daysOld: number = 7): Promise<number> {
    try {
      ...
      const result = await database.query(`
        DELETE FROM in_app_notifications 
        WHERE user_id = $1 
        AND (is_read = true OR expires_at < $2)
        AND created_at < $2
   ...
   */
  private async storeInAppNotification(notification: InAppNotification): Promise<void> {
    await database.query(`
      INSERT INTO in_app_notifications (
        id, user_id, title, message, type, priority, data, 
        action_url, actions, is_read, is_displayed, created_at, expires_at
   ...
   */
  private addToUserNotifications(userId: string, notification: InAppNotification): void {
   ...
   */
  private removeFromCache(userId: string, notificationId: string): void {
    const userNotifications = this.activeNotifications.get(userId);
    if (!userNotifications) return;
   ...
   */
  private cleanupUserCache(userId: string, cutoffDate: Date): void {
    const userNotifications = this.activeNotifications.get(userId);
    if (!userNotifications) return;
   ...
   */
  private mapDbRowToInAppNotification(row: any): InAppNotification {
    return {
      id: row.id,
      userId: row.user_id,
      title: row.title,
  ...
  private startCleanupJob(): void {
    ...
    setInterval(async () => {
      try {
        ...
        this.activeNotifications.forEach((notifications, userId) => {
          const now = new Date();
