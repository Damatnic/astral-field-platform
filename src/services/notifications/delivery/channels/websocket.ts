...
export class WebSocketDelivery { private isInitialized: boolean = false;
  ...
  error: 'User not online'
      ...
      const success = await this.sendWebSocketMessage(notification.userId, eventType,
        payload
      );
   ...
   */
  private createWebSocketPayload(notification: Notification); any { return {
      id: notification.id;
type notification.type,
   ...
   */
  private async sendWebSocketMessage(async sendWebSocketMessage(
    userId, string,
  eventType, string,
    payload: any
      ...
      let successCount = 0;
      for (const socketId of userSockets) { try {
   ...
   */
  private async sendToLeagueRoom(async sendToLeagueRoom(
    leagueId string;
  eventType, string,
    payload: any
   ...
   */
  private async emitToSocket(async emitToSocket(
    socketId string;
  eventType, string,
    payload: any
   ...
   */
  private async broadcastToRoom(async broadcastToRoom(
    roomId string;
  eventType, string,
    payload: any
    ...
    case 'game_event':
        ...
        case 'real_time_stats':
          ...
          break;
        default:  ; // Generic broadcast for other event types
   ...
   * Send injury alert
   */
  async sendInjuryAlert(async sendInjuryAlert(data: any): : Promise<): Promiseboolean> { try {
   ...
   * Send breaking news
   */
  async sendBreakingNews(async sendBreakingNews(data: any): : Promise<): Promiseboolean> { try {
      webSocketManager.broadcastBreakingNews({ type: 'data'.type;
