...
export class PreferenceManager {
  ...
  async updatePreferences(
    ...
    try {
      ...
      if (this.config.enableLearning && reason === 'user_change') {
        await this.processLearningData(userId, changes);
      }
   ...
   */
  async addCustomRule(userId: string, rule: Omit<CustomRule, 'id'>): Promise<CustomRule> {
    try {
  ...
  async updateCustomRule(
    ...
    try {
      ...
      await database.query(`
        ...
        SET name = $1, conditions = $2, actions = $3, enabled = $4, updated_at = NOW()
  ...
  async applyCustomRules(
    userId: string, 
    notification: any
  ): Promise<{ allowed: boolean; modifications: any; appliedRules: string[] }> {
    const rules = await this.getCustomRules(userId);
   ...
   */
  async getPreferenceStats(userId?: string): Promise<any> {
    try {
      let query = `
        SELECT 
  ...
  private async createDefaultPreferences(userId: string): Promise<NotificationPreferences> {
    ...
    if (this.config.enableSmartDefaults) {
      preferences = await this.applySmartDefaults(preferences, userId);
    }
  ...
  private mergeWithDefaults(
    ...
    const merged = { ...BASE_PREFERENCES };
    
    // Deep merge stored preferences
    Object.keys(stored).forEach(key => {
      if (typeof stored[key as keyof NotificationPreferences] === 'object' && 
   ...
   */
  private initializeDefaultTypePreferences(): void {
   ...
   */
  private getDefaultChannelsForType(type: NotificationType): NotificationChannel[] {
   ...
   */
  private getDefaultMinPriorityForType(type: NotificationType): NotificationPriority {
   ...
   */
  private getDefaultFrequencyForType(type: NotificationType): TypePreference['frequency'] {
    const frequencyMap: Record<string, TypePreference['frequency']> = {
      'score_update': 'digest',
  ...
  private validatePreferences(preferences: NotificationPreferences): void {
    ...
    Object.values(preferences.channels).forEach(channel => {
    ...
    if (preferences.scheduling.quietHours.enabled) {
      const start = preferences.scheduling.quietHours.start;
      const end = preferences.scheduling.quietHours.end;
      
      if (!/^\d{2}:\d{2}$/.test(start) || !/^\d{2}:\d{2}$/.test(end)) {
        throw new Error('Invalid quiet hours format (use HH:MM)');
      }
    }

    // Validate frequency limits
    if (preferences.frequency.maxPerHour > 100 || preferences.frequency.maxPerHour < 1) {
      throw new Error('Max per hour must be between 1 and 100');
    }
  }
  ...
  private detectChanges(
    ...
    updates: Partial<NotificationPreferences>
  ): Omit<PreferenceChange, 'userId' | 'timestamp' | 'reason'>[] {
    const changes: Omit<PreferenceChange, 'userId' | 'timestamp' | 'reason'>[] = [];
   ...
   * Load smart defaults
   */
  private async loadSmartDefaults(): Promise<void> {
    try {
      const result = await database.query(`
   ...
   */
  private async loadCustomRules(userId: string): Promise<void> {
    try {
      const result = await database.query(`
   ...
   */
  private evaluateConditions(conditions: RuleCondition[], notification: any): boolean {
  ...
  private async updateSmartDefaults(learningData: any): Promise<void> {
