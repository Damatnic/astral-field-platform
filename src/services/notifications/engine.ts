...
export class NotificationEngine extends EventEmitter {
  ...
  async createNotification(input: CreateNotificationInput): Promise<string> {
    ...
    try {
      ...
      if (this.config.aiFilteringEnabled) {
        const aiContext = await this.buildAIContext(notification, preferences);
      ...
      if (this.config.analyticsEnabled) {
        await this.analytics.trackNotificationCreated(notification);
      }
   ...
   */
  async sendImmediate(notificationId: string): Promise<DeliveryResult[]> {
    try {
      ...
      // Track delivery
      if (this.config.analyticsEnabled) {
        await this.analytics.trackDelivery(notification, results);
      }
  ...
  async markAsRead(notificationId: string, userId: string): Promise<void> {
    try {
      ...
      // Track engagement
      if (this.config.analyticsEnabled) {
        await this.analytics.trackEngagement(notification, 'read');
      }
  ...
  async trackClick(notificationId: string, userId: string, action?: string): Promise<void> {
    try {
      ...
      // Track click
      if (this.config.analyticsEnabled) {
        await this.analytics.trackEngagement(notification, 'click', { action });
      }
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      const notifications = await this.queue.dequeue(this.config.batchSize);
      ...
      await this.handleError({
        ...
        channel: 'all',
        type: 'system',
        message: error instanceof Error ? error.message : 'Queue processing error',
        retryable: true,
        timestamp: new Date().toISOString()
      });
    }
  ...
  private async processNotification(notification: Notification): Promise<void> {
    ...
    try {
      ...
      const processingTime = Date.now() - startTime;
      this.updateMetrics({ 
        averageLatency: (this.metrics.averageLatency + processingTime) / 2,
  ...
  private emitEvent(
    ...
    this.listeners.forEach(listener => {
      if (listener.events.includes(type)) {
        try {
  ...
  private async handleFilteredNotification(notification: Notification, reason: string): Promise<void> {
    await this.updateNotificationStatus(notification.id, 'failed');
  ...
  private async storeNotification(notification: Notification): Promise<void> {
    const query = `
      INSERT INTO notifications (
        id, type, title, message, short_message, user_id, league_id, team_id, player_id,
        priority, channels, trigger_type, status, data, scheduled_at, expires_at,
  ...
  private async handleError(error: NotificationError): Promise<void> {
    console.error('Notification Error:', error);
    
    // Store error for analysis
    await database.query(`
      INSERT INTO notification_errors (id, notification_id, channel, type, message, retryable, created_at)
