...
export class AIFilter {
   ...
   */
  async shouldDeliver(notification: Notification, context: AIContext): Promise<AIDecision> {
    if (!this.config.enabled) {
      return {
        deliver: true,
        confidence: 1.0,
    ...
    try {
      ...
      if (this.config.learningEnabled) {
        await this.storeLearningData(userId, notification, context, decision);
      }
   ...
   */
  private async detectSpam(notification: Notification, context: AIContext): Promise<SpamDetection> {
    ...
    // Repetitive content
    const recentSimilar = await this.findSimilarRecentNotifications(
      context.user.id, 
   ...
   */
  private async checkFrequency(userId: string, notificationType: string): Promise<{ allowed: boolean; reason: string }> {
    try {
      ...
      return { allowed: true, reason: 'Frequency check error - defaulting to allow' };
    }
  }
  ...
  private async predictEngagement(
    ...
    let prediction = userInsight.engagementScore;
  ...
  private async generateOptimizations(
    ...
    context: AIContext,
    userInsight: UserInsight
  ): Promise<AIDecision['optimizations']> {
    const optimizations: AIDecision['optimizations'] = {};

    // Channel optimization
    if (this.config.channelOptimization) {
      const optimalChannels = this.getOptimalChannels(notification, userInsight);
    ...
    // Timing optimization
    if (this.config.personalizedTiming) {
      const optimalTiming = this.getOptimalTiming(userInsight, context);
      if (optimalTiming) {
    ...
    if (this.config.contentOptimization) {
      const optimizedContent = await this.optimizeContent(notification, userInsight);
   ...
   */
  private async getUserInsight(userId: string): Promise<UserInsight> {
    // Check cache first
   ...
   */
  private async getOptimalTimes(userId: string): Promise<TimeSlot[]> {
    try {
      const result = await database.query(`
        SELECT 
  ...
  private async scoreContentRelevance(
    ...
    if (notification.playerId && context.environment.gameDay) {
      score += 0.3;
    }
   ...
   */
  private scoreTimingRelevance(context: AIContext, userInsight: UserInsight): number {
   ...
   */
  private scoreContextRelevance(notification: Notification, context: AIContext): number {
    let score = 0.5;
    ...
    if (context.environment.userOnline && 
        ['websocket', 'in_app'].some(ch => notification.channels.includes(ch))) {
      score += 0.2;
    }
   ...
   */
  private getOptimalTiming(userInsight: UserInsight, context: AIContext): string | undefined {
    if (userInsight.optimalTimes.length === 0) {
      return undefined;
    }
    ...
    for (const slot of userInsight.optimalTimes) {
   ...
   * Helper methods
   */
  private async getTypeEngagementRate(userId: string, type: string): Promise<number> {
    try {
      const result = await database.query(`
        SELECT 
          COUNT(*) as total,
  ...
  private getPriorityEngagementMultiplier(priority: NotificationPriority): number {
    const multipliers: Record<NotificationPriority, number> = {
      critical: 1.5,
      urgent: 1.3,
      high: 1.1,
      normal: 1.0,
  ...
  private getTypeLimitPerHour(type: string): number {
  ...
  private async findSimilarRecentNotifications(
    userId: string, 
    message: string, 
    hours: number
  ): Promise<number> {
  ...
  private startLearningProcess(): void {
    ...
    // 1. Periodically analyzing user behavior
