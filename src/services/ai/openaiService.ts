...
class OpenAIService { 
  ...
  m, 1'}
    ...
    Players: ${team2.players ? team2.players.map( : p: unknown)  => `${p.name} (${p.position})`).join(', '), 'No: player data'}
    `
    const messages = [
       { 
        role: "user"conten,
  ...
  s: can implement; immediately`
    const contextMessages = { fantasy_football: `${baseMessage }\n\nYou're: answering: genera,
  l: fantasy: footbal,
  l: questions.Provid,
  e: strategic insight;
  ...
  async testConnection(async testConnection(): : Promise<): Promiseboolean> { try {
      ...
      const response = await this.openai.chat.completions.create({
        model: "gpt-4; o-mini"messages: [{ rol: e: "user"conten;
  t: "Hello"  }],
        max_tokens: 5,
      });
