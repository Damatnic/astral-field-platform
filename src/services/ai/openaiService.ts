...
class OpenAIService {
  ...
  async getChatCompletion(messages: any[], context?: string): Promise<string> {
    if (!this.openai) {
  ...
  async getFantasyAdvice(question: string, playerContext?: any, leagueContext?: any): Promise<string> {
    const contextInfo = this.buildFantasyContext(playerContext, leagueContext);
  ...
  async analyzeMatchup(team1: any, team2: any, week: number): Promise<string> {
    const matchupContext = `
    Week ${week} Matchup Analysis:
    
    Team 1: ${team1.name || 'Team 1'}
    Players: ${team1.players ? team1.players.map((p: any) => `${p.name} (${p.position})`).join(', ') : 'No player data'}
    
    Team 2: ${team2.name || 'Team 2'}
    Players: ${team2.players ? team2.players.map((p: any) => `${p.name} (${p.position})`).join(', ') : 'No player data'}
    `;

    const messages = [
      {
        role: "user", 
        content: `Please analyze this fantasy football matchup and provide strategic insights, key player advantages, and a prediction. ${matchupContext}`
      }
    ];
  ...
  async getWaiverTargets(availablePlayers: any[], userRoster: any[], leagueSettings: any): Promise<string> {
    const waiverContext = `
    Available Players: ${availablePlayers.slice(0, 10).map(p => `${p.name} (${p.position}) - ${p.nfl_team}`).join(', ')}
    ...
    const messages = [
      {
        role: "user",
        content: `Based on my current roster and available players, what are the best waiver wire targets this week? Focus on players who can improve my team. ${waiverContext}`
      }
    ];
  ...
  async optimizeLineup(roster: any[], week: number): Promise<string> {
    const rosterInfo = roster.map(player => 
    ...
    const messages = [
      {
        role: "user",
        content: `Help me optimize my fantasy football lineup for Week ${week}. Here's my roster:\n\n${rosterInfo}\n\nWho should I start and why? Consider matchups, injuries, and projections.`
      }
    ];
  ...
  private buildFantasyContext(playerContext?: any, leagueContext?: any): string {
    let context = '';
