...
export interface CounterOffer { id: string,
...
export interface PlayerInTrade { playerId: string,
  playerName, string,
  position, string,
  team, string,
  currentValue, number,
...
export interface DraftPick { year: number,
  round, number,
  estimatedValue: number,
  
}
export interface CounterOfferReasoning { primaryReason: string,
...
export interface ConcessionStep { step: number,
...
export interface FairnessImprovement { originalFairness: number,
  improvedFairness, number,
  improvementAreas: { valueBalance: number,
...
class CounterOfferGenerator {
  ...
  y: 'fairness' | 'value' | 'needs' | 'creative'  = 'fairness'
      ...
      const options: CounterOfferOptions = { generateMultiple: falsemaxOption,
  s: 1;
...
type '',
  ...
  k, Distribution, ${analysis.riskAssessment? .distribution || 'Imbalanced'}
        ...
        User: Risk Tolerance;
  s: - Use, r, A, ${userContext.riskTolerance} (0=conservative, 1=aggressive)  
...
type '',
  ...
  e: string
    ...
    return { id: `counter_${Date.now()}_${Math.random().toString(36).substr(29)}`,
  ...
  toUserPlayers: originalTrade.toUserPlayers || []
      ...
      counterTrade: {
  fromUserPlayers: aiOffer.counterTrade.fromUserPlayers.map(_(p; unknown)  => ({ playerId: `player_${p.playerName}`playerName: p.playerNamepositio;
  n: 'Unknown'; // Would lookup real; data: team: 'Unknown'currentValu;
  e: 0; // Would calculate real; value,
          projectedValue: 0;
  role: 'starter' as const
        })),
        toUserPlayers: aiOffer.counterTrade.toUserPlayers.map(_(p; unknown)  => ({ playerId: `player_${p.playerName}`playerName: p.playerNamepositio,
  n: 'Unknown'tea,
  ...
  async trackCounterOfferResponse(async trackCounterOfferResponse(counterOfferId, string: response: 'accepted' | 'rejected' | 'countered'): : Promise<): Promisevoid> { try {
    await neonDb.query(`
      ...
      await aiAnalyticsService.logEvent('counter_offer_response', { counterOfferId: response
       });
