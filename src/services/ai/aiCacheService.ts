...
class AICacheService {
  ...
  // Enhanced cache key generation with semantic analysis
  generateCacheKey(request: AIRequest): string {
    ...
    if (request.context) {
      ...
      keyComponents.push(`ctx:${contextHash.substring(0, 8)}`)
    }

    return `ai:${keyComponents.join(':')}:${contentHash.substring(0, 12)}`
  }

  // Semantic similarity-based cache lookup
  async findSimilarCachedResponse(request: AIRequest): Promise<AIResponse | null> {
    const primaryKey = this.generateCacheKey(request)
  ...
  async store(request: AIRequest, response: AIResponse): Promise<void> {
  ...
  async get(key: string): Promise<AIResponse | null> {
    ...
    // Track analytics
    this.hitCount++
    this.totalCostSavings += entry.response.actualCost
    ...
    const cachedResponse: AIResponse = {
      ...entry.response,
      cached: true,
      latency: 50 // Cache retrieval latency
    }
  ...
  async invalidate(patterns: {
    ...
    let invalidated = 0
    const cacheKeys = MemoryCache.getStats().keys
    ...
    for (const key of cacheKeys) {
      ...
      if (patterns.capabilities) {
        const keyCapabilities = key.split(':')[3] // Extract capabilities from key
  ...
  getAnalytics(): CacheStats {
    ...
    const popularQueries = Array.from(this.accessLog.entries())
      .map(([key, hits]) => {
        const entry = MemoryCache.get<CacheEntry>(key)
        return {
    ...
    const estimatedMemoryUsage = cacheKeys.length * 2048 // Rough estimate: 2KB per entry
  ...
  async optimizeCache(): Promise<{
    ...
    const beforeCount = MemoryCache.getStats().keys.filter(k => k.startsWith('ai:')).length
    ...
    for (const key of cacheKeys) {
      ...
      const daysSinceAccessed = (now.getTime() - new Date(entry.lastAccessed).getTime()) / (1000 * 60 * 60 * 24)
      ...
      if (shouldRemove) {
        costSavingsLost += entry.costSavings
        MemoryCache.delete(key)
      }
    }

    const afterCount = MemoryCache.getStats().keys.filter(k => k.startsWith('ai:')).length
    const entriesRemoved = beforeCount - afterCount
    const memoryFreed = entriesRemoved * 2048 // Estimate
  ...
  private hashContent(messages: Array<{role: string, content: string}>): string {
  ...
  private async findSemanticMatch(request: AIRequest): Promise<AIResponse | null> {
    const requestSemanticKey = this.generateSemanticKey(request)
    const cacheKeys = MemoryCache.getStats().keys.filter(k => k.startsWith('ai:'))
    ...
    for (const key of cacheKeys) {
      ...
      const similarity = this.calculateSemantic(requestSemanticKey, entry.semanticKey)
      if (similarity > 0.8) { // 80% similarity threshold
        ...
        return {
          ...entry.response,
          cached: true,
          confidence: Math.round(entry.response.confidence * similarity) // Adjust confidence based on similarity
        }
      }
    }
  ...
  private calculateSemantic(key1: string, key2: string): number {
  ...
  private async updateSemanticIndex(semanticKey: string, cacheKey: string): Promise<void> {
  ...
  private determineTTL(request: AIRequest | null, response: AIResponse): number {
    ...
    // Complex queries get longer TTL
    ...
    // General queries get shorter TTL
    return CacheDurations.MEDIUM
  }
