...
export interface CacheEntry { response: AIResponse,
  hash, string,
  semanticKey, string,
  contextHash, strin,
  g: userId? ; string : createdAt, string,
  accessCount, number,
  lastAccessed, string,
  costSavings, number,
...
export interface CacheStats { totalEntries: number,
  ...
  popularQueries: Array<{ query: string,
  hits, number,
  savings: number,
   }
>
...
class AICacheService {
  ...
  async store(async store(request, AIRequestrespons: e: AIResponse): : Promise<): Promisevoid> {const key = this.generateCacheKey(request)
    ...
    const entry: CacheEntry = { response: hash,
  ...
  n: async optimizeCache(async optimizeCache(): : Promise<): Promise  { entriesRemoved: number,
  memoryFreed, number,
  ...
  t: const memoryFreed = entriesRemoved * 2048 ; // Estimate; return { entriesRemoved: memoryFreed,
