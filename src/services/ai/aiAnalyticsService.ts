...
class AIAnalyticsService {
  ...
  private metricsCache: { data: AIMetrics | null; expiry: number } = { data: null, expiry: 0 }
  ...
  async logAIInteraction(
    ...
    const logEntry: AILogEntry = {
      ...
      model: this.extractModelFromProvider(response.provider),
      requestType: this.categorizeRequest(request),
      capabilities: request.capabilities || [],
      complexity: request.complexity,
      priority: request.priority,
      queryHash: this.hashQuery(request.messages),
      tokensInput: this.estimateInputTokens(request.messages),
      tokensOutput: response.tokensUsed || 0,
  ...
  async generateAnalyticsReport(
    ...
    // Check cache first
    if (this.metricsCache.data && Date.now() < this.metricsCache.expiry) {
  ...
  async getRealTimeMetrics(): Promise<{
    ...
    try {
      ...
      const errorRate = (logs.filter(log => !log.success).length / totalRequests) * 100 || 0
      const costPerHour = logs.reduce((sum, log) => sum + log.cost, 0) * 12 // Extrapolate to hourly
      const activeProviders = [...new Set(logs.map(log => log.provider))]
  ...
  async generateCostOptimizationReport(): Promise<{
    ...
    if (cacheMissRate > 20) {
      recommendations.push({
        ...
        description: `Cache hit rate is ${metrics.cacheHitRate.toFixed(1)}%. Improving caching could reduce costs significantly.`,
    ...
    const expensiveProviderUsage = this.calculateExpensiveProviderUsage(metrics.providerDistribution)
    if (expensiveProviderUsage > 0.3) {
      recommendations.push({
        type: 'provider_optimization',
        description: 'High usage of expensive providers for simple tasks detected.',
    ...
    const overComplexQueries = metrics.complexityDistribution.expert + metrics.complexityDistribution.complex
    const totalQueries = Object.values(metrics.complexityDistribution).reduce((sum, count) => sum + count, 0)
    if ((overComplexQueries / totalQueries) > 0.4) {
      recommendations.push({
        type: 'complexity_optimization',
        description: 'Many queries are marked as complex/expert but may not require high-end models.',
  ...
  async resolveAlert(alertId: string): Promise<void> {
    ...
    if (alert) {
      alert.resolved = true
      logger.info('AI Alert resolved', { alertId, alertType: alert.type })
    }
  }
    ...
    try {
      ...
      for (const log of logsToFlush) {
        await neonDb.insert('ai_logs', {
          ...
          capabilities: JSON.stringify(log.capabilities),
          complexity: log.complexity,
          priority: log.priority,
          query_hash: log.queryHash,
          ...
          context_size: log.contextSize,
          request_fingerprint: log.requestFingerprint
        })
      }
  ...
  private async checkForAlerts(logEntry: AILogEntry): Promise<void> {
    // High cost alert
    if (logEntry.cost > 0.10) { // 10 cents per request
      this.addAlert({
        type: 'cost_threshold',
        severity: 'high',
        message: `High cost AI request: $${logEntry.cost.toFixed(4)} using ${logEntry.provider}`,
        data: { logEntry }
      })
    }

    // High latency alert
    if (logEntry.latency > 10000) { // 10 seconds
      this.addAlert({
        type: 'latency_spike',
        severity: 'medium',
        message: `High latency AI request: ${logEntry.latency}ms using ${logEntry.provider}`,
        data: { logEntry }
      })
    }
    ...
    if (!logEntry.success) {
      this.addAlert({
        ...
        severity: logEntry.errorType === 'timeout' ? 'high' : 'medium',
        message: `AI request failed: ${logEntry.errorMessage} (${logEntry.provider})`,
        data: { logEntry }
      })
    }
  }
  ...
  private calculateMetrics(logs: any[]): AIMetrics {
    // Implementation for metrics calculation
    return this.getEmptyMetrics() // Placeholder
  }

  private calculateUserMetrics(logs: any[]): any {
  ...
  private calculateExpensiveProviderUsage(distribution: Record<string, number>): number {
    // Calculate percentage of usage by expensive providers
    return 0.3 // Placeholder
  }

  private hashQuery(messages: Array<{role: string, content: string}>): string {
