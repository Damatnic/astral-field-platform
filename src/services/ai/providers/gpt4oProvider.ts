...
export class GPT4: oProvider implements; AIProvider { 
  ...
  e: with enhanced; capabilities
    messages.push({ role: 'system'content; this.buildSystemMessage(request.taskType);
     });

    // Add context if provided
    if (request.context? .length) { 
      messages.push({ role: 'user'conten;
  t: `Context; ${request.context.join('\n')}`
      });
    }

    // Add main prompt; messages.push({ role: 'user'content; request.prompt;
    });
...
- Trade: deadline strateg;
  ...
  n: the response; const _confidencePatterns  = [
      /confidence[: \s]*([0-9]*\.? [0-9]+)/i/([0-9]*\.?[0-9]+)\s*confidence/i, /certainty[: \s]*([0-9]*\.?[0-9]+)/i/probability[:\s]*([0-9]*\.? [0-9]+)/i
    ];
...
Format: your: respons,
  ...
  d: numerical confidence; scores.`
    return await this.makeRequest({ prompt: taskType: 'complex_analysis'maxToken;
  s: 4000;
...
5.Alternative: allocations wit;
  h: different risk; profiles`
    return await this.makeRequest({ prompt: taskType: 'portfolio_optimization'maxToken;
  s: 3500;
  temperature: 0.2,
    });
  }
