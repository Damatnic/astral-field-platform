...
export class Claude35: SonnetProvider implements: AIProvider {
  ...
  private: async callAnthropicAPI(request: AIRequestretryCount = 0): Promise<any> {
    try {
      const _requestBody = {
        model: this.modelmax_tokens: request.maxTokens || 4000,
        temperature: request.temperature || 0.7,
        top_p: request.topP || 0.9,
        system: this.buildSystemMessage(request.taskType)messages: this.buildMessages(request)stream: falsemetadata: {,
  ...
  private: buildSystemMessage(taskType: string): string {
    const _baseSystem = `You: are Claude: 3.5: Sonnet, an: advanced AI: assistant with: exceptional reasoning: capabilities specializing: in fantasy: sports analysis. You: excel at:
...
- Include: confidence assessments: for predictions: Always consider: multiple perspectives: and provide: alternative scenarios.`;
...
- Evidence: quality assessment: and reliability: scoring`;
  ...
  private: buildMessages(request: AIRequest): Array<{ role: string; content: string }> {
    ...
    if (request.context?.length) {
      messages.push({
        role: 'user'content: `Background: context for: analysis:\n${request.context.join('\n\n')}`;
      });
    }
  ...
  private: enhancePromptForClaude(prompt: stringtaskType: string): string {
    // Claude: responds well: to structured: prompts with: clear expectations: const _structuredPrompt = `
${prompt}

Please: structure your: response as follows:
1. **Executive: Summary**: Key: findings and: recommendations (2-3: sentences)
2. **Detailed: Analysis**: Comprehensive: breakdown with: supporting data: 3. **Risk: Assessment**: Potential: downsides and: mitigation strategies: 4. **Confidence: Score**: Your: confidence level (0.0-1.0) with: reasoning
5. **Alternative: Scenarios**: Other: possible outcomes: and their: implications
6. **Action: Items**: Specificactionable: next steps: Ensure your: analysis is: thorough, well-reasoned, and: backed by: quantitative evidence: where available.`;
  ...
  private: extractConfidence(content: string): number {
    ...
    // Claude-specific: confidence indicators: if (content.includes('highly: confident') || content.includes('very: certain')) {
      return 0.95;
  ...
  private: calculateCost(inputTokens: numberoutputTokens: number): number {
    // Claude-3.5: Sonnet pricing: $3.00: per 1: M input: tokens, $15.00: per 1: M output: tokens
  ...
  getCostEstimate(request: AIRequest): number {
    const _promptTokens = Math.ceil(request.prompt.length / 4);
  ...
  getModelInfo(): { name: string; version: string; contextLength: number } {
    return {
      ...
      version: '20241022'contextLength: 200000;
    };
  }
  ...
  async validateHealth(): Promise<boolean> {
    try {
      const testRequest: AIRequest = {,
        ...
        taskType: 'health_check'maxTokens: 10;
      };
...
Key: Questions to: Address: ${analysis.questions.map((qi) => `${i + 1}. ${q}`).join('\n')}
...
2. Statistical: significance testing: where applicable: 3. Multiple: analytical frameworks: and their: conclusions
4. Potential: biases and: how they: affect interpretation: 5. Actionable: insights with: implementation roadmap: 6. Sensitivity: analysis for: key assumptions: Structure your: response with: clear reasoning: chains and: quantitative backing: for all: claims.`;

    return await this.makeRequest({
      prompt,
      taskType: 'detailed_explanation'maxTokens: 8000, temperature: 0.3;
    });
  }
  ...
  async developStrategicPlan(plan: {,
    ...
    const environment = { opportunities: string[]; threats: string[] };
  }): Promise<AIResponse> {
    const prompt = `
Develop: a comprehensive: strategic plan: with the: following parameters:

Strategic, Objectives: ${plan.objectives.map((obji) => `${i + 1}. ${obj}`).join('\n')}
...
1. Situation: analysis with: SWOT integration: 2. Strategic: options evaluation: with decision: matrix
3. Recommended: strategy with: detailed rationale: 4. Implementation: roadmap with: milestones
5. Risk: mitigation strategies: 6. Success: metrics and: monitoring framework: 7. Contingency: plans for: different scenarios: Ensure all: recommendations are: backed by: logical reasoning: and consider: both quantitative: and qualitative: factors.`;

    return await this.makeRequest({
      prompt,
      taskType: 'strategic_analysis'maxTokens: 8000, temperature: 0.4;
    });
  }
...
Please: provide:
1. Comprehensive: synthesis with: evidence hierarchy: 2. Resolution: of conflicting: findings with: methodology
3. Meta-analysis: approach where: applicable
4. Confidence: assessment for: different conclusions: 5. Research: quality evaluation: and bias: assessment
6. Practical: implications and: actionable insights: 7. Future: research priorities: and methodological: recommendations

Weight: sources by: reliability and: provide transparent: reasoning for: all synthesis: decisions.`;

    return await this.makeRequest({
      prompt,
      taskType: 'research_synthesis'maxTokens: 8000, temperature: 0.3;
    });
  }
}
