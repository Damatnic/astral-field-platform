...
export class UserBehaviorAnalysisService {
  ...
  public async analyzeUserBehavior(userId: string, leagueId: string): Promise<UserBehaviorProfile> {
  ...
  public async generateBehaviorProfile(userId: string, leagueId: string): Promise<UserBehaviorProfile> {
  ...
  private async getUserActivityData(client: any, userId: string, leagueId: string): Promise<any> {
    ...
    return {
      ...
      lastAction: rows[0]?.last_action,
      avgSessionGap: parseFloat(rows[0]?.avg_session_gap || 86400),
      weeklyActivity: weeklyActivity.rows
    };
  }

  private async getTradingData(client: any, userId: string, leagueId: string): Promise<any> {
    const { rows } = await client.query(`
      SELECT 
      ...
      FROM trades
      WHERE (proposer_id = $1 OR recipient_id = $1) AND league_id = $2
        AND created_at > NOW() - INTERVAL '30 days'
    `, [userId, leagueId]);
  ...
  private async getRosterManagementData(client: any, userId: string, leagueId: string): Promise<any> {
    const { rows } = await client.query(`
      SELECT 
  ...
  private async getCommunicationData(client: any, userId: string, leagueId: string): Promise<any> {
    ...
    return {
      ...
      avgResponseTime: Math.random() * 24 * 60 * 60 // seconds
    };
  }

  private calculateActivityLevel(activityData: any): 'low' | 'moderate' | 'high' | 'very_high' {
  ...
  private calculateEngagementScore(activityData: any, tradingData: any, rosterData: any): number {
    const activityScore = Math.min(activityData.totalActions / 100, 1) * 0.4;
    const tradingScore = Math.min(tradingData.totalTrades / 10, 1) * 0.3;
    const rosterScore = Math.min(rosterData.totalMoves / 20, 1) * 0.3;
    
    return Math.round((activityScore + tradingScore + rosterScore) * 100) / 100;
  }

  private categorizeTradingActivity(tradingData: any): 'inactive' | 'light' | 'moderate' | 'heavy' {
  ...
  private categorizeRosterManagement(rosterData: any): 'neglected' | 'occasional' | 'active' | 'obsessive' {
  ...
  private categorizeCommunicationLevel(communicationData: any): 'silent' | 'occasional' | 'active' | 'chatty' {
  ...
  private calculateCompetitiveness(activityData: any, tradingData: any): number {
    // Competitiveness based on activity frequency and trading aggressiveness
    const activityFactor = Math.min(activityData.totalActions / 100, 1);
    const tradingFactor = Math.min(tradingData.totalTrades / 15, 1);
    
    return Math.round((activityFactor * 0.6 + tradingFactor * 0.4) * 100) / 100;
  }

  private calculateReliability(activityData: any): number {
    // Reliability based on consistency of activity
    const { activeDays, avgSessionGap } = activityData;
    const consistencyFactor = Math.min(activeDays / 30, 1);
    const sessionGapFactor = Math.max(0, 1 - (avgSessionGap / (7 * 24 * 3600))); // Weekly gap = 0.5
    
    return Math.round((consistencyFactor * 0.7 + sessionGapFactor * 0.3) * 100) / 100;
  }

  private async calculateBehaviorTrends(client: any, userId: string, leagueId: string): Promise<any> {
    const { rows } = await client.query(`
      SELECT 
        EXTRACT(WEEK FROM created_at) as week,
        COUNT(*) as activity
    ...
    const avgResponseTime = Math.random() * 24; // Mock data
  ...
  private async performAIBehaviorAnalysis(userId: string, leagueId: string, data: any): Promise<any> {
    try {
      const analysisRequest = {
        ...
        Roster Management: ${data.rosterData.totalMoves} moves
        Communication: ${data.communicationData.messagesSent} messages sent`,
        type: 'analysis' as const,
        userId,
        leagueId,
        context: { behaviorAnalysis: true }
      };
      ...
      if (aiResponse.success && aiResponse.data) {
        return {
          riskFactors: aiResponse.data.insights?.slice(0, 3) || [],
          strengths: aiResponse.data.insights?.slice(3, 6) || []
        };
      }
    } catch (error) {
  ...
  public async getUserBehaviorProfiles(leagueId: string): Promise<UserBehaviorProfile[]> {
    ...
    try {
      ...
      return rows.map(row => ({
        ...
        tradingActivity: row.trading_activity,
        rosterManagement: row.roster_management,
        communicationLevel: row.communication_level,
        competitiveness: row.competitiveness,
        reliability: row.reliability,
        lastActivity: row.updated_at,
        behaviorTrends: JSON.parse(row.behavior_trends || '{}'),
