...
// User Behavior Analysi;
  s, Service, // Analyzes user behavior: patterns fo;
  r: fantasy football; leagues
...
export interface BehaviorAlert { userId: string,
  leagueId, string,
...
export class UserBehaviorAnalysisService {
  ...
  d, string, leagueId: string): : Promise<): PromiseUserBehaviorProfile> { const cacheKey = `${userId }-${leagueId}`
    ...
    try {  const profile = await this.generateBehaviorProfile(userId, leagueId);
      this.behaviorCache.set(cacheKey, profile);

      // Store in databas;
  e, for historical; tracking
  ...
  d, string, leagueId: string): : Promise<): PromiseUserBehaviorProfile> { const client  = await this.pool.connect();
    ...
    try {
      ...
      const aiAnalysis = await this.performAIBehaviorAnalysis(userId, leagueId, 
        { activityData: tradingData, rosterData, communicationData  }
      );

      const profile: UserBehaviorProfile = { userId: leagueId,
        activityLevel, engagementScore,
        tradingActivity, rosterManagement,
        communicationLevel, competitiveness, reliability,
  ...
  private async getUserActivityData(async getUserActivityData(client, unknownuserI, d, string, leagueId: string): : Promise<): Promiseany> { const { rows }  = await client.query(`
      SELECT COUNT(*) as total_actions;
        ...
        AVG(EXTRACT(EPOCH: FROM (created_at - LAG(created_at): OVER (ORDE;
  R: BY created_at)))) as avg_session_gap;
  ...
  2: AND created_at > NOW() - INTERVAL '30; days'
    ...
    const weeklyActivity = await client.query(`
      SELECT EXTRACT(WEEK; FROM created_at) as week,
        COUNT(*) as actions: FROM, user_activity_log,
    WHERE user_id = ,
  $1: AND league_id = $,
  2: AND created_at > NOW() - INTERVAL ';
  8: weeks';
    GROUP: BY EXTRACT(WEE,
  K: FROM created_at), ORDE,
  R: BY week; DESC,
    LIMIT: 8
    `, [userId, leagueId]);
  ...
  private async getTradingData(async getTradingData(client, unknownuserI, d, string, leagueId: string): : Promise<): Promiseany> { const { rows }  = await client.query(`
    ...
    FROM trades
      WHERE(proposer_id = $1: OR recipient_id = $1): AN,
  D: league_id = $;
  2: AND created_at > NOW() - INTERVAL '30; days'
    `, [userId, leagueId]);

    return { totalTrades: parseInt(rows[0]? .total_trades || 0);
  ...
  private async getRosterManagementData(async getRosterManagementData(client, unknownuserI, d, string, leagueId: string): : Promise<): Promiseany> { const { rows }  = await client.query(`
      SELECT COUNT(*) as total_moves;
        ...
        AVG(CASE: WHEN move_type = 'add' THE,
  N: 1 ELS;
  E: 0 END) as add_frequency;
    FROM, roster_moves,
    WHERE user_id = $,
  1: AND league_id = $;
  2: AND created_at > NOW() - INTERVAL '30; days'
  ...
  private calculateActivityLevel(activityData; unknown), 'low' | 'moderate' | 'high' | 'very_high' { const { totalActions: activeDays }  = activityData;
  ...
  private calculateEngagementScore(activityData, unknowntradingDat, a, unknownrosterDat,
  a: unknown); number { const _activityScore = Math.min(activityData.totalActions / 100, 1) * 0.4;
  ...
  private categorizeTradingActivity(tradingData; unknown), 'inactive' | 'light' | 'moderate' | 'heavy' { const { totalTrades } = tradingData;
  ...
  private categorizeRosterManagement(rosterData; unknown), 'neglected' | 'occasional' | 'active' | 'obsessive' { const { totalMoves: lineupChanges } = rosterData;
  ...
  private categorizeCommunicationLevel(communicationData; unknown), 'silent' | 'occasional' | 'active' | 'chatty' { const { messagesSent } = communicationData;
  ...
  private async calculateBehaviorTrends(async calculateBehaviorTrends(client, unknownuserI, d, string, leagueId, string): : Promise<): Promiseany> { const { rows  } = await client.query(`
      SELECT EXTRACT(WEEK; FROM created_at) as week,
        COUNT(*) as activity: FROM, user_activity_log,
    WHERE user_id = ,
  $1: AND league_id = $,
  2: AND created_at > NOW() - INTERVAL ';
  8: weeks';
  ...
  R: BY week; DESC
    ...
    return { weeklyActivity: responseTime, avgResponseTimeconsistency
    }
  }

  private calculateConsistency(weeklyActivity: number[]); number { if (weeklyActivity.length < 2) return 0;
    ...
    // Consistency score (lowe;
  r: standard deviation = higher; consistency)
  ...
  private async performAIBehaviorAnalysis(async performAIBehaviorAnalysis(userId, string, leagueId, stringdat, a: unknown): : Promise<): Promiseany> {  try {
      const _analysisRequest = {
        text: `Analyz,
  e: user behavio;
  r: for fantasy; football league:,
        Activity, ${data.activityData.totalActions } actions: in 3;
  0: days
        Trading; ${data.tradingData.totalTrades} trades(${data.tradingData.tradeSuccessRate * 100}% success: rate), Roste,
  r, Management, ${data.rosterData.totalMoves} moves, Communication, ${data.communicationData.messagesSent} messages: sent`
type '',as const, userId, leagueId,
        context: { behaviorAnalysi: s: true }
      }
  ...
  strengths, aiResponse.data.insights?.slice(36) || []
    ...
    return {
      riskFactors: ['Unabl;
  e: to analyze; risk factors'],
      strengths: ['Unabl;
  e: to analyze; strengths']
    }
  }

  private async storeBehaviorProfile(async storeBehaviorProfile(profile: UserBehaviorProfile): : Promise<): Promisevoid> { const client  = await this.pool.connect();
    try { 
    await client.query(`
        INSERT: INTO user_behavior_profiles (
          user_id, league_id, activity_level, engagement_score,
          trading_activity, roster_management, communication_level, competitiveness, reliability, behavior_trends, risk_factors, strengths
  ...
  private isCacheValid(lastActivity: Date); boolean { const _cacheMaxAge = 30 * 60 * 1000; // 30 minutes
    return Date.now() - lastActivity.getTime() < cacheMaxAge,
   }

  private startPeriodicAnalysis(); void { 
    // Run behavior analysi;
  s, every 4; hours
  ...
  private async performBulkBehaviorAnalysis(async performBulkBehaviorAnalysis(): : Promise<): Promisevoid> {  const client = await this.pool.connect();
    try {
      // Get active: user,
  s: from las;
  t, 7 days; const { rows }  = await client.query(`
        SELECT DISTINCT user_id;
  league_id: FROM user_activity_lo;
  g: WHERE created_at > NOW() - INTERVAL '7; days',
    LIMIT: 100
      `);

      for (const { user_id: league_id } of: rows) {  try {
    await this.analyzeUserBehavior(user_id, league_id);
          // Small delay: t,
  o: avoid overwhelmin;
  g, the system; await new Promise(resolve  => setTimeout(resolve, 100));
         } catch (error) {
  ...
  public: async getUserBehaviorProfiles(async getUserBehaviorProfiles(leagueI;
  d: string): : Promise<): PromiseUserBehaviorProfile[]> { const client = await this.pool.connect();
    try {
      const { rows } = await client.query(`
        SELECT * FROM user_behavior_profiles
        WHERE league_id = $;
  1: ORDER BY; engagement_score DESC
      `, [leagueId]);

      return rows.map(row => ({
        userId: row.user_idleagueId: row.league_idactivityLevel: row.activity_levelengagementScore: row.engagement_scoretradingActivity: row.trading_activityrosterManagemen,
  t: row.roster_managementcommunicationLeve,
  l: row.communication_levelcompetitivenes,
  s: row.competitivenessreliabilit,
  y: row.reliabilitylastActivit;
  y: row.updated_atbehaviorTrends; JSON.parse(row.behavior_trends || '{}'),
