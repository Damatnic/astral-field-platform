...
export class CompetitiveTeamGeneratorService {
  ...
  constructor(
    ...
    rosterConstructor: RosterConstructionSimulatorService
  ) {
    this.pool = pool;
  ...
  async generateCompetitiveLeague(leagueId: string): Promise<LeagueComposition> {
    ...
    const rosterConstructions = await this.rosterConstructor.generateTeamCompositions(
      leagueId, 
      teamPersonalities
    );

    // Step 3: Create competitive balance and adjust if needed
    const balancedRosters = await this.applyCompetitiveBalancing(rosterConstructions);

    // Step 4: Generate team storylines and rivalries
    const teamsWithNarratives = await this.generateTeamNarratives(balancedRosters, teamPersonalities);

    // Step 5: Project season outcomes and playoff race
    const seasonProjections = await this.projectSeasonOutcomes(teamsWithNarratives);

    // Step 6: Create key matchups and storylines
    const leagueStorylines = await this.generateLeagueStorylines(teamsWithNarratives);
    const keyMatchups = await this.generateKeyMatchups(teamsWithNarratives);

    const composition: LeagueComposition = {
      teams: teamsWithNarratives,
      competitiveBalance: this.calculateCompetitiveBalance(teamsWithNarratives),
      parityScore: this.calculateParityScore(teamsWithNarratives),
      storylines: leagueStorylines,
      expectedPlayoffRace: seasonProjections,
      keyMatchups
    };
  ...
  private async generateTeamPersonalities(leagueId: string): Promise<any[]> {
    // Create 10 diverse team personalities with realistic owner archetypes
    const ownerArchetypes = [
      {
        name: "The Analytics Expert",
        strategy: "value_based",
        personality: {
          riskTolerance: 0.3,
          positionPreferences: { QB: 0.15, RB: 0.35, WR: 0.35, TE: 0.10, K: 0.02, DST: 0.03 },
          personalityTraits: {
      ...
      },
      {
        ...
        personality: {
          riskTolerance: 0.7,
          positionPreferences: { QB: 0.25, RB: 0.15, WR: 0.40, TE: 0.15, K: 0.02, DST: 0.03 },
          personalityTraits: {
            handcuffsLover: false,
            rookieFocused: false,
            veteranBias: false,
      ...
      },
      {
        ...
        personality: {
          riskTolerance: 0.8,
          positionPreferences: { QB: 0.10, RB: 0.35, WR: 0.40, TE: 0.10, K: 0.02, DST: 0.03 },
          personalityTraits: {
            handcuffsLover: false,
            rookieFocused: true,
            veteranBias: false,
      ...
      },
      {
        ...
        personality: {
          riskTolerance: 0.2,
          positionPreferences: { QB: 0.20, RB: 0.30, WR: 0.30, TE: 0.15, K: 0.02, DST: 0.03 },
          personalityTraits: {
            handcuffsLover: true,
            rookieFocused: false,
            veteranBias: true,
      ...
      },
      {
        ...
        personality: {
          riskTolerance: 0.5,
          positionPreferences: { QB: 0.08, RB: 0.50, WR: 0.30, TE: 0.07, K: 0.02, DST: 0.03 },
          personalityTraits: {
            handcuffsLover: true,
            rookieFocused: false,
            veteranBias: false,
      ...
      },
      {
        ...
        personality: {
          riskTolerance: 0.5,
          positionPreferences: { QB: 0.18, RB: 0.28, WR: 0.30, TE: 0.18, K: 0.03, DST: 0.03 },
          personalityTraits: {
            handcuffsLover: false,
            rookieFocused: false,
            veteranBias: false,
      ...
      },
      {
        ...
        personality: {
          riskTolerance: 0.9,
          positionPreferences: { QB: 0.12, RB: 0.32, WR: 0.38, TE: 0.13, K: 0.02, DST: 0.03 },
          personalityTraits: {
            handcuffsLover: false,
            rookieFocused: true,
            veteranBias: false,
      ...
      },
      {
        ...
        personality: {
          riskTolerance: 0.1,
          positionPreferences: { QB: 0.22, RB: 0.26, WR: 0.30, TE: 0.17, K: 0.02, DST: 0.03 },
          personalityTraits: {
            handcuffsLover: true,
            rookieFocused: false,
            veteranBias: true,
      ...
      },
      {
        ...
        personality: {
          riskTolerance: 0.6,
          positionPreferences: { QB: 0.25, RB: 0.25, WR: 0.35, TE: 0.10, K: 0.02, DST: 0.03 },
          personalityTraits: {
            handcuffsLover: false,
            rookieFocused: false,
            veteranBias: false,
      ...
      },
      {
        ...
        personality: {
          riskTolerance: 0.4,
          positionPreferences: { QB: 0.16, RB: 0.30, WR: 0.34, TE: 0.15, K: 0.02, DST: 0.03 },
          personalityTraits: {
            handcuffsLover: false,
            rookieFocused: false,
            veteranBias: false,
    ...
    try {
      ...
      return teams.map((team, index) => ({
        ...
        userId: team.user_id,
        ownerArchetype: ownerArchetypes[index % ownerArchetypes.length].name,
  ...
  private async applyCompetitiveBalancing(constructions: any[]): Promise<any[]> {
    ...
    return constructions.map(construction => {
      ...
      const adjustmentNeeded = Math.abs(deviation) > stdDev;
      
      if (adjustmentNeeded) {
        const targetValue = avgValue + (Math.sign(deviation) * stdDev * 0.8);
        const adjustmentFactor = targetValue / construction.totalValue;
        
        return {
          ...construction,
          totalValue: targetValue,
          competitiveness: construction.competitiveness * adjustmentFactor,
          // Maintain the team's character while balancing power level
          adjustedForBalance: true,
          originalValue: construction.totalValue
        };
      }
  ...
  private async generateTeamNarratives(
    ...
    for (let i = 0; i < constructions.length; i++) {
      ...
      // Generate team storyline using AI
      const storyline = await this.generateTeamStoryline(construction, personality);
      
      // Identify unique factors
      const uniqueFactors = this.identifyUniqueFactors(construction, personality);
      
      // Calculate projections
      const projectedWins = this.projectTeamWins(construction);
      const strengthOfSchedule = Math.random() * 0.4 + 0.8; // 0.8 to 1.2
      
      teams.push({
        teamId: construction.teamId,
        teamName: personality.teamName,
        ownerName: personality.ownerArchetype,
        strategy: personality.strategy,
        personality,
        roster: {
          starters: construction.startingLineup,
          bench: construction.bench
        },
        projectedWins,
        strengthOfSchedule,
  ...
  private async generateTeamStoryline(construction: any, personality: any): Promise<string> {
    const prompt = `Create a compelling fantasy football team storyline:
...
Generate a 2-3 sentence storyline describing this team's identity, approach, and what makes them interesting this season.`;
    ...
    try {
      const response = await this.aiRouter.generateResponse({
        ...
        context: { action: 'team_storyline', strategy: personality.strategy }
      });
  ...
  private identifyUniqueFactors(construction: any, personality: any): string[] {
  ...
  private projectTeamWins(construction: any): number {
    // Project wins based on construction quality with some randomness
    let baseWins = construction.competitiveness * 14; // 14-game season
    
    // Add randomness for realistic variation
    const randomFactor = (Math.random() - 0.5) * 3; // ±1.5 games
  ...
  private generateRivalries(teams: GeneratedTeam[]): GeneratedTeam[] {
    // Create natural rivalries based on competing strategies and narratives
    const rivalryPairs: Array<[number, number]> = [];
    ...
    for (let i = 0; i < teams.length; i++) {
      for (let j = i + 1; j < teams.length; j++) {
        ...
        const competitiveDiff = Math.abs(team1.competitiveRating - team2.competitiveRating);
        if (competitiveDiff < 0.1) {
          rivalryScore += 2;
        }

        // Projected close record creates rivalry
        const winDiff = Math.abs(team1.projectedWins - team2.projectedWins);
        if (winDiff < 1.5) {
          rivalryScore += 1;
        }
  ...
  private async projectSeasonOutcomes(teams: GeneratedTeam[]): Promise<{
    ...
    // Sort teams by projected performance
    const sorted = [...teams].sort((a, b) => {
      const aScore = a.projectedWins + a.competitiveRating;
      const bScore = b.projectedWins + b.competitiveRating; 
      return bScore - aScore;
    });

    return {
      favorites: sorted.slice(0, 3).map(t => t.teamName),
      wildcards: sorted.slice(3, 6).map(t => t.teamName),
  ...
  private async generateLeagueStorylines(teams: GeneratedTeam[]): Promise<string[]> {
    ...
    // Strategy battle storyline  
    const strategies = [...new Set(teams.map(t => t.strategy))];
  ...
  private async generateKeyMatchups(teams: GeneratedTeam[]): Promise<Array<{
    ...
    for (const team of teams) {
      for (const rivalry of team.rivalries) {
        ...
        if (rival) {
          matchups.push({
            week: Math.floor(Math.random() * 13) + 1, // Random week 1-13
            team1: team.teamName,
            team2: rival.teamName,
            importance: 'High',
            narrative: `${team.strategy} vs ${rival.strategy} philosophy clash`
          });
        }
      }
    }

    // Championship contender matchups
    const topTeams = teams
    ...
    for (let i = 0; i < topTeams.length - 1; i++) {
      matchups.push({
        week: Math.floor(Math.random() * 4) + 10, // Late season weeks 10-13
        team1: topTeams[i].teamName,
        team2: topTeams[i + 1].teamName,
        importance: 'Critical',
        narrative: 'Potential playoff preview between championship favorites'
      });
    }
  ...
  private calculateParityScore(teams: GeneratedTeam[]): number {
    const winDifference = Math.max(...teams.map(t => t.projectedWins)) - 
  ...
  private async storeGeneratedLeague(leagueId: string, composition: LeagueComposition): Promise<void> {
    const client = await this.pool.connect();
    try {
      await client.query(`
      ...
      for (const team of composition.teams) {
        await client.query(`
          ...
          ON CONFLICT (team_id)
          DO UPDATE SET
            owner_archetype = EXCLUDED.owner_archetype,
  ...
  // Public API methods
  async getGeneratedLeague(leagueId: string): Promise<LeagueComposition | null> {
    const client = await this.pool.connect();
    try {
