...
export class RosterConstructionSimulatorService {
  ...
  async simulateRealisticRosterConstruction(
    personalities: TeamPersonalityProfile[],
    ...
    // Apply competitive balancing
    return this.applyCompetitiveBalancing(constructions);
  }

  private async buildRealisticRoster(
    personality: TeamPersonalityProfile,
    playerPool: PlayerProfile[],
    rosterConfig: any,
    existingConstructions: RosterConstruction[]
  ): Promise<RosterConstruction> {
    ...
    const startingLineup: RosterSlot[] = [
      { position: 'QB', required: true, filled: false },
      { position: 'RB', required: true, filled: false },
      { position: 'RB', required: true, filled: false },
      { position: 'WR', required: true, filled: false },
      { position: 'WR', required: true, filled: false },
      { position: 'TE', required: true, filled: false },
      { position: 'FLEX', required: true, filled: false }, // RB/WR/TE
      { position: 'DST', required: true, filled: false },
      { position: 'K', required: true, filled: false }
    ];

    const bench: RosterSlot[] = Array(7).fill(null).map(() => ({
      position: 'BENCH',
      required: false,
      filled: false
    }));
    ...
    for (let round = 1; round <= 15; round++) {
      ...
      if (pick) {
        ...
        const playerIndex = availablePlayers.findIndex(p => p.id === pick.player.id);
        if (playerIndex >= 0) {
    ...
    const totalValue = this.calculateRosterValue(draftedPlayers.map(p => p.player));
  ...
  private async simulateDraftPick(
    ...
    bench: RosterSlot[],
    round: number,
    draftedSoFar: { player: PlayerProfile; round: number }[]
  ): Promise<{ player: PlayerProfile; round: number; slot: RosterSlot } | null> {
    // Determine positional needs
    const needs = this.calculatePositionalNeeds(startingLineup, bench, draftedSoFar);
    
    // Filter players based on strategy and needs
    const candidates = this.filterCandidatesByStrategy(
      availablePlayers,
      personality,
      needs,
      round
    ...
    // Select player based on personality
    const selectedPlayer = await this.selectPlayerByPersonality(
      candidates,
      personality,
      round,
      needs
  ...
  private calculatePositionalNeeds(
    ...
    // Count required positions still needed
    ...
    // Add depth needs
    const drafted = draftedSoFar.map(d => d.player);
    ...
    const idealCounts = { QB: 2, RB: 5, WR: 6, TE: 2, K: 1, DST: 2 };
  ...
  private filterCandidatesByStrategy(
    ...
    if (personality.personalityTraits.sleperHunter) {
      ...
      candidates = candidates.sort((a, b) => {
        const aValue = a.projectedPoints / (a.adp || 200);
        const bValue = b.projectedPoints / (b.adp || 200);
    ...
    if (round <= 3) {
      ...
      // Late rounds: fill K/DST needs first
      const kickers = candidates.filter(p => p.position === 'K');
  ...
  private async selectPlayerByPersonality(
    ...
    const scoredCandidates = candidates.map(player => {
      let score = player.projectedPoints / 100; // Base score
      ...
      const positionPref = personality.positionPreferences[player.position] || 0;
      ...
      switch (personality.strategy) {
        case 'value_based':
          const adpValue = Math.max(0, (200 - (player.adp || 150)) / 200);
          score += adpValue * 0.2;
          break;
  ...
  private findRosterSlot(
    ...
    for (const slot of bench) {
      if (!slot.filled) {
        slot.position = player.position; // Set bench position
        return slot;
      }
    }

    // Shouldn't happen in normal draft, but return first bench slot as fallback
    return bench[0];
  }
  ...
  private calculateCompetitiveness(
    ...
    let competitiveness = (totalValue / 2000) * 0.6 + positionBalance * 0.4;
  ...
  private calculateUniqueness(
    ...
    existingConstructions.forEach(construction => {
      ...
      const intersection = new Set([...playerIds].filter(id => otherPlayerIds.has(id)));
  ...
  private async generatePickReasoning(
    ...
    const prompt = `Generate brief draft pick reasoning:
...
Generate 1 sentence explaining this pick from the team's perspective.`;
    ...
    try {
      const response = await this.aiRouter.generateResponse({
        ...
        context: { action: 'draft_reasoning', round, strategy: personality.strategy }
      });
  ...
  private applyCompetitiveBalancing(constructions: RosterConstruction[]): RosterConstruction[] {
    ...
    return constructions.map(construction => {
      let adjustedCompetitiveness = construction.competitiveness;
  ...
  // Public API methods
  async generateTeamCompositions(
    leagueId: string,
    personalities: TeamPersonalityProfile[]
  ): Promise<RosterConstruction[]> {
    // Get player pool from database
    const playerPool = await this.getPlayerPool(leagueId);
    
    // Get roster configuration
    const rosterConfig = await this.getRosterConfiguration(leagueId);
    ...
    try {
      const { rows } = await client.query(`
        SELECT 
          ...
          bye_week
        FROM players
        WHERE rostered = false
          AND position IN ('QB', 'RB', 'WR', 'TE', 'K', 'DST')
          AND projected_points > 0
        ORDER BY adp ASC NULLS LAST, projected_points DESC
        LIMIT 300
      `);
      ...
      return rows.map(row => ({
        ...
        adp: row.adp || 999,
        injuryRisk: this.calculateInjuryRisk(row.injury_status, row.position, row.years_exp),
        upside: this.calculateUpside(row),
        floor: this.calculateFloor(row),
        consistency: this.calculateConsistency(row),
  ...
  private calculateInjuryRisk(injuryStatus: string, position: string, yearsExp: number): number {
  ...
  private calculateUpside(player: any): number {
    let upside = 0.5;
    
    if (player.years_exp === 0) upside += 0.3; // Rookies have upside
    if (player.years_exp <= 3) upside += 0.1; // Young players
  ...
  private calculateFloor(player: any): number {
    let floor = 0.5;
