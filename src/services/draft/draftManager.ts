...
class DraftManager {
  ...
  async createDraft(settings: Omit<DraftSettings, 'id' | 'status'>): Promise<DraftSettings> {
    ...
    await database.query(`
      INSERT INTO drafts (
        id, league_id, draft_type, rounds, seconds_per_pick, 
        draft_date, auction_budget, draft_order, status, 
        current_pick, current_round, created_at
  ...
  async startDraft(draftId: string): Promise<void> {
    const draft = await this.getDraftSettings(draftId);
  ...
  async pauseDraft(draftId: string, commissionerId: string): Promise<void> {
    const draft = await this.getDraftSettings(draftId);
    if (!draft) throw new Error('Draft not found');
  ...
  async resumeDraft(draftId: string, commissionerId: string): Promise<void> {
    const draft = await this.getDraftSettings(draftId);
    if (!draft) throw new Error('Draft not found');
  ...
  private async advanceToNextPick(draft: DraftSettings): Promise<void> {
    const totalPicks = draft.draftOrder.length * draft.rounds;
  ...
  private calculateSnakeDraftTeam(pickNumber: number, draftOrder: string[]): string {
  ...
  async nominatePlayer(draftId: string, teamId: string, playerId: string): Promise<AuctionNomination> {
    const draft = await this.getDraftSettings(draftId);
    ...
    await database.query(`
      INSERT INTO auction_nominations (
        id, draft_id, player_id, nominating_team_id, current_bid, 
        time_remaining, is_active, created_at
  ...
  async placeBid(nominationId: string, teamId: string, bidAmount: number): Promise<void> {
    const nomination = await this.getAuctionNomination(nominationId);
    ...
    await database.query(`
      ...
      SET current_bid = $1, current_bidder = $2, updated_at = NOW()
      WHERE id = $3
  ...
  private startAuctionTimer(nomination: AuctionNomination): void {
  ...
  private async completeAuction(nominationId: string): Promise<void> {
    const nomination = await this.getAuctionNomination(nominationId);
  ...
  async makeAutoPick(draftId: string, teamId: string): Promise<DraftPick> {
    const draft = await this.getDraftSettings(draftId);
    if (!draft) throw new Error('Draft not found');
  ...
  private async getBestAvailablePlayer(teamId: string, leagueId: string): Promise<DraftablePlayer | null> {
    ...
    for (const player of availablePlayers) {
      const score = this.calculatePlayerScore(player, teamNeeds);
      if (score > bestScore) {
        bestScore = score;
        bestPlayer = player;
      }
  ...
  private async getAvailablePlayers(leagueId: string): Promise<DraftablePlayer[]> {
  ...
  private async validatePick(draft: DraftSettings, teamId: string, playerId: string): Promise<void> {
    ...
    if (playerDrafted.rows.length > 0) {
  ...
  private startPickTimer(draft: DraftSettings): void {
    if (draft.timePerPick <= 0) return;
  ...
  private async getDraftSettings(draftId: string): Promise<DraftSettings | null> {
    // Check cache first
  ...
  private async updateDraftStatus(draftId: string, updates: any): Promise<void> {
    ...
    // Update cache
    if (this.draftStates.has(draftId)) {
      const draft = this.draftStates.get(draftId)!;
      Object.assign(draft, updates);
    }
  }

  private async storeDraftPick(pick: DraftPick): Promise<void> {
    await database.query(`
      INSERT INTO draft_picks (
        id, draft_id, team_id, player_id, pick_number, round, 
        pick_time, time_taken, is_keeper, auto_drafted, created_at
  ...
  private async addPlayerToRoster(teamId: string, playerId: string, acquisitionType: string, cost?: number): Promise<void> {
    await database.query(`
      INSERT INTO rosters (team_id, player_id, acquisition_type, acquisition_cost, created_at)
      VALUES ($1, $2, $3, $4, NOW())
  ...
  private broadcastDraftUpdate(draft: DraftSettings, data: any): void {
  ...
  private broadcastPickMade(draft: DraftSettings, pick: DraftPick): void {
  ...
  private broadcastAuctionNomination(draft: DraftSettings, nomination: AuctionNomination): void {
  ...
  private broadcastAuctionBid(nomination: AuctionNomination, teamId: string, bidAmount: number): void {
  ...
  private broadcastAuctionComplete(draft: DraftSettings, nomination: AuctionNomination, pick: DraftPick): void {
    webSocketManager.io?.to(`draft:${draft.id}`).emit('auction_complete', {
      nomination,
      pick,
      timestamp: new Date().toISOString()
    });
  ...
  async undoPick(draftId: string, commissionerId: string): Promise<void> {
    const draft = await this.getDraftSettings(draftId);
    if (!draft) throw new Error('Draft not found');
  ...
  async completeDraft(draftId: string): Promise<void> {
    ...
    for (const [nominationId, timer] of this.auctionTimers.entries()) {
      if (nominationId.startsWith(draftId)) {
        clearTimeout(timer);
  ...
  private async validateCommissionerPermission(leagueId: string, userId: string): Promise<void> {
    const result = await database.query(`
  ...
  private calculateTier(adp: number): number {
    if (adp <= 12) return 1;
    if (adp <= 24) return 2;
    if (adp <= 36) return 3;
    if (adp <= 60) return 4;
  ...
  private calculatePlayerScore(player: any, teamNeeds: any): number {
    // Simple scoring algorithm - can be enhanced
    let score = 100 - player.adp; // Higher score for better ADP
    
    if (teamNeeds.remainingNeeds.includes(player.position)) {
      score += 50; // Bonus for needed position
    }
    
    return score;
  }

  private calculateUpcomingPicks(draft: DraftSettings): UpcomingPick[] {
    ...
    for (let i = 0; i < 5 && (draft.currentPick! + i) <= totalPicks; i++) {
      ...
      const teamId = draft.type === 'snake' 
        ? this.calculateSnakeDraftTeam(pickNumber, draft.draftOrder)
        : draft.draftOrder[(pickNumber - 1) % teamCount];
      ...
      picks.push({
        ...
        timeRemaining: i === 0 ? draft.timePerPick : draft.timePerPick
      });
    }
  ...
  private async analyzeTeamNeeds(teamId: string): Promise<any> {
    ...
    const standardRoster = { QB: 2, RB: 4, WR: 4, TE: 2, DST: 1, K: 1 };
  ...
  private async analyzeAllTeamNeeds(leagueId: string): Promise<TeamNeedsAnalysis[]> {
    ...
    for (const team of teamsResult.rows) {
      const needs = await this.analyzeTeamNeeds(team.id);
      analysis.push({
        teamId: team.id,
  ...
  private async getPlayerRecommendations(teamId: string, leagueId: string): Promise<PlayerRecommendation[]> {
    const teamNeeds = await this.analyzeTeamNeeds(teamId);
  ...
  private async getDraftedPlayers(draftId: string): Promise<DraftedPlayer[]> {
    const result = await database.query(`
  ...
  private async getDraftPickHistory(draftId: string): Promise<DraftPick[]> {
    const result = await database.query(`
  ...
  private async getAuctionNomination(nominationId: string): Promise<AuctionNomination | null> {
    const result = await database.query(`
  ...
  private async validateNomination(draft: DraftSettings, teamId: string, playerId: string): Promise<void> {
  ...
  private async validateBid(nomination: AuctionNomination, teamId: string, bidAmount: number): Promise<void> {
    if (bidAmount <= nomination.currentBid) {
    ...
    if (teamBudgetResult.rows.length === 0) {
