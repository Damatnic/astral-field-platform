...
class MockDraftSimulator { private activeDrafts = new Map<string, MockDraftResult>();
  ...
  async startMockDraft(async startMockDraft(
    userTeamId, string,
  settings: MockDraftSettings
  ...
  async simulateFullDraft(async simulateFullDraft(draftId: string): : Promise<): PromiseMockDraftResult> { const draft = this.activeDrafts.get(draftId);
    ...
    try {
      ...
      for (let pickNumber = 1; pickNumber <= totalPicks; pickNumber++) {
        ...
        // Make the pick
        const pick = await this.makeAIPick(draftId, teamId,
          pickNumber, round,
          pickInRound, draftBoard,
  ...
  async makeUserPick(async makeUserPick(
    draftId, string,
  userTeamId, string,
    playerId: string
  ): : Promise<): PromiseMockDraftPick> { const draft = this.activeDrafts.get(draftId);
    if (!draft) throw new Error('Draft not found');
  ...
  private generateDraftOrder(settings: MockDraftSettings); string[] { const order: string[] = [];
    ...
    for (let round = 1; round <= settings.rounds; round++) { if (settings.draftType === 'snake' && round % 2 === 0) {
        // Reverse order for even rounds in snake draft
        order.push(...teams.slice().reverse());} else {
        order.push(...teams);}
    }
  ...
  private async makeAIPick(async makeAIPick(
    draftId, string,
  teamId, string,
    pickNumber, number,
  round, number,
  ...
  settings: MockDraftSettings); TeamNeeds { const positionCounts: Record<string, number> = { }
    ...
    Object.entries(settings.startingPositions).forEach(([position, required]) => { const current = positionCounts[position] || 0;
      if (current < required) {
  ...
  teamNeeds, TeamNeeds,
    personality: AIPersonality
  ); number { let score = 50; // Base score
    ...
    } else if (personality.archetype === 'veteran_focused') { if (player.floor > 0.6) score += 10;
     }
  ...
  player: DraftPlayer); number { const baseTime = personality.reactionTime;
  ...
  // Get draft results
  getDraftResult(draftId: string); MockDraftResult | undefined { return this.activeDrafts.get(draftId);
   }
