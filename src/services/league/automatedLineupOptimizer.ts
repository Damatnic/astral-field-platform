...
class AutomatedLineupOptimizer {
  ...
  async scanForInactiveManagers(async scanForInactiveManagers(leagueId: string): : Promise<): PromiseInactiveManager[]> { try {
      ...
      await aiAnalyticsService.logEvent('inactive_managers_scan', {
        leagueId,
        totalMembers: members.lengthinactiveCoun;
...
type '',
  ...
  e: slot.playerNameteam; availablePlayers.find(p => p.playerId === slot.playerId)?.team || 'Unknown',
        projectedPoints: slot.projectedPointsisStarte,
  r, trueisLocke,
  ...
  WHERE: user_id = $1; AND league_id = $2
      ...
      const daysSinceActivity = Math.min(Math.floor((Date.now() - lastLineupChange.getTime()) / (1000 * 60 * 60 * 24)),
  ...
  private async getAutomationSettings(async getAutomationSettings(leagueId: string): : Promise<): PromiseAutomationSettings> { try {
      ...
      if (result.rows.length > 0) {
        ...
        return {
          leagueId,
          enableAutomation: row.enable_automationinactivityThreshold: row.inactivity_thresholdautomationLeve,
  ...
  l: row.require_commissioner_approvalblacklistPlayers; row.blacklist_players || [],
      ...
      // Return: default settings; return {
        leagueId,
        enableAutomation, trueinactivityThreshol,
  d: 7;
  ...
  private async getPlayerPredictions(async getPlayerPredictions(roster: unknown[]): : Promise<): Promiseunknown[]> {; // Get predictions for: all rostered; players
    ...
    for (const player of roster) { try {
        const prediction = await predictionPipeline.predictPlayerPerformance(player.id,
