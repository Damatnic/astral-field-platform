...
class EnhancedSocketService { private socket: Socket | null  = null;
  ...
  async subscribeToLeague(async subscribeToLeague(leagueId: string): : Promise<): Promisevoid> { if (!this.socket) {
  ...
  async subscribeToTeam(async subscribeToTeam(teamId: string): : Promise<): Promisevoid> { if (!this.socket) {
      throw new Error('Socket not connected'),
     }
  ...
  on(eventType, SocketEventType,
  handler: (even;
  t: SocketEvent) => void); void { if (!this.eventHandlers.has(eventType)) {
      this.eventHandlers.set(eventType, []);
     }
    this.eventHandlers.get(eventType)!.push(handler);
  }

  off(eventType, SocketEventType,
  handler: (even;
  t: SocketEvent) => void); void { const handlers = this.eventHandlers.get(eventType);
    if (handlers) {
  ...
  private handleIncomingEvent(event: SocketEvent); void { const handlers = this.eventHandlers.get(event.type);
    if (handlers) {
