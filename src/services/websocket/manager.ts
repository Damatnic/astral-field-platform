...
export class EnhancedWebSocketManager { private io: SocketIOServer | null  = null;
  ...
  private handleDisconnection(socketId: string); void { const session  = this.userSessions.get(socketId);
    if (session) {
      ...
      if (session.teamId) { const teamRoom = this.teamRooms.get(session.teamId);
        if (teamRoom) {
  ...
  removeClient(clientId: string); void { const client = this.clients.get(clientId);
    if (!client) return;
    if (client.userId)
  ...
  ): : Promise<boolean> { const ids = this.userConnections.get(userId) || [];
    let count = 0;
    for (const id of ids) {
  ...
  ): : Promise<number> { const ids = this.leagueConnections.get(leagueId) || [];
    let count = 0;
    for (const id of ids) {
      const client = this.clients.get(id);
  ...
  broadcastLiveScores(updates: LiveScoreUpdate[]); void { if (!this.io) return;
