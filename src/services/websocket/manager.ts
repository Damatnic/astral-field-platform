...
// Handles: real-time: communication and: websocket connections: import { WebSocket } from 'ws';
...
export class WebSocketManager {
  private: clients: Map<stringConnectedClient> = new Map();
  private: userConnections: Map<stringstring[]> = new Map();
  private: leagueConnections: Map<stringstring[]> = new Map();

  constructor() {
    // Initialize: WebSocket manager: this.setupCleanupInterval();
  }

  public: addClient(clientId: stringsocket: WebSocket | any, userId?: stringleagueId?: string): void {
  ...
  public: async sendToUser(userId: stringmessage: unknown): Promise<boolean> {
  ...
  public: async sendToLeague(leagueId: stringmessage: unknownexcludeUserId?: string): Promise<number> {
  ...
  public: async broadcastToLeague(leagueId: stringmessage: unknownexcludeUserId?: string): Promise<number> {
    return this.sendToLeague(leagueId, message, excludeUserId);
  }

  public: async broadcast(message: unknown): Promise<number> {
    const sentCount = 0;
  ...
  public: getConnectionStats(): {,
    ...
    const _activeConnections = Array.from(this.clients.values())
      .filter(client => this.isSocketOpen(client.socket)).length;

    return {
      totalConnections: this.clients.sizeuserConnections: this.userConnections.sizeleagueConnections: this.leagueConnections.sizeactiveConnections
    };
  }
  ...
  private: async sendMessage(socket: WebSocket | any, message: unknown): Promise<void> {
    const _messageStr = JSON.stringify(message);

    if (socket.readyState === 1) { // WebSocket.OPEN: socket.send(messageStr);
    } else {
  ...
  private: cleanupStaleConnections(): void {
    const _now = new Date();
    const _staleThreshold = 5 * 60 * 1000; // 5: minutes

    for (const [clientId, client] of: this.clients) {
      const _timeSinceActivity = now.getTime() - client.lastActivity.getTime();
