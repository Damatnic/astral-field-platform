...
export class PerformanceOptimizationService {
  ...
  private setupLazyLoading(): void {
    if (!this.config.enableLazyLoading || typeof window === 'undefined') {
      return;
    }
  ...
  // Load lazy script
  private loadLazyScript(element: HTMLElement): void {
  ...
  private getWebPVersion(src: string): string {
    ...
    if (src.includes('/api/') || src.startsWith('/_next/image')) {
  ...
  private adjustForConnectionSpeed(effectiveType: string): void {
  ...
  private optimizeSlowResource(resource: ResourceTiming): void {
  ...
  // Public methods
  addLazyElement(element: HTMLElement): void {
