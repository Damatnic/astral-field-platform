...
export class PushNotificationService {
  ...
  async sendScoreUpdateNotification(teamName: string, currentScore: number, projectedScore: number): Promise<void> {
    const data: NotificationData = {
      title: '⚡ Live Score Update',
      body: `${teamName}: ${currentScore} pts (proj: ${projectedScore})`,
      ...
      actions: [
        { action: 'view', title: 'View Live Scores', icon: '/icons/view-icon.png' },
  ...
  async sendMatchupReminderNotification(opponent: string, timeUntilStart: string): Promise<void> {
    const data: NotificationData = {
      ...
      actions: [
        { action: 'view-lineup', title: 'Check Lineup', icon: '/icons/lineup-icon.png' },
        { action: 'view-matchup', title: 'View Matchup', icon: '/icons/matchup-icon.png' }
      ]
    };
  ...
  async sendWaiverAlertNotification(playerName: string, action: 'claimed' | 'available'): Promise<void> {
    const title = action === 'claimed' ? '🎉 Waiver Claim Processed' : '🔄 Player Available';
    const body = action === 'claimed' 
      ? `You successfully claimed ${playerName}`
      : `${playerName} is now available on waivers`;
    ...
    const data: NotificationData = {
      ...
      actions: [
        { action: 'view', title: 'View Waivers', icon: '/icons/waiver-icon.png' },
        { action: 'dismiss', title: 'Dismiss' }
      ]
    };
  ...
  async sendTradeNotification(traderName: string, action: 'proposed' | 'accepted' | 'rejected'): Promise<void> {
    const titleMap = {
    ...
    const data: NotificationData = {
      ...
      actions: action === 'proposed' ? [
        { action: 'view-trade', title: 'View Trade', icon: '/icons/trade-icon.png' },
        { action: 'dismiss', title: 'Later' }
      ] : [
        { action: 'view', title: 'View Trades', icon: '/icons/trade-icon.png' }
      ]
    };
  ...
  async sendLineupReminderNotification(unsetPositions: string[]): Promise<void> {
    const data: NotificationData = {
      ...
      actions: [
        { action: 'set-lineup', title: 'Set Lineup', icon: '/icons/lineup-icon.png' },
  ...
  async sendInjuryAlertNotification(playerName: string, injuryStatus: string): Promise<void> {
    const data: NotificationData = {
      title: '🏥 Injury Alert',
      ...
      actions: [
        { action: 'view-player', title: 'View Player', icon: '/icons/player-icon.png' },
        { action: 'find-replacement', title: 'Find Replacement', icon: '/icons/waiver-icon.png' }
      ]
    };
  ...
  private async sendNotificationToServer(type: NotificationType, data: NotificationData): Promise<void> {
    if (!this.subscription) {
  ...
  private async verifySubscriptionOnServer(subscription: PushSubscription): Promise<void> {
    try {
  ...
  async sendTestNotification(): Promise<void> {
    ...
    if (this.subscription) {
      await this.sendNotificationToServer('general-alert', data);
    } else {
      await this.showLocalNotification(data);
    }
  }
