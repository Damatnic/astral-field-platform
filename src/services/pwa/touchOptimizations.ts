...
export class TouchOptimizationService {
  ...
  initialize(options?: Partial<TouchOptimizationOptions>): void {
    if (options) {
  ...
  // Handle tap gesture
  private handleTap(startPoint: TouchPoint, element: HTMLElement, timestamp: number): void {
  ...
  private handleSwipe(startPoint: TouchPoint, endPoint: TouchPoint, element: HTMLElement): void {
  ...
  private handleLongPress(startPoint: TouchPoint, element: HTMLElement): void {
  ...
  // Handle drag gesture
  private handleDrag(startPoint: TouchPoint, currentPoint: TouchPoint, element: HTMLElement): void {
  ...
  onGesture(gestureType: GestureData['type'], callback: GestureCallback): () => void {
    ...
    return () => {
      const callbacks = this.gestureListeners.get(gestureType);
      if (callbacks) {
  ...
  private triggerGesture(gestureType: GestureData['type'], gesture: GestureData): void {
    const callbacks = this.gestureListeners.get(gestureType);
    if (callbacks) {
  ...
  setupDraftBoardTouch(container: HTMLElement): void {
    ...
    this.onGesture('drag', (gesture) => {
      if (gesture.element.classList.contains('player-card')) {
        this.handlePlayerCardDrag(gesture);
      }
    });

    // Long press for player details
    this.onGesture('long-press', (gesture) => {
      if (gesture.element.classList.contains('player-card')) {
    ...
    this.onGesture('swipe', (gesture) => {
      if (gesture.element.closest('.draft-categories')) {
  ...
  setupRosterTouch(container: HTMLElement): void {
    ...
    this.onGesture('drag', (gesture) => {
      if (gesture.element.classList.contains('roster-player')) {
        this.handleLineupDrag(gesture);
      }
    });

    // Double tap for quick lineup actions
    this.onGesture('double-tap', (gesture) => {
      if (gesture.element.classList.contains('roster-slot')) {
  ...
  private handlePlayerCardDrag(gesture: GestureData): void {
    const playerCard = gesture.element;
  ...
  private showPlayerDetails(element: HTMLElement): void {
  ...
  private handleCategorySwipe(gesture: GestureData): void {
  ...
  private handleLineupDrag(gesture: GestureData): void {
    const playerElement = gesture.element;
  ...
  private handleQuickLineupAction(gesture: GestureData): void {
    const slot = gesture.element;
  ...
  private createDragGhost(element: HTMLElement): HTMLElement {
  ...
  // Haptic feedback
  private playHapticFeedback(intensity: 'light' | 'medium' | 'heavy'): void {
  ...
  private setupPullToRefreshForElement(element: HTMLElement, refreshCallback: () => void): void {
    const pullThreshold = 80;
    let startY = 0;
    let isPulling = false;
  ...
  private resetPullToRefreshUI(element?: HTMLElement): void {
    const refreshIndicator = (element || document).querySelector('.pull-refresh-indicator');
    if (refreshIndicator) {
  ...
  private preventZoom(event: TouchEvent): void {
    const timeSinceLastTouch = Date.now() - this.lastTapTime;
