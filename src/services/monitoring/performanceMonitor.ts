...
class PerformanceMonitor {
  ...
  private async checkNFLDataHealth(): Promise<ServiceHealth> {
    const startTime = Date.now();
    let status: 'healthy' | 'degraded' | 'unhealthy' = 'healthy';
    let errorCount = 0;
    let details = '';
    ...
    try {
      ...
      if (health.cacheSize > 1000) {
        details += ` Large cache size: ${health.cacheSize} items`;
      }
  ...
  private async checkScoringEngineHealth(): Promise<ServiceHealth> {
    const startTime = Date.now();
    let status: 'healthy' | 'degraded' | 'unhealthy' = 'healthy';
    let errorCount = 0;
    let details = '';
    ...
    try {
      ...
      } else if (health.isProcessing) {
        details = 'Currently processing scores';
      }
      
      // Check cache size
      if (health.cacheSize > 500) {
        details += ` High cache usage: ${health.cacheSize} scores cached`;
      }
  ...
  private async checkAPIHealth(): Promise<ServiceHealth> {
    ...
    if (errorRate > 0.05) {
      status = errorRate > 0.1 ? 'unhealthy' : 'degraded';
  ...
  // Create alert
  createAlert(severity: 'info' | 'warning' | 'critical', service: string, message: string) {
    const alert: Alert = {
