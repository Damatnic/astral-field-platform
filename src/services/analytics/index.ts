...
export type {
  MarketTrendData,
  ValueDataPoint,
  TrendAnalysis as MarketTrendAnalysis,
  VolatilityMetrics,
...
export const AnalyticsUtils = {
   ...
   */
  calculateAccuracy: (predictions: number[], actuals: number[]): number => {
   ...
   */
  calculateTrend: (values: number[]): { direction: 'up' | 'down' | 'stable'; strength: number } => {
    if (values.length < 2) {
   ...
   */
  calculateConsistency: (values: number[]): number => {
   ...
   */
  calculateRiskAdjustedReturn: (returns: number[], riskFreeRate = 0.02): number => {
   ...
   */
  exponentialMovingAverage: (values: number[], alpha: number): number[] => {
    if (values.length === 0) return [];
   ...
   */
  formatCurrency: (num: number, decimals = 2): string => {
    return `$${Number(num).toFixed(decimals)}`;
  },
   ...
   */
  getTrendIndicator: (direction: 'up' | 'down' | 'stable'): string => {
    switch (direction) {
      case 'up': return '📈';
      case 'down': return '📉';
...
export const AnalyticsConstants = {
  ...
  REAL_TIME_UPDATE: 30000,      // 30 seconds
