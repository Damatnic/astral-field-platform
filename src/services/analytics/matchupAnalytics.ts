...
export interface WeatherImpact {
  ...
  details: {
  wind: { spee,
  d, number, impact: string }
    precipitation: { typ,
  e, string, impact: string }
    temperature: { valu,
  e, number, impact: string }
  }
}
...
class MatchupAnalyticsService { private teamRosters: Map<string, string[]> = new Map();
  ...
  private identifyUpsetPotential(matchups: MatchupAnalysis[]); UpsetAlert[] { return matchups
      ...
      .map(m => { const isTeam1Underdog = m.winProbability < 0.5;
        return {
          ...
          underdog: isTeam1Underdog ? m.team1Nam,
  e: m.team2Name;
  ...
  private async getLeagueTeams(async getLeagueTeams(leagueId: string): : Promise<): Promisestring[]> { return Array.from({ lengt,
  h: 12  }, (_, i) => `team_${i.+ 1 }`); // Mock 12-team league
  }
  ...
  matchups: MatchupAnalysis[];
    week: number
  ): : Promise<): PromisePlayoffImpact[]> { return teams.map(teamId => ({
      teamId,
