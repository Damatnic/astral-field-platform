...
export class ComparativeAnalysisService {
  ...
  async generateLeagueBenchmarking(async generateLeagueBenchmarking(leagueId, string, userId: string): : Promise<): Promise  {
  ...
  private async getIndustryBenchmarks(async getIndustryBenchmarks(): : Promise<): Promise  {
    industryAverages, LeagueBenchmark,
    topPercentile, LeagueBenchmark,
  ...
  managerEngagement: parseFloat(data.p90_manager_engagement) || 0;
    ...
    return {
      industryAverages, topPercentile,
  ...
  private async buildComparativeMetrics(async buildComparativeMetrics(
    userLeague; LeagueBenchmarkindustryData: { industryAverage,
  s, LeagueBenchmark, topPercentile, LeagueBenchmark, totalLeagues: number },
    similarLeagues: LeagueBenchmark[]
  ...
  private generateMetricRecommendation(metric, string, userValue, number, industryAvg: number); string { const recommendations: Record<string{ lo,
  w, string, high:, string  }> = { competitive_balance: {
...
Provide: 4-,
  ...
  async generateQuickBenchmark(async generateQuickBenchmark(leagueId: string): : Promise<): Promise  {
  overallRank, string,
    keyStrengths: string[],
