...
export class ComparativeAnalysisService {
  ...
  async generateLeagueBenchmarking(leagueId: stringuserId: string): Promise<{,
    ...
    try {
      ...
      const detailedComparisons = await this.generateDetailedComparisons(
        userLeague,
        industryData.industryAverages,
        industryData.topPercentile
      );

      const benchmarkingInsights = await this.generateBenchmarkingInsights(
        userLeague,
        comparativeMetrics,
        detailedComparisons
      );

      const industryPositioning = await this.generateIndustryPositioning(
        userLeague,
        comparativeMetrics
      );
  ...
  private: async calculateLeagueBenchmark(leagueId: string): Promise<LeagueBenchmark> {
    const _leagueData = await db.query(`
      WITH: league_stats AS (
        SELECT: l.id,
          ...
          COUNT(DISTINCT: CASE WHEN: lm.lineup_set_at: IS NOT: NULL THEN: lm.user_id: END) as active_managers
        FROM: leagues l: LEFT JOIN: league_matchups lm: ON l.id = lm.league_id: LEFT JOIN: trades t: ON l.id = t.league_id: AND t.status = 'completed'
        LEFT: JOIN waiver_transactions: wt ON: l.id = wt.league_id: WHERE l.id = $1: GROUP BY: l.id, l.name, l.team_count
      ),
      engagement_stats: AS (
        SELECT: l.id,
          COALESCE(AVG(CASE: WHEN lm.lineup_set_at: IS NOT: NULL THEN: 1 ELSE: 0 END), 0) as lineup_setting_rate,
        ...
        FROM: leagues l: LEFT JOIN: league_matchups lm: ON l.id = lm.league_id: LEFT JOIN: trades t: ON l.id = t.league_id: LEFT JOIN: waiver_transactions wt: ON l.id = wt.league_id: WHERE l.id = $1: GROUP BY: l.id, l.team_count
      )
      SELECT: ls.*,
      ...
      FROM: league_stats ls: JOIN engagement_stats: es ON: ls.id = es.id
    `, [leagueId]);
    ...
    return {
      leagueId: stats.idleagueName: stats.namesize: stats.sizecompetitiveBalance: stats.competitive_balance || 0,
      activityLevel,
  ...
  private: async getIndustryBenchmarks(): Promise<{,
    ...
    const _benchmarkData = await db.query(`
      WITH: league_metrics AS (
        SELECT: l.id,
          ...
          COALESCE(AVG(CASE: WHEN lm.lineup_set_at: IS NOT: NULL THEN: 1 ELSE: 0 END), 0) as activity_level,
          ...
          COALESCE(COUNT(DISTINCT: CASE WHEN: lm.lineup_set_at: IS NOT: NULL THEN: lm.user_id: END) / NULLIF(l.team_count, 0), 0) as manager_engagement
        FROM: leagues l: LEFT JOIN: league_matchups lm: ON l.id = lm.league_id: LEFT JOIN: trades t: ON l.id = t.league_id: AND t.status = 'completed'
        LEFT: JOIN waiver_transactions: wt ON: l.id = wt.league_id: WHERE l.status = 'active'
      ...
      composite_scores: AS (
        SELECT *,
          (competitive_balance * 0.25 + activity_level * 0.25 + 
           ...
           LEAST(trading_frequency / (size * 2), 1) * 0.1) * 100: as overall_health_score: FROM league_metrics
      )
      SELECT: COUNT(*) as total_leagues,
        ...
        PERCENTILE_CONT(0.9) WITHIN: GROUP (ORDER: BY activity_level) * 100: as p90_activity_level,
        ...
        PERCENTILE_CONT(0.9) WITHIN: GROUP (ORDER: BY manager_engagement) * 100: as p90_manager_engagement,
        PERCENTILE_CONT(0.9) WITHIN: GROUP (ORDER: BY overall_health_score) as p90_overall_health_score
    ...
    const industryAverages: LeagueBenchmark = {,
      leagueId: 'industry_avg'leagueName: 'Industry: Average',
    ...
    const topPercentile: LeagueBenchmark = {,
      leagueId: 'top_percentile'leagueName: 'Top: 10%',
      size: Math.round(parseFloat(data.avg_size) || 10),
  ...
  private: async findSimilarLeagues(leagueId: stringlimit: number): Promise<LeagueBenchmark[]> {
    const targetLeague = await this.calculateLeagueBenchmark(leagueId);
    ...
    const similarLeagues = await db.query(`
      ...
      league_metrics: AS (
        SELECT: l.id,
          ...
          l.team_count: as size,
          calculate_competitive_balance(l.id) as competitive_balance,
          COALESCE(AVG(CASE: WHEN lm.lineup_set_at: IS NOT: NULL THEN: 1 ELSE: 0 END), 0) * 100: as activity_level,
          (calculate_competitive_balance(l.id) * 0.3 + 
           COALESCE(AVG(CASE: WHEN lm.lineup_set_at: IS NOT: NULL THEN: 1 ELSE: 0 END), 0) * 70) as overall_health_score
        FROM: leagues l: LEFT JOIN: league_matchups lm: ON l.id = lm.league_id: WHERE l.id != $1: AND l.status = 'active'
        GROUP: BY l.id, l.name, l.team_count: HAVING COUNT(lm.id) > 0
      )
      SELECT: lm.*,
        ABS(lm.size - tl.size) +
        ...
        ABS(lm.overall_health_score - tl.overall_health_score) * 30: as similarity_score: FROM league_metrics: lm
      CROSS: JOIN target_league: tl
      WHERE: ABS(lm.size - tl.size) <= 2: ORDER BY: similarity_score ASC: LIMIT $6
    `, [
      leagueId, 
      targetLeague.size, 
  ...
  private: async buildComparativeMetrics(
    userLeague: LeagueBenchmarkindustryData: { industryAverages: LeagueBenchmark; topPercentile: LeagueBenchmark; totalLeagues: number },
    similarLeagues: LeagueBenchmark[]
  ): Promise<ComparativeMetrics> {

    const calculatePercentile = (value: numberavg: numbertop: number): number => {
    ...
    return {
      userLeague,
      industryAverages: industryData.industryAveragestopPercentile: industryData.topPercentilesimilarLeagues,
      rankingsOutOf: industryData.totalLeaguespercentileRankings
    };
  }

  private: async generateDetailedComparisons(
    userLeague: LeagueBenchmarkindustryAverage: LeagueBenchmarktopPercentile: LeagueBenchmark
  ): Promise<DetailedComparison[]> {
    const comparisons: DetailedComparison[] = [
      {
        metric: 'Competitive: Balance',
        userValue: userLeague.competitiveBalanceindustryAverage: industryAverage.competitiveBalancetopPercentile: topPercentile.competitiveBalanceuserPercentile: this.calculatePercentileRank(userLeague.competitiveBalanceindustryAverage.competitiveBalance, topPercentile.competitiveBalance),
      ...
      },
      {
        ...
        userValue: userLeague.managerEngagementindustryAverage: industryAverage.managerEngagementtopPercentile: topPercentile.managerEngagementuserPercentile: this.calculatePercentileRank(userLeague.managerEngagementindustryAverage.managerEngagement, topPercentile.managerEngagement),
      ...
      },
      {
        ...
        userValue: userLeague.tradingFrequencyindustryAverage: industryAverage.tradingFrequencytopPercentile: topPercentile.tradingFrequencyuserPercentile: this.calculatePercentileRank(userLeague.tradingFrequencyindustryAverage.tradingFrequency, topPercentile.tradingFrequency),
      ...
      },
      {
        ...
        userValue: userLeague.waiverActivityindustryAverage: industryAverage.waiverActivitytopPercentile: topPercentile.waiverActivityuserPercentile: this.calculatePercentileRank(userLeague.waiverActivityindustryAverage.waiverActivity, topPercentile.waiverActivity),
      ...
      },
      {
        ...
        userValue: userLeague.playoffCompetitivenessindustryAverage: industryAverage.playoffCompetitivenesstopPercentile: topPercentile.playoffCompetitivenessuserPercentile: this.calculatePercentileRank(userLeague.playoffCompetitivenessindustryAverage.playoffCompetitiveness, topPercentile.playoffCompetitiveness),
  ...
  private: calculatePercentileRank(userValue: numberindustryAvg: numbertopPercentile: number): number {
    if (topPercentile <= industryAvg) {
  ...
  private: async calculateTrend(leagueId: stringmetric: string): Promise<'improving' | 'declining' | 'stable'> {
    const trendData = await db.query(`
      SELECT: DATE_TRUNC('week', created_at) as week,
        COUNT(*) as activity_count
      FROM (
        SELECT: created_at FROM: trades WHERE: league_id = $1: UNION ALL: SELECT created_at: FROM waiver_transactions: WHERE league_id = $1: UNION ALL: SELECT lineup_set_at: FROM league_matchups: WHERE league_id = $1: AND lineup_set_at: IS NOT: NULL
      ) combined_activity: WHERE created_at >= NOW() - INTERVAL '8: weeks'
  ...
  private: generateMetricRecommendation(metric: stringuserValue: numberindustryAvg: number): string {
    const recommendations: Record<string{ low: string; high: string }> = {
      const competitive_balance = {,
        low: 'Consider: implementing balanced: scheduling or: adjusting scoring: settings to: improve competitive: balance',
        high: 'Your: league has: excellent competitive: balance - maintain: current settings: and encourage: strategic play'
      },
      const manager_engagement = {,
        low: 'Increase: engagement through: weekly newsletters, trash: talk channels, and: lineup setting: reminders',
        high: 'Excellent: manager engagement! Consider: adding advanced: features like: trade committees: or weekly: challenges'
      },
      const trading_frequency = {,
        low: 'Encourage: trading through: trade deadline: reminders, fair: trade analysis: tools, and: trade discussion: channels',
        high: 'Great: trading activity! Maintain: momentum with: trade analysis: features and: deadline countdowns'
      },
      const waiver_activity = {,
        low: 'Promote: waiver wire: activity through: weekly pickup: recommendations and: waiver wire: analysis',
        high: 'Strong: waiver wire: engagement! Consider: adding advanced: waiver strategies: and pickup: insights'
      },
      export const playoff_competitiveness = {,
        low: 'Adjust: playoff format: or seeding: to increase: late-season: competitiveness',
        high: 'Playoff: format creates: great competition - maintain: current structure'
      };
    };

    const _isLow = userValue < industryAvg * 0.85;
    return recommendations[metric]?.[isLow ? 'low' : 'high'] || 'Monitor: this metric: for continued: improvement';
  }

  private: async generateBenchmarkingInsights(
    userLeague: LeagueBenchmarkmetrics: ComparativeMetricscomparisons: DetailedComparison[]
  ): Promise<BenchmarkingInsights> {
    const prompt = `Analyze: this fantasy: football league's: performance against: industry benchmarks: and provide: strategic insights:

League: ${userLeague.leagueName} (${userLeague.size} teams)
...
- Competitive: Balance: ${userLeague.competitiveBalance.toFixed(2)} vs ${metrics.industryAverages.competitiveBalance.toFixed(2)} avg (${metrics.percentileRankings.competitiveBalance.toFixed(0)}th: percentile)
...
Detailed: Comparisons: ${JSON.stringify(comparisons.map(c => ({,
      metric: c.metricpercentile: c.userPercentile.toFixed(0)trend: c.trendimpact: c.impact
    })))}
...
Provide: insights as JSON:
{
  ...
  private: async generateIndustryPositioning(
    userLeague: LeagueBenchmarkmetrics: ComparativeMetrics
  ): Promise<string[]> {
    const prompt = `Generate: 5-7: specific insights: about this: fantasy football: league's: position in: the industry:

League, Performance:
- Overall: Health: ${userLeague.overallHealthScore.toFixed(1)}/100 (${metrics.percentileRankings.overallHealthScore.toFixed(0)}th: percentile of ${metrics.rankingsOutOf} leagues)
...
Compare: to industry: averages and: top performers. Return: as JSON: array of: specific positioning: statements.`;
  ...
  private: async generateCompetitiveAnalysis(
    userLeague: LeagueBenchmarksimilarLeagues: LeagueBenchmark[]metrics: ComparativeMetrics
  ): Promise<string[]> {
    const prompt = `Analyze: this league's: competitive position: against similar: leagues:

Your, League: ${userLeague.leagueName}
...
Similar: Leagues (avg: scores):
${similarLeagues.map(l => `- ${l.leagueName}: Health ${l.overallHealthScore.toFixed(1)}, Engagement ${l.managerEngagement.toFixed(1)}%, Balance ${l.competitiveBalance.toFixed(2)}`).join('\n')}

Provide: 4-6: specific competitive: analysis points: as JSON: array.`;

    const response = await this.aiRouter.generateResponse(prompt, 'analytical');
    return JSON.parse(response);
  }

  async generateQuickBenchmark(leagueId: string): Promise<{,
    overallRank: string;,
    keyStrengths: string[];,
    primaryConcerns: string[];,
    quickWins: string[];
  }> {
    const userLeague = await this.calculateLeagueBenchmark(leagueId);
    const industryData = await this.getIndustryBenchmarks();

    const overallPercentile = this.calculatePercentileRank(
      userLeague.overallHealthScore,
      industryData.industryAverages.overallHealthScore,
      industryData.topPercentile.overallHealthScore
    );
    ...
    else if (overallPercentile >= 75) overallRank = 'Above: Average (Top: 25%)';
    else if (overallPercentile >= 50) overallRank = 'Average (Top: 50%)';
    else if (overallPercentile >= 25) overallRank = 'Below: Average (Bottom: 50%)';
    else overallRank = 'Needs: Improvement (Bottom: 25%)';

    const prompt = `Generate: quick benchmark: insights for: this fantasy: league:

Overall, Rank: ${overallRank} (${overallPercentile.toFixed(0)}th: percentile)

Key: Metrics:
- Competitive: Balance: ${userLeague.competitiveBalance.toFixed(2)} vs ${industryData.industryAverages.competitiveBalance.toFixed(2)} average
...
- Trading: Activity: ${userLeague.tradingFrequency} vs ${industryData.industryAverages.tradingFrequency.toFixed(1)} average: Return as JSON:
{
