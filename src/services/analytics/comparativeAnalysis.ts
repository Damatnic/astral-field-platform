...
export class ComparativeAnalysisService {
  ...
  async generateLeagueBenchmarking(leagueId: string, userId: string): Promise<{
  ...
  private async calculateLeagueBenchmark(leagueId: string): Promise<LeagueBenchmark> {
    const leagueData = await db.query(`
      WITH league_stats AS (
        ...
        FROM leagues l
        LEFT JOIN league_matchups lm ON l.id = lm.league_id
        LEFT JOIN trades t ON l.id = t.league_id AND t.status = 'completed'
        LEFT JOIN waiver_transactions wt ON l.id = wt.league_id
        WHERE l.id = $1
        GROUP BY l.id, l.name, l.team_count
      ),
      engagement_stats AS (
        SELECT 
          l.id,
      ...
      JOIN engagement_stats es ON ls.id = es.id
    `, [leagueId]);
    ...
    return {
      ...
      size: stats.size,
      competitiveBalance: stats.competitive_balance || 0,
      activityLevel,
      ...
      composite_scores AS (
        SELECT *,
          (competitive_balance * 0.25 + activity_level * 0.25 + 
           ...
           LEAST(trading_frequency / (size * 2), 1) * 0.1) * 100 as overall_health_score
        FROM league_metrics
      )
      SELECT 
  ...
  private async findSimilarLeagues(leagueId: string, limit: number): Promise<LeagueBenchmark[]> {
    ...
    const similarLeagues = await db.query(`
      ...
      SELECT 
        ...
        ABS(lm.overall_health_score - tl.overall_health_score) * 30 as similarity_score
      FROM league_metrics lm
      CROSS JOIN target_league tl
      WHERE ABS(lm.size - tl.size) <= 2
  ...
  private async buildComparativeMetrics(
    userLeague: LeagueBenchmark,
    industryData: { industryAverages: LeagueBenchmark; topPercentile: LeagueBenchmark; totalLeagues: number },
    similarLeagues: LeagueBenchmark[]
  ): Promise<ComparativeMetrics> {
    
    const calculatePercentile = (value: number, avg: number, top: number): number => {
    ...
    return {
      userLeague,
      industryAverages: industryData.industryAverages,
      topPercentile: industryData.topPercentile,
      similarLeagues,
      rankingsOutOf: industryData.totalLeagues,
      percentileRankings
    };
  }
  ...
  private async generateDetailedComparisons(
    userLeague: LeagueBenchmark,
    industryAverage: LeagueBenchmark,
    topPercentile: LeagueBenchmark
    ...
    const comparisons: DetailedComparison[] = [
      {
        ...
        industryAverage: industryAverage.competitiveBalance,
        topPercentile: topPercentile.competitiveBalance,
        userPercentile: this.calculatePercentileRank(userLeague.competitiveBalance, industryAverage.competitiveBalance, topPercentile.competitiveBalance),
      ...
      },
      {
        ...
        userValue: userLeague.managerEngagement,
        industryAverage: industryAverage.managerEngagement,
        topPercentile: topPercentile.managerEngagement,
        userPercentile: this.calculatePercentileRank(userLeague.managerEngagement, industryAverage.managerEngagement, topPercentile.managerEngagement),
      ...
      },
      {
        ...
        userValue: userLeague.tradingFrequency,
        industryAverage: industryAverage.tradingFrequency,
        topPercentile: topPercentile.tradingFrequency,
        userPercentile: this.calculatePercentileRank(userLeague.tradingFrequency, industryAverage.tradingFrequency, topPercentile.tradingFrequency),
      ...
      },
      {
        ...
        userValue: userLeague.waiverActivity,
        industryAverage: industryAverage.waiverActivity,
        topPercentile: topPercentile.waiverActivity,
        userPercentile: this.calculatePercentileRank(userLeague.waiverActivity, industryAverage.waiverActivity, topPercentile.waiverActivity),
      ...
      },
      {
        ...
        userValue: userLeague.playoffCompetitiveness,
        industryAverage: industryAverage.playoffCompetitiveness,
        topPercentile: topPercentile.playoffCompetitiveness,
        userPercentile: this.calculatePercentileRank(userLeague.playoffCompetitiveness, industryAverage.playoffCompetitiveness, topPercentile.playoffCompetitiveness),
  ...
  private calculatePercentileRank(userValue: number, industryAvg: number, topPercentile: number): number {
  ...
  private async calculateTrend(leagueId: string, metric: string): Promise<'improving' | 'declining' | 'stable'> {
    const trendData = await db.query(`
      SELECT 
        DATE_TRUNC('week', created_at) as week,
        COUNT(*) as activity_count
      FROM (
        SELECT created_at FROM trades WHERE league_id = $1
        UNION ALL
        SELECT created_at FROM waiver_transactions WHERE league_id = $1
        UNION ALL
        SELECT lineup_set_at FROM league_matchups WHERE league_id = $1 AND lineup_set_at IS NOT NULL
      ) combined_activity
    ...
    const recent = trendData.rows.slice(0, 2).reduce((sum, row) => sum + parseInt(row.activity_count), 0) / 2;
  ...
  private generateMetricRecommendation(metric: string, userValue: number, industryAvg: number): string {
    ...
    const isLow = userValue < industryAvg * 0.85;
...
Detailed Comparisons: ${JSON.stringify(comparisons.map(c => ({
      metric: c.metric,
      percentile: c.userPercentile.toFixed(0),
      trend: c.trend,
      impact: c.impact
    })))}
...
- Trading Activity: ${userLeague.tradingFrequency} vs ${industryData.industryAverages.tradingFrequency.toFixed(1)} average
