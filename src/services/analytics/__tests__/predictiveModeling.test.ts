...
import predictiveModelingService, { 
  PredictionModel, 
  PlayerProjection, 
  AdvancedMetrics, 
  WeatherImpact, 
...
describe('PredictiveModelingService', () => {
  ...
  describe('Player Projection Generation', () => {
    it('generates valid player projection with required fields', async () => {
    ...
    it('ensures projected points are positive', async () => {
      const projection = await predictiveModelingService.generatePlayerProjection('test-player', 8);
    ...
    it('validates confidence is within expected range', async () => {
      const projection = await predictiveModelingService.generatePlayerProjection('test-player', 8);
    ...
    it('ensures boom/bust probabilities are valid percentages', async () => {
      const projection = await predictiveModelingService.generatePlayerProjection('test-player', 8);
    ...
    it('provides meaningful key factors', async () => {
      const projection = await predictiveModelingService.generatePlayerProjection('test-player', 8);
    ...
    it('handles invalid player IDs gracefully', async () => {
      const projection = await predictiveModelingService.generatePlayerProjection('invalid-player', 8);
    ...
    it('handles invalid week numbers', async () => {
      const projection = await predictiveModelingService.generatePlayerProjection('test-player', 25);
  ...
  describe('Batch Projections', () => {
    it('generates projections for multiple players', async () => {
    ...
    it('handles empty player list', async () => {
      const projections = await predictiveModelingService.generateBatchProjections([], 8);
    ...
    it('continues processing when individual projections fail', async () => {
      const playerIds = ['valid-player', 'invalid-player', 'another-valid-player'];
      const projections = await predictiveModelingService.generateBatchProjections(playerIds, 8);
  ...
  describe('Injury Risk Calculation', () => {
    it('calculates injury risk with valid parameters', async () => {
      const injuryRisk = await predictiveModelingService.calculateInjuryRisk('test-player');
    ...
    it('ensures risk level is within valid range', async () => {
      const injuryRisk = await predictiveModelingService.calculateInjuryRisk('test-player');
    ...
    it('provides reasonable recovery timeline', async () => {
      const injuryRisk = await predictiveModelingService.calculateInjuryRisk('test-player');
    ...
    it('handles missing player data', async () => {
      const injuryRisk = await predictiveModelingService.calculateInjuryRisk('nonexistent-player');
  ...
  describe('Matchup Analysis', () => {
    it('analyzes matchup between two teams', async () => {
      const matchup = await predictiveModelingService.analyzeMatchup('HOME', 'AWAY', 8);
    ...
    it('provides reasonable home advantage', async () => {
      const matchup = await predictiveModelingService.analyzeMatchup('HOME', 'AWAY', 8);
    ...
    it('calculates pace adjustment correctly', async () => {
      const matchup = await predictiveModelingService.analyzeMatchup('HOME', 'AWAY', 8);
    ...
    it('provides key matchup insights', async () => {
      const matchup = await predictiveModelingService.analyzeMatchup('HOME', 'AWAY', 8);
  ...
  describe('Model Performance Evaluation', () => {
    const mockTestData = [
      { features: { recent_points_avg: 15, target_share: 0.2 }, actual: 16.5 },
      { features: { recent_points_avg: 12, target_share: 0.15 }, actual: 11.8 },
      { features: { recent_points_avg: 20, target_share: 0.25 }, actual: 22.1 }
    ];

    it('evaluates model performance with test data', async () => {
      const evaluation = await predictiveModelingService.evaluateModelPerformance(
        'main_projections', 
        mockTestData
      );
    ...
    it('calculates accuracy within reasonable bounds', async () => {
      const evaluation = await predictiveModelingService.evaluateModelPerformance(
        'main_projections', 
        mockTestData
      );
    ...
    it('provides feature importance rankings', async () => {
      const evaluation = await predictiveModelingService.evaluateModelPerformance(
        'main_projections', 
        mockTestData
      );
    ...
    it('handles empty test data', async () => {
      const evaluation = await predictiveModelingService.evaluateModelPerformance(
        'main_projections', 
        []
      );
  ...
  describe('Health Check', () => {
    it('performs health check on all models', async () => {
      const healthStatus = await predictiveModelingService.healthCheck();
    ...
    it('reports individual model status', async () => {
      const healthStatus = await predictiveModelingService.healthCheck();
    ...
    it('determines overall health based on model status', async () => {
      const healthStatus = await predictiveModelingService.healthCheck();
  ...
  describe('Edge Cases and Error Handling', () => {
    it('handles null player IDs', async () => {
      const projection = await predictiveModelingService.generatePlayerProjection('', 8);
      
      expect(projection).toBeDefined();
      expect(projection.playerId).toBe('');
    });

    it('handles negative week numbers', async () => {
      const projection = await predictiveModelingService.generatePlayerProjection('test-player', -1);
      
      expect(projection).toBeDefined();
      expect(projection.week).toBe(-1);
    });

    it('handles zero week numbers', async () => {
      const projection = await predictiveModelingService.generatePlayerProjection('test-player', 0);
      
      expect(projection).toBeDefined();
      expect(projection.week).toBe(0);
    });

    it('provides consistent fallback values', async () => {
      const projection1 = await predictiveModelingService.generatePlayerProjection('fallback-test', 8);
  ...
  describe('Performance and Caching', () => {
    it('completes projections within reasonable time', async () => {
      const startTime = Date.now();
    ...
    it('handles concurrent projection requests', async () => {
  ...
  describe('Data Quality and Validation', () => {
    it('validates projection data types', async () => {
      const projection = await predictiveModelingService.generatePlayerProjection('test-player', 8);
    ...
    it('ensures projection values are reasonable', async () => {
      const projection = await predictiveModelingService.generatePlayerProjection('test-player', 8);
