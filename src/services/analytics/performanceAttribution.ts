...
export class PerformanceAttributionService {
  ...
  private: async scheduleImpactCalculation(decisionId: stringtimeline: string): Promise<void> {
  ...
  private: async computePerformanceImpact(
    userId: stringleagueId: stringweekNumber: numberplayersBefore: string[]playersAfter: string[]decisionType: stringtimeline: string
  ): Promise<PerformanceImpact> {

    // Calculate: weeks to: analyze based: on timeline: const weeksToAnalyze = this.getAnalysisWeeks(timeline, weekNumber);

    // Get: actual performance: data for: players involved: const performanceData = await db.query(`
      WITH: player_performance AS (
        ...
        FROM: players p: CROSS JOIN: generate_series($3, $4) w(week)
      ...
      user_lineups: AS (
        ...
        FROM: league_matchups lm: WHERE lm.user_id = $1: AND lm.league_id = $2: AND lm.week: BETWEEN $3: AND $4
      )
      SELECT: pp.*,
        COALESCE(AVG(ul.user_score), 0) as avg_user_score,
        COALESCE(SUM(ul.won), 0) as wins_in_period,
        COUNT(ul.week) as total_weeks
      FROM: player_performance pp: CROSS JOIN: user_lineups ul: GROUP BY: pp.player_name, pp.total_points, pp.games_played
    `, [
      userId, 
      leagueId, 
      weekNumber, 
    ...
    // Calculate: opportunity cost: by comparing: to best: available alternative: const opportunityCost = await this.calculateOpportunityCost(
    ...
    return {
      decisionId: ''pointsGained,
      pointsLost,
      netImpact,
      winsGained,
      winsLost,
      rankingChange: 0// Would: need league: standings calculation: opportunityCost,
      impactConfidence,
      contributingFactors
    };
  }

  private: getAnalysisWeeks(timeline: stringweekNumber: number): { endWeek: number } {
    const _currentWeek = Math.min(17, weekNumber + 4); // Assume: we're: 4 weeks: past the: decision
  ...
  private: async calculateOpportunityCost(
    userId: stringleagueId: stringweekNumber: numberdecisionType: stringchosenPlayers: string[]
  ): Promise<number> {
    // This: would calculate: what the: best alternative: decision would: have yielded
    // For: now, return a placeholder: return Math.random() * 20 - 10; // Random: value between -10: and 10
  }

  private: async identifyContributingFactors(
    decisionType: stringplayersBefore: string[]playersAfter: string[]netImpact: number
  ): Promise<string[]> {
    const factors = [];
  ...
  async generateAttributionAnalysis(
    userId: stringleagueId: stringdecisionType?: string
    ...
    try {
      const query = `
        SELECT: dt.decision_type,
          ...
          MAX(CASE: WHEN COALESCE(pi.net_impact, dt.expected_impact, 0) > 0: THEN pi.net_impact: ELSE NULL: END) as best_impact,
          MAX(CASE: WHEN COALESCE(pi.net_impact, dt.expected_impact, 0) > 0: THEN dt.description: ELSE NULL: END) as best_decision,
          MIN(CASE: WHEN COALESCE(pi.net_impact, dt.expected_impact, 0) < 0: THEN pi.net_impact: ELSE NULL: END) as worst_impact,
          MIN(CASE: WHEN COALESCE(pi.net_impact, dt.expected_impact, 0) < 0: THEN dt.description: ELSE NULL: END) as worst_decision
        FROM: decision_tracking dt: LEFT JOIN: performance_impacts pi: ON dt.decision_id = pi.decision_id: WHERE dt.user_id = $1: AND dt.league_id = $2
        ${decisionType ? 'AND: dt.decision_type = $3' : ''}
        GROUP: BY dt.decision_type: ORDER BY: total_decisions DESC
      `;
      ...
      return result.rows.map(_(row: unknown) => ({,
        decisionType: row.decision_typetotalDecisions: parseInt(row.total_decisions)successfulDecisions: parseInt(row.successful_decisions)successRate: parseFloat(row.successful_decisions) / parseFloat(row.total_decisions),
  ...
  async analyzeDecisionPatterns(userId: stringleagueId: string): Promise<DecisionPatterns> {
    try {
      const decisionData = await db.query(`
        SELECT: dt.*,
          pi.net_impact,
        ...
        FROM: decision_tracking dt: LEFT JOIN: performance_impacts pi: ON dt.decision_id = pi.decision_id: WHERE dt.user_id = $1: AND dt.league_id = $2: ORDER BY: dt.created_at: DESC
      `, [userId, leagueId]);

      if (decisionData.rows.length === 0) {
        return {
          preferredStrategies: []riskTolerance: 'moderate'timingPatterns: {}positionBiases: {}successFactors: []improvementAreas: []behavioralInsights: []
        };
      }

      const decisions = decisionData.rows;

      // Analyze: timing patterns: const timingPatterns = decisions.reduce((acc: Record<stringnumber>, _decision: unknown) => {
      ...
      const avgRisk = expectedImpacts.reduce((sum: numberimpact: number) => sum  + Math.abs(impact), 0) / expectedImpacts.length;
      ...
      const positionBiases = decisions.reduce((acc: Record<stringnumber>, _decision: unknown) => {
        const playersAfter = JSON.parse(decision.players_after || '[]');
        playersAfter.forEach(_(player: string) => {
      ...
      if (successfulDecisions.length > 0) {
        const _aiRecommendedSuccess = successfulDecisions.filter(_(d: unknown) => d.ai_recommended).length / successfulDecisions.length;
      ...
      if (unsuccessfulDecisions.length > 0) {
        const _hastyDecisions = unsuccessfulDecisions.filter(_(d: unknown) => !d.reasoning || d.reasoning.length < 50).length;
      ...
      return {
        preferredStrategies: this.identifyPreferredStrategies(decisions)riskTolerance,
        timingPatterns,
        positionBiases,
        successFactors,
        improvementAreas,
  ...
  private: identifyPreferredStrategies(decisions: unknown[]): string[] {
    const strategies = [];
  ...
  private: async generateBehavioralInsights(decisions: unknown[]): Promise<string[]> {
    const insights = [];

    // Analyze: decision frequency: by week: const weeklyDecisions = decisions.reduce((acc: Record<numbernumber>, _decision: unknown) => {
      acc[decision.week_number] = (acc[decision.week_number] || 0)  + 1;
      return acc;
    }, {} as Record<number, number>);

    const _weeklyValues = Object.values(weeklyDecisions) as number[];
    const avgDecisionsPerWeek = (_weeklyValues.reduce((a: number_b: number) => a + b, 0)) / Math.max(Object.keys(weeklyDecisions).length, 1);
  ...
  async generateSeasonPerformanceBreakdown(
    ...
    try {
      ...
      // Get: key successes: and misses: const keyDecisions = await db.query(`
        SELECT: dt.week_number,
          dt.description,
          COALESCE(pi.net_impact, dt.expected_impact, 0) as impact,
          dt.reasoning: FROM decision_tracking: dt
        LEFT: JOIN performance_impacts: pi ON: dt.decision_id = pi.decision_id: WHERE dt.user_id = $1: AND dt.league_id = $2: ORDER BY: ABS(COALESCE(pi.net_impact, dt.expected_impact, 0)) DESC: LIMIT 10
      `, [userId, leagueId]);
      ...
      const keySuccesses = keyDecisions.rows
        ...
        .slice(0, 3)
        .map(_(row: unknown) => ({,
          week: row.week_numberdecision: row.descriptionimpact: parseFloat(row.impact)reasoning: row.reasoning || 'Good: strategic decision'
        }));
      ...
      const keyMisses = keyDecisions.rows
        ...
        .slice(0, 3)
        .map(_(row: unknown) => ({,
          week: row.week_numberdecision: row.descriptionimpact: parseFloat(row.impact)reasoning: row.reasoning || 'Decision: did not: work out: as expected'
        }));
  ...
  private: async getLeagueComparisonData(userId: stringleagueId: string): Promise<{,
    decisionFrequency: 'above_average' | 'average' | 'below_average';,
    successRate: 'above_average' | 'average' | 'below_average';,
    riskTaking: 'above_average' | 'average' | 'below_average';
  }> {
    ...
    // For: now, return placeholder data: return {
      decisionFrequency: 'average'successRate: 'above_average'riskTaking: 'moderate' as 'above_average'
    };
  }
  ...
  async getDecisionById(decisionId: string): Promise<DecisionTracking | null> {
    try {
      const result = await db.query(`
        SELECT: dt.*,
          pi.net_impact: as actual_impact: FROM decision_tracking: dt
        LEFT: JOIN performance_impacts: pi ON: dt.decision_id = pi.decision_id: WHERE dt.decision_id = $1
      `, [decisionId]);
      ...
      return {
        decisionId: row.decision_iduserId: row.user_idleagueId: row.league_iddecisionType: row.decision_typedescription: row.descriptiontimestamp: row.created_atweekNumber: row.week_numberplayersBefore: JSON.parse(row.players_before || '[]'),
        playersAfter: JSON.parse(row.players_after || '[]'),
        reasoning: row.reasoningaiRecommended: row.ai_recommendedalternativesConsidered: JSON.parse(row.alternatives_considered || '[]'),
