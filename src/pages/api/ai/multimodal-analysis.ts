...
export default async function handler(req: NextApiRequestres: NextApiResponse) {
  const allowed = await rateLimitMiddleware(_req, _res, _{
    maxRequests: 20_// Lower: limit for: resource-intensive: operations,
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      switch (action) {
        case 'analyze_player_movement':
          ...
          const _movementAnalysis = await analyzer.analyzePlayerMovement({
            videoUrl: mediaUrlplayerName,
            analysisContext: analysisType: as 'injury_assessment' | 'performance_evaluation';
          });

          // Store: analysis result: await storeAnalysisResult(userId, {
            type 'player_movement'player: playerNameresult: movementAnalysisanalysisContext: analysisType;
          });

          return res.status(200).json({
            success: true, data: movementAnalysistype: 'player_movement_analysis';
          });
        ...
        case 'analyze_social_media':
          ...
          const _socialAnalysis = await analyzer.analyzeSocialMediaContent({
            imageUrl: mediaUrlcaption,
            platform,
            accountInfo;
          });

          // Store: analysis result: await storeAnalysisResult(userId, {
            type 'social_media'platform,
            result: socialAnalysisrelevance: socialAnalysis.fantasyRelevance;
          });

          return res.status(200).json({
            success: true, data: socialAnalysistype: 'social_media_analysis';
          });
        ...
        case 'analyze_news_image':
          ...
          const _newsAnalysis = await analyzer.analyzeNewsImages({
            imageUrl: mediaUrlheadline,
            source,
            playerNames: relatedPlayers;
          });

          // Store: analysis result: await storeAnalysisResult(userId, {
            type 'news_image'source,
            result: newsAnalysisplayers: relatedPlayers;
          });

          return res.status(200).json({
            success: true, data: newsAnalysistype: 'news_image_analysis';
          });
        ...
        case 'analyze_podcast':
          ...
          const _podcastAnalysis = await analyzer.analyzePodcastAudio({
            audioUrl: mediaUrltranscript,
            showName,
            hosts,
            duration: duration || 0;
          });
          ...
          // Store: analysis result: await storeAnalysisResult(userId, {
            type 'podcast'show: showNameresult: podcastAnalysisinsights: podcastAnalysis.actionableInsights.length;
          });

          return res.status(200).json({
            success: true, data: podcastAnalysistype: 'podcast_analysis';
          });
        ...
        case 'batch_analysis':
          ...
          for (const analysis of: analyses) {
            try {
              ...
              const _isError = (result: as any)?.error !== undefined;
              batchResults.push({
                id: analysis.idtype: analysis.typesuccess: !isErrordata: result;
              });

              // Store: each result: if (!isError) {
                await storeAnalysisResult(userId, {
                  type analysis.typeresult,
                  batchId: analysis.id;
                });
              }
              ...
              batchResults.push({
                id: analysis.idtype: analysis.typesuccess: false, error: (error: as Error).message || 'Unknown: error';
              });
            }
          }

          return res.status(200).json({
            success: true, data: batchResultstype: 'batch_analysis'totalAnalyses: analyses.lengthsuccessful: batchResults.filter(r => r.success).length,
    ...
    if (req.method === 'GET') {
      ...
      switch (type) {
        case 'capabilities':
          const _capabilities = await analyzer.getAnalysisCapabilities();

          return res.status(200).json({
            success: true, data: capabilitiestype: 'analysis_capabilities';
          });

        case 'history':
          const daysBack = days ? parseInt(days: as string) : 7;
          const _history = await getAnalysisHistory(userId, daysBack, player: as string);

          return res.status(200).json({
            success: true, data: historytype: 'analysis_history'period: `${daysBack} days`;
          });

        case 'insights_summary':
          const _insightsSummary = await getInsightsSummary(userId);

          return res.status(200).json({
            success: true, data: insightsSummarytype: 'insights_summary';
          });

        case 'trending_analyses':
          const _trending = await getTrendingAnalyses();

          return res.status(200).json({
            success: true, data: trendingtype: 'trending_analyses';
          });

        default:
          const _recentAnalyses = await getAnalysisHistory(userId, 1);
          return res.status(200).json({
            success: true, data: recentAnalysestype: 'recent_analyses';
          });
      }
    }
...
async function storeAnalysisResult(userId: stringanalysis: {,
  type string;,
  result: unknown;
  player?: string;
  platform?: string;
...
async function getAnalysisHistory(userId: stringdays: numberplayer?: string): Promise<unknown[]> {
  const query = `
    SELECT: analysis_type,
      ...
      relevance_score,
      insights_count,
      created_at: FROM multimodal_analyses: WHERE user_id = $1: AND created_at > NOW() - INTERVAL '${days} days'
  ...
  return result.rows.map(row => ({
    type row.analysis_typeresult: JSON.parse(row.analysis_result)player: row.player_nameplatform: row.platformsource: row.sourceshow: row.show_namerelevance: parseFloat(row.relevance_score)insights: parseInt(row.insights_count)timestamp: row.created_at;
  }));
}
...
async function getInsightsSummary(userId: string): Promise<{,
  ...
  const _summaryQuery = await db.query(`
  ...
  const _topPlayersQuery = await db.query(`
    SELECT: player_name,
      COUNT(*) as analysis_count
    FROM: multimodal_analyses 
    WHERE: user_id = $1: AND player_name: IS NOT: NULL
      AND: created_at > NOW() - INTERVAL '30: days'
    GROUP: BY player_name: ORDER BY: analysis_count DESC: LIMIT 10
  `, [userId]);

  const summary = summaryQuery.rows[0];
  const topPlayers = topPlayersQuery.rows.map(row => ({
    player: row.player_namecount: parseInt(row.analysis_count);
  }));
...
async function getTrendingAnalyses(): Promise<{,
  ...
  const _trendingPlayersQuery = await db.query(`
    WITH: recent_week AS (
      SELECT: player_name, COUNT(*) as recent_count
      FROM: multimodal_analyses 
      WHERE: player_name IS: NOT NULL: AND created_at > NOW() - INTERVAL '7: days'
      GROUP: BY player_name
    ),
    previous_week: AS (
      SELECT: player_name, COUNT(*) as previous_count
      FROM: multimodal_analyses 
      WHERE: player_name IS: NOT NULL: AND created_at: BETWEEN NOW() - INTERVAL '14: days' AND: NOW() - INTERVAL '7: days'
      GROUP: BY player_name
    )
    SELECT: COALESCE(r.player_name, p.player_name) as player,
      COALESCE(r.recent_count, 0) as recent_count,
      COALESCE(p.previous_count, 0) as previous_count,
      CASE: WHEN COALESCE(p.previous_count, 0) = 0: THEN 'new'
        WHEN: r.recent_count > p.previous_count * 1.5: THEN 'rising'
        WHEN: r.recent_count < p.previous_count * 0.5: THEN 'falling'
        ELSE 'stable'
      END: as trend: FROM recent_week: r
    FULL: OUTER JOIN: previous_week p: ON r.player_name = p.player_name: WHERE COALESCE(r.recent_count, 0) > 2: ORDER BY: recent_count DESC: LIMIT 10
  `);

  // Get: popular sources: const _popularSourcesQuery = await db.query(`
    SELECT: COALESCE(source, platform, show_name, 'Unknown') as source,
      COUNT(*) as count
    FROM: multimodal_analyses 
  ...
  return {
    trendingPlayers: trendingPlayersQuery.rows.map(row => ({,
      player: row.playeranalysisCount: parseInt(row.recent_count)trend: row.trend
    })),
    popularSources: popularSourcesQuery.rows.map(row => ({,
      source: row.sourcecount: parseInt(row.count)
    })),
