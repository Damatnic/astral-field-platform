...
describe('WebSocket Integration Tests', () => {
  ...
  describe('Connection Management', () => {
    ...
    test('should handle connection errors gracefully', (done) => {
      const badClient = Client(serverAddress, {
  ...
  describe('League Room Management', () => {
    ...
    test('should handle manual room joining/leaving', (done) => {
      ...
      setTimeout(() => {
        const connections = webSocketManager.getLeagueConnections('test-league-1');
  ...
  describe('Chat Messaging', () => {
    test('should broadcast chat messages to league members', (done) => {
      const testMessage = {
    ...
    test('should handle GIF messages', (done) => {
      const gifMessage = {
        leagueId: 'test-league-1',
        roomType: 'general',
        message: '',
  ...
  describe('Direct Messaging', () => {
    test('should send direct messages between users', (done) => {
      const dmData = {
    ...
    test('should handle DM reactions', (done) => {
      const reactionData = {
        messageId: 'test-dm-message-id',
        emoji: '👍'
      };
  ...
  describe('Live Game Commentary', () => {
    ...
    test('should handle play reactions', (done) => {
      const reactionData = {
        playId: 'test-play-id',
        emoji: '🔥',
  ...
  describe('Typing Indicators', () => {
    ...
    test('should handle typing stop events', (done) => {
      const typingData = {
        leagueId: 'test-league-1',
        roomType: 'general'
      };
  ...
  describe('Notifications', () => {
    test('should deliver real-time notifications', (done) => {
      const notification = {
    ...
    test('should handle player injury notifications', (done) => {
      const injuryData = {
        playerId: 'player-123',
      ...
      setTimeout(() => {
        webSocketManager.broadcastPlayerUpdate({
          ...
          status: 'injured',
          stats: { injury: injuryData.injury }
        });
      }, 100);
    });
  });
  ...
  describe('Error Handling', () => {
    test('should handle unauthorized league access', (done) => {
      const unauthorizedMessage = {
        leagueId: 'unauthorized-league',
  ...
  describe('Performance', () => {
    ...
    test('should maintain low latency under load', (done) => {
      const testMessage = {
        leagueId: 'test-league-1',
  ...
  describe('Connection Recovery', () => {
    ...
    test('should maintain room memberships after reconnection', (done) => {
      const testMessage = {
        leagueId: 'test-league-1',
      ...
      clientSocket2.on('league_message', (data) => {
        if (data.message === testMessage.message) {
...
jest.mock('jsonwebtoken', () => ({
  verify: jest.fn((token: string) => {
    if (token === 'mock-jwt-token-user1') {
      return { userId: 'test-user-1' };
    } else if (token === 'mock-jwt-token-user2') {
      return { userId: 'test-user-2' };
    }
...
jest.mock('../lib/database', () => ({
  database: {
    query: jest.fn().mockImplementation((query: string, params: any[]) => {
      ...
      if (query.includes('SELECT id, username FROM users')) {
      ...
      if (query.includes('SELECT DISTINCT l.id as league_id')) {
        return {
          rows: [
