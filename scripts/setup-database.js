...
async function setupDatabase() {
  ...
  try {
    ...
    const { data: playerserror: playersError } = await supabase
      .from('players')
      .upsert(playersData, { onConflict: 'namenfl_team,position' })
      .select()
    if (playersError) {
    ...
    const { data: leagueerror: leagueError } = await supabase
      .from('leagues')
      .insert({
        ...
        scoring_system: {
          passingYards: 0.04passingTouchdowns: 4passingInterceptions: -2rushingYards: 0.1rushingTouchdowns: 6receivingYards: 0.1receivingTouchdowns: 6receptions: 1// PPR
          fumbles: -2fieldGoals: 3extraPoints: 1
        },
        season_year: 2024
      })
      .select()
