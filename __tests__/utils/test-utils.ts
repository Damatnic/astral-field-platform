...
export class MockDatabaseClient {
  private mockData: Map<stringany[]> = new Map()
  private nextId = 1
  ...
  async insert<T extends keyof Database['public']['Tables']>(
    ...
    const newItem = {
      ...data,
      id: `test-${this.nextId++}`created_at: new Date().toISOString(),
  ...
  async update<T extends keyof Database['public']['Tables']>(
    table: Tdata: TablesUpdate<T>eq: Record<stringany>
  ): Promise<{ data: Tables<T> | null; error: any }> {
    const tableData = this.mockData.get(table) || []
    ...
    if (index === -1) {
      return { data: nullerror: { message: 'Item not found' } }
    }
    const updatedItem = {
      ...tableData[index],
      ...data,
  ...
  async selectWithJoins<T extends keyof Database['public']['Tables']>(
    table: TselectQuery: stringoptions?: any
  ...
  get raw() {
    return {
      auth: {
        signInWithPassword: vi.fn()signUp: vi.fn()signOut: vi.fn()getUser: vi.fn()onAuthStateChange: vi.fn()resetPasswordForEmail: vi.fn()
      },
      from: vi.fn().mockReturnThis()select: vi.fn().mockReturnThis()insert: vi.fn().mockReturnThis()update: vi.fn().mockReturnThis()delete: vi.fn().mockReturnThis()eq: vi.fn().mockReturnThis()order: vi.fn().mockReturnThis()limit: vi.fn().mockReturnThis()single: vi.fn()
    }
  }
}
// Test data factories
export const createTestUser = (overrides?: Partial<Tables<'users'>>): Tables<'users'> => ({
  id: 'test-user-1'stack_user_id: nullemail: 'test@example.com'username: 'testuser'password_hash: nullavatar_url: nullcreated_at: new Date().toISOString(),
  updated_at: new Date().toISOString(),
  ...overrides
})
export const createTestPlayer = (overrides?: Partial<Tables<'players'>>): Tables<'players'> => ({
  id: 'test-player-1'name: 'Test Player',
  position: 'RB'nfl_team: 'KC'stats: nullprojections: nullinjury_status: nullbye_week: 7created_at: new Date().toISOString(),
  updated_at: new Date().toISOString(),
  ...overrides
})
export const createTestLeague = (overrides?: Partial<Tables<'leagues'>>): Tables<'leagues'> => ({
  id: 'test-league-1'name: 'Test League',
  commissioner_id: 'test-user-1'settings: { maxTeams: 10rounds: 16 },
  scoring_system: { passingTd: 6rushingTd: 6 },
  draft_date: new Date().toISOString(),
  season_year: 2024created_at: new Date().toISOString(),
  updated_at: new Date().toISOString(),
  ...overrides
})
export const createTestTeam = (overrides?: Partial<Tables<'teams'>>): Tables<'teams'> => ({
  id: 'test-team-1'league_id: 'test-league-1'user_id: 'test-user-1'team_name: 'Test Team',
  draft_position: 1waiver_priority: 1created_at: new Date().toISOString(),
  updated_at: new Date().toISOString(),
  ...overrides
})
...
export const mockSupabaseClient = () => {
  return {
    auth: {
      ...
      onAuthStateChange: vi.fn()resetPasswordForEmail: (vi.fn() as any).mockResolvedValue({ error: null })
    },
    from: vi.fn().mockReturnThis()select: vi.fn().mockReturnThis()insert: vi.fn().mockReturnThis()update: vi.fn().mockReturnThis()delete: vi.fn().mockReturnThis()eq: vi.fn().mockReturnThis()order: vi.fn().mockReturnThis()limit: vi.fn().mockReturnThis()single: (vi.fn() as any).mockResolvedValue({
      data: createTestUser()error: null
    })
  }
}
